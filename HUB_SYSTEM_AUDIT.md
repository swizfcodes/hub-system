...
if (!biz) throw Object.assign(new Error("Business context required"), { status: 400 });
```
...
```js
const rows = await repo.list(client, { business: biz, search, type, limit, offset }); // filtered
