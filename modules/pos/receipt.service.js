...
function buildTemplateData(business, tx) {
  ...
  const lines_html = lines.map((l, i) =>
    `<tr class="${i % 2 === 0 ? 'even' : ''}"><td>${l.description || ''}</td><td class="c">${l.quantity}</td><td class="r">${l.unit_price}</td><td class="r">${l.line_total}</td></tr>`
  ).join('');
