...
function derivePeriod(periodLabel) {
  ...
  if (/^\d{4}$/.test(periodLabel)) {
    return { start: `${periodLabel}-01-01`, end: `${periodLabel}-12-31`, kind: "year" };
  }
...
async function computeForType(client, business, taxType, periodLabel) {
  ...
  const base = { tax_type: taxType, period_label: periodLabel, period_start: start, period_end: end, currency: "NGN" };
...
async function computeVat(client, business, base, start, end) {
  const flows = await repo.getAccountFlows(client, ["2210", "2220"], start, end);
  ...
  const lines = await repo.getAccountLines(client, ["2210", "2220"], start, end);
  const warnings = await dataWarnings(client, start, end);
  if (input === 0) {
    warnings.push("No input VAT recorded — purchases aren't splitting recoverable VAT (account 2220). Net VAT may be overstated.");
  }
  ...
  return {
    ...
    summary: {
      ...
      input_vat: input,
      net_payable: net,
      note: net < 0 ? `Net credit of ₦${Math.abs(net).toLocaleString()} carries forward` : null,
    },
    meta: { rate: VAT_DEFAULT_RATE, deadline: "21st of next month (NRS portal)" },
...
async function computeWht(client, base, start, end) {
  const flows = await repo.getAccountFlows(client, ["2340", "1340"], start, end);
  ...
  const lines = await repo.getAccountLines(client, ["2340", "1340"], start, end);
  const warnings = [];
  if (withheld === 0) {
    warnings.push("No WHT withheld this period — supplier payments aren't deducting WHT (account 2340). Required for vendors above the SME threshold.");
  }
  return {
    ...base,
    computed_amount: withheld,
    summary: { wht_withheld_to_remit: withheld, wht_suffered_creditable: suffered },
...
async function computePaye(client, base, start, end) {
  ...
  return {
    ...
    meta: { deadline: "10th of next month (Lagos LIRS e-Tax)", jurisdiction: "Lagos State" },
    lines: lines.map(fmtLine),
    warnings: [],
  };
}
...
async function computeCit(client, base, start, end) {
  ...
  const warnings = await dataWarnings(client, start, end);
  if (depreciation > 0) {
    warnings.push("Book depreciation added back; enter capital allowances as an adjustment (no fixed-asset register yet).");
  }
  ...
  return {
    ...
    meta: { deadline: "Within 6 months of financial year-end", entity: profile.legal_name, tin: profile.tin },
    lines: breakdown,
    warnings,
  };
}
...
function fmtLine(r) {
  return {
    date: r.entry_date, entry: r.entry_number, description: r.description,
    source: r.reference_type, account: `${r.account_code} ${r.account_name}`,
...
async function dataWarnings(client, start, end) {
  ...
  if (n > 0) w.push(`${n} confirmed campaign order(s) in this period have no journal entry — revenue, VAT and COGS are understated until the storefront posts to the ledger.`);
  return w;
}
...
async function updateProfile(business, data, user) {
  return withBusinessContext(business, async (client) => {
    ...
    await auditService.log(client, {
      ...
      business, module: "tax", action: "edit", table: "tax_profile",
      recordId: p?.profile_id || null, after: p,
    });
    return p;
  });
}
...
async function saveDraft(business, taxType, periodLabel, user) {
  return withBusinessContext(business, async (client) => {
    ...
    const existing = await repo.findFilingByTypePeriod(client, taxType, periodLabel);
    if (existing && ["filed", "paid"].includes(existing.status)) {
      throw bad(`This ${taxType} return for ${periodLabel} is already ${existing.status} and cannot be recomputed.`);
    }

    const adjustments = existing ? await repo.sumAdjustments(client, existing.filing_id) : 0;
    ...
    const fields = {
      ...
      final_amount: final,
      workpaper: wp,
      status: existing ? existing.status : (taxType === "CIT" && wp.summary?.small_company ? "exempt" : "draft"),
    };
...
async function addAdjustment(business, filingId, data, user) {
  return withBusinessContext(business, async (client) => {
    ...
    await repo.addAdjustment(client, {
      filing_id: filingId, label: data.label, amount: money(data.amount),
      reason: data.reason, created_by: user.user_id,
    });
    ...
    const updated = await repo.updateFiling(client, filingId, {
      adjustment_amount: adjustments, final_amount: final,
    });
    return { ...updated, adjustments: await repo.listAdjustments(client, filingId) };
  });
}
...
async function confirm(business, filingId, user) {
  return withBusinessContext(business, async (client) => {
    ...
    if (filing.status !== "draft" && filing.status !== "exempt") {
      throw bad(`Only a draft can be confirmed (current status: ${filing.status}).`);
    }
    const updated = await repo.updateFiling(client, filingId, { status: "reviewed" });
    await auditService.log(client, {
      userId: user.user_id, userName: user.display_name || "staff",
      business, module: "tax", action: "approve", table: "tax_filings",
      recordId: filingId, after: { status: "reviewed" },
    });
    return updated;
  });
}
...
async function markFiled(business, filingId, { filing_reference }, user) {
  return withBusinessContext(business, async (client) => {
    ...
    const updated = await repo.updateFiling(client, filingId, {
      status: "filed", filing_reference: filing_reference || null,
      filed_at: new Date().toISOString(), filed_by: user.user_id,
    });
    await auditService.log(client, {
      userId: user.user_id, userName: user.display_name || "staff",
      business, module: "tax", action: "edit", table: "tax_filings",
      recordId: filingId, after: { status: "filed", filing_reference },
    });
    return updated;
  });
}
...
async function settle(business, filingId, { bank_account_code, payment_date }, user) {
  return withBusinessContext(business, async (client) => {
    const filing = await repo.findFilingById(client, filingId);
    if (!filing) throw Object.assign(new Error("Filing not found"), { status: 404 });
    if (filing.status === "paid") throw bad("Filing already settled.");
    if (filing.status === "exempt") throw bad("Nothing to settle — this return is exempt/nil.");

    const bankCode = bank_account_code || "1210";
    const bank = await journalService.getAccountId(client, bankCode);
    if (!bank) throw bad(`Bank account ${bankCode} not found in the chart of accounts.`);
    ...
    if (filing.tax_type === "VAT") {
      ...
      const net = money(output - input);
      if (net <= 0) throw bad("Net VAT is a credit — nothing to remit. Carry it forward.");
      ...
      if (input > 0 && inAcc) lines.push({ account_id: inAcc, debit: 0, credit: input });
      lines.push({ account_id: bank, debit: 0, credit: net });
    } else {
      const codeByType = { PAYE: "2310", WHT: "2340", CIT: "2350", DEV_LEVY: "2360" };
      const liabAcc = await journalService.getAccountId(client, codeByType[filing.tax_type]);
      const amount = money(filing.final_amount);
      if (!liabAcc) throw bad(`Liability account ${codeByType[filing.tax_type]} not found.`);
    ...
    const updated = await repo.updateFiling(client, filingId, {
      status: "paid", paid_at: new Date().toISOString(), settlement_entry_id: entry.entry_id,
      filed_at: filing.filed_at || new Date().toISOString(),
    });
    await auditService.log(client, {
      userId: user.user_id, userName: user.display_name || "staff",
      business, module: "tax", action: "approve", table: "tax_filings",
      recordId: filingId, after: { status: "paid", entry: entry.entry_number },
    });
...
async function exportCsv(business, filingId) {
  return withBusinessContext(business, async (client) => {
    ...
    if (filing.tax_type === "CIT") {
      ...
      for (const l of wp.lines || []) rows.push([l.name, l.code, l.type, l.amount]);
    } else {
      rows.push(["Date", "Entry", "Description", "Source", "Account", "Debit", "Credit"]);
      for (const l of wp.lines || []) {
        rows.push([l.date, l.entry, l.description, l.source, l.account, l.debit, l.credit]);
      }
    }
    const csv = rows
...
async function dashboard(business) {
  return withBusinessContext(business, async (client) => {
    ...
    const deadlines = [
      { tax_type: "PAYE", period_label: prior, due: `${y}-${String(m + 1).padStart(2, "0")}-10`, authority: "Lagos LIRS" },
      { tax_type: "VAT", period_label: prior, due: `${y}-${String(m + 1).padStart(2, "0")}-21`, authority: "NRS" },
      { tax_type: "WHT", period_label: prior, due: `${y}-${String(m + 1).padStart(2, "0")}-21`, authority: "NRS" },
    ];
...
module.exports = {
  preview, getProfile, updateProfile,
  saveDraft, getFiling, listFilings,
  addAdjustment, confirm, markFiled, settle,
  exportCsv, dashboard,
};
