...
async function appendToHTML(html, userId, business) {
  ...
  try {
    ...
    // Branded layouts carry a slot inside the white content cell — the
    // signature must render there, not after </html> where clients
    // paint it detached from the card on the page background.
    const slot = "<!--hub-signature-slot-->";
    if (html && html.includes(slot)) {
