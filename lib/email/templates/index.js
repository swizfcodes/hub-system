...
function delivery_dispatched(data, brand) {
  const itemRows = (data.items || [])
    .map(
      (item) => `
      <tr>
        <td style="padding:6px 10px;font-size:13px;color:#333;border-bottom:1px solid #eee;">${esc(item.description)}</td>
        <td style="padding:6px 10px;font-size:13px;color:#333;text-align:right;border-bottom:1px solid #eee;">× ${esc(item.quantity)}</td>
      </tr>`,
    )
    .join("");
  ...
  return {
    ...
    body: `
      ...
      ${
        data.driver_name || data.driver_phone
          ? `<table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:16px 0;width:100%;">
              <tr><td style="padding:14px 18px;background:${brand.secondary_colour};border-radius:8px;">
                <p style="margin:0;font-size:12px;color:#888;text-transform:uppercase;letter-spacing:0.5px;">Your driver</p>
                <p style="margin:6px 0 0 0;font-size:15px;font-weight:bold;color:#333;">${esc(data.driver_name || "—")}</p>
                ${data.driver_phone ? `<p style="margin:2px 0 0 0;font-size:13px;color:#555;">${esc(data.driver_phone)}</p>` : ""}
              </td></tr>
            </table>`
          : ""
      }
      ${
        itemRows
          ? `<table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="margin:14px 0;border:1px solid #eee;border-radius:8px;border-collapse:separate;overflow:hidden;">${itemRows}</table>`
          : ""
      }
      <p style="font-size:13px;color:#666;">Delivering to: ${esc(data.delivery_address)}</p>
      ${
        data.signing_url
          ? `<p>When your order arrives, please confirm receipt by signing below — it takes a few seconds on your phone:</p>
             ${ctaButton("Confirm receipt & sign", data.signing_url, brand.accent_colour)}
             <p style="font-size:12px;color:#999;">The signing link is valid for 48 hours.</p>`
          : ""
      }
...
// ─────────────────────────────────────────────────────────────
// 12. PASSWORD RESET OTP — self-service "forgot password" code
// ─────────────────────────────────────────────────────────────
  ...
  return {
    ...
    body: `
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:28px auto;">
        <tr>
          <td align="center" style="padding:18px 40px;background:${brand.secondary_colour};border:1px dashed ${brand.accent_colour};border-radius:10px;">
            <span style="font-family:'Courier New',monospace;font-size:32px;font-weight:bold;letter-spacing:10px;color:#1a1a1a;">${esc(data.otp)}</span>
          </td>
        </tr>
      </table>
...
// ─────────────────────────────────────────────────────────────
// 13. PASSWORD CHANGED — security notice after a successful reset
// ─────────────────────────────────────────────────────────────
