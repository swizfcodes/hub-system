...
function receipt(data, brand) {
  return {
    subject: `Your receipt from ${brand.display_name} — ${data.transaction_number}`,
    preview: `Thank you for your purchase. Your receipt is attached.`,
    body: `
      <h2 style="font-family:Georgia,serif;font-size:22px;font-weight:normal;color:#333;margin:0 0 16px 0;">
        Your receipt
      </h2>
      <p>Dear ${esc(data.contact_name || "valued customer")},</p>
      <p>Thank you for your purchase. Please find your receipt attached for your records.</p>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="margin:20px 0;width:100%;">
        <tr>
          <td style="padding:16px 20px;background:${brand.secondary_colour};border-radius:8px;">
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="font-size:13px;color:#666;padding:4px 0;">Receipt</td>
                <td style="font-size:13px;font-weight:bold;text-align:right;color:#333;">${esc(data.transaction_number)}</td>
              </tr>
              <tr>
      ...
      <p>We appreciate your business and look forward to seeing you again.</p>
      <p style="color:#888;font-size:13px;">— The ${esc(brand.display_name)} Team</p>
    `,
  };
}
