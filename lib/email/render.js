...
function renderEmail(templateName, business, data) {
  ...
  const { subject, body } = template(data, brand);
  const html = wrapInLayout(body, brand);
  return { subject, html };
}
...
function resolveBrand(business) {
  ...
  return {
    ...
    hub_base_url:     config.app.hubBaseUrl || "",
  };
}
 ...
 */
function wrapInLayout(bodyHtml, brand) {
  ...
  const footerParts = [];
  if (brand.address) footerParts.push(escapeHtml(brand.address));
  if (brand.phone)   footerParts.push(escapeHtml(brand.phone));
          ...
          <tr>
            <td style="background-color:${secondary};padding:24px 32px;border-top:1px solid rgba(0,0,0,0.06);">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
                ${socialIconsHtml}
                ${footerParts.length ? `<tr><td align="center" style="font-family:${fontBody};font-size:12px;line-height:1.5;color:#888888;padding:8px 0;">
                  ${footerParts.join("&nbsp;&nbsp;·&nbsp;&nbsp;")}
                </td></tr>` : ""}
                ${brand.email_footer_text ? `<tr><td align="center" style="font-family:${fontBody};font-size:11px;line-height:1.5;color:#aaaaaa;padding:8px 0 0 0;">
