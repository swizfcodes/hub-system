...
export const supplierInviteSchema = z.object({
  ...
  primary_phone:z.string().regex(/^[+0-9()\-\s]{6,20}$/).optional().or(z.literal('')),
  whatsapp_number: z.string().regex(/^[+0-9()\-\s]{6,20}$/).optional().or(z.literal('')),
  // Supplier-specific:
...
export const rfqCreateSchema = z.object({
  ...
  invited_supplier_ids: z.array(z.string().uuid()).min(1, 'Pick at least one supplier'),
...
export const grLineSchema = z.object({
  ...
  { message: 'Accepted + rejected must equal received', path: ['quantity_received'] },
).refine(
  (v) => v.quantity_rejected === 0 || (v.rejection_reason && v.rejection_reason.trim().length > 0),
  { message: 'Add a reason when rejecting goods', path: ['rejection_reason'] },
);
