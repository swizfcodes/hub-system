...
export const createEventSchema = z.object({
  ...
  event_type:      z.enum(['meeting','viewing','appointment','delivery','task','reminder','other']),
...
export const createTaskSchema = z.object({
  ...
  description:    z.string().max(2000).optional().or(z.literal('')),
  status:         z.enum(['inbox','today','this_week','this_month','later','done','cancelled']).default('inbox'),
