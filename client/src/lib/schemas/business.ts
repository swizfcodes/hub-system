...
export const stepIdentitySchema = z.object({
  business_key:  z.string().regex(/^[a-z][a-z0-9_]{1,30}$/, 'Lowercase, letters/digits/underscores, must start with a letter'),
...
export const stepBrandingSchema = z.object({
  ...
  accent_colour:       z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Must be a hex like #C9A86C'),
...
export const stepProvisioningSchema = z.object({
  ...
  prefix:           z.string().regex(/^[A-Z]{2,5}$/, '2–5 uppercase letters').optional().or(z.literal('')),
});
...
export const businessCreateSchema = stepIdentitySchema
  ...
  .refine(
    ...
    { message: 'A 2–5 letter document prefix is required when provisioning a new schema', path: ['prefix'] },
  );
export type BusinessCreateValues = z.infer<typeof businessCreateSchema>;

// ── Edit (Profile tab) ───────────────────────────────────────
export const profilePatchSchema = stepIdentitySchema.omit({ business_key: true });
