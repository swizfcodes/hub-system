...
export const ADJUSTMENT_TYPES = ['count', 'write_off', 'damage', 'found', 'correction'] as const;
export const MOVEMENT_TYPES = [
  'received', 'sold', 'pos_sale',
  'returned_from_customer', 'returned_to_supplier',
  'transferred_out', 'transferred_in',
  'consigned_out', 'consigned_returned',
  'reserved', 'reservation_released',
  'written_off', 'damaged', 'sample', 'adjustment',
] as const;
...
export const transferCreateSchema = z.object({
  ...
  (v) => v.from_location_id !== v.to_location_id,
  { message: 'From and To must differ', path: ['to_location_id'] },
);
...
export const manualExitSchema = z.object({
  ...
  movement_type: z.enum(['sample', 'consigned_out', 'written_off', 'damaged', 'returned_to_supplier']),
...
export const manufacturerSchema = z.object({
  ...
  (v) => v.auto_generate_sku || (v.manufacturer_code && v.manufacturer_code.length > 0),
  { message: 'Provide a manufacturer code or tick "auto-generate"', path: ['manufacturer_code'] },
);
