...
export const productCreateSchema = z.object({
  sku:               z.string().min(1, 'Required').max(60).regex(/^[A-Z0-9-]+$/i, 'Letters, digits, dashes only'),
  ...
  { message: 'Minimum selling price must be ≤ selling price', path: ['min_selling_price'] },
);
...
export const LOCATION_TYPES = ['warehouse', 'showroom', 'pos_terminal', 'retail_partner', 'transit'] as const;
