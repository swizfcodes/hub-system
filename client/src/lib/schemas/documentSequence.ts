...
export const documentSequenceSchema = z.object({
  ...
  prefix:        z.string().regex(/^[A-Z][A-Z0-9-]*$/, 'Uppercase alphanumeric with dashes (e.g. JWL-INV)'),
...
export const resetSequenceSchema = z.object({
  ...
  reset_reason: z.string().min(10, 'Please explain (≥10 chars) why this sequence is being reset'),
});
