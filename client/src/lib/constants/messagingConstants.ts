...
export const PLATFORM_META: Record<Platform | 'internal', {
  label: string; color: string; bg: string; icon: string;
}> = {
  whatsapp_business_account: { label: 'WhatsApp', color: '#25D366', bg: '#25D36615', icon: '💬' },
  instagram:                 { label: 'Instagram', color: '#E1306C', bg: '#E1306C15', icon: '📷' },
  page:                      { label: 'Messenger', color: '#0084FF', bg: '#0084FF15', icon: '💙' },
  email:                     { label: 'Email',     color: '#4E9AF1', bg: '#4E9AF115', icon: '📧' },
  internal:                  { label: 'Internal',  color: '#C9A86C', bg: '#C9A86C15', icon: '🏢' },
};
...
export const INBOX_TABS = [
  ...
  { key: 'internal',  label: 'Internal',  channelType: 'group',           platform: 'internal'  },
] as const;
...
export function getChannelDisplayName(channel: { name?: string | null; channel_type: string; members?: any[] | null }): string {
  if (channel.name) return channel.name;
  if (channel.channel_type === 'direct' && channel.members?.length) {
    return channel.members.map((m) => m.display_name).filter(Boolean).join(', ') || 'Direct Message';
  }
  return 'Conversation';
}
...
export function getChannelPlatform(channel: { channel_type: string; metadata?: any }): Platform | 'internal' {
  if (channel.channel_type === 'group' || channel.channel_type === 'direct') return 'internal';
