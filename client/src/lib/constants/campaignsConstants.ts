...
import type { CampaignStatus, CampaignType, RecipientStatus } from '@typedefs/campaigns';
...
export const CAMPAIGN_TYPE_META: Record<CampaignType, { label: string; color: string; icon: string }> = {
...
export const TEMPLATE_VARIABLES = [
  { token: '{{customer_name}}',  label: 'Customer full name',  example: 'Adaeze Obi'   },
  ...
  { token: '{{product_name}}',   label: 'Product name',        example: 'Gold Bangle Set' },
  { token: '{{delivery_date}}',  label: 'Delivery date',       example: '15 Jan 2026'  },
