...
export const TRANSACTION_TYPE_META: Record<TransactionType, {
  label: string; sign: '+' | '-' | '±'; color: string; bgColor: string;
}> = {
  earned:     { label: 'Earned',     sign: '+', color: '#2D9CDB', bgColor: '#2D9CDB15' },
  bonus:      { label: 'Bonus',      sign: '+', color: '#C9A86C', bgColor: '#C9A86C15' },
  adjustment: { label: 'Adjustment', sign: '±', color: '#9E9891', bgColor: '#9E989115' },
  redeemed:   { label: 'Redeemed',   sign: '-', color: '#F97316', bgColor: '#F9731615' },
  expired:    { label: 'Expired',    sign: '-', color: '#EF4444', bgColor: '#EF444415' },
};
...
export const createTierSchema = z.object({
  ...
  colour:        z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Must be a valid hex colour').default('#64748B'),
...
export const redeemPointsSchema = z.object({
  points:         z.number({ required_error: 'Points required' }).int().positive('Points must be positive'),
