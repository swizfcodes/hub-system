...
export function previewPayslip(params: {
  ...
  const { basicSalary, housingRatio = 0.20, transportRatio = 0.10, commissionAmount = 0 } = params;

  const housingAllowance   = +(basicSalary * housingRatio).toFixed(2);
  const transportAllowance = +(basicSalary * transportRatio).toFixed(2);
  const grossSalary        = +(basicSalary + housingAllowance + transportAllowance + commissionAmount).toFixed(2);
  ...
  const { totalCRA, taxableIncome, annualPAYE } = calcAnnualPAYE(taxableGross * 12);
  ...
  return {
    ...
    grossSalary, totalCRA: +(totalCRA / 12).toFixed(2),
    taxableIncome: +(taxableIncome / 12).toFixed(2),
    monthlyPAYE, pensionEmployee, nhf,
