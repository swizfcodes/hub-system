...
export const NOTIFICATION_TYPE_META: Record<string, { icon: string; color: string; label: string }> = {
...
export function getNotificationMeta(type: string) {
  return NOTIFICATION_TYPE_META[type] ?? { icon: '🔔', color: '#9E9891', label: 'Notification' };
}
...
export function getPeriodParams(period: string): { year: number; month?: number } {
  const now = new Date();
  switch (period) {
    case 'last_month': {
...
export function getKpiColor(value: number, threshold?: number, inverse = false): string {
