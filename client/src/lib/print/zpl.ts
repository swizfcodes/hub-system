...
function barcodeBlock(symbology: BarcodeSymbology, value: string, barHeight: number): string {
...
export function buildZpl(input: ZplLabelInput): string {
  ...
  let y = margin;
  if (input.name) {
    lines.push(`^FO${margin},${y}^A0N,${Math.round(heightDots * 0.13)},${Math.round(heightDots * 0.13)}^FB${widthDots - margin * 2},2,0,L^FD${sanitize(input.name)}^FS`);
    y += Math.round(heightDots * 0.28);
  }

  lines.push(`^FO${margin},${y}^BY2`);
  lines.push(`^FO${margin},${y}${barcodeBlock(input.symbology ?? 'CODE128', input.value, barHeight)}`);
  y += barHeight + (isQR ? 0 : Math.round(heightDots * 0.16));

  const footer = [input.sku, input.priceLine].filter(Boolean).join('   ');
  if (footer) {
    lines.push(`^FO${margin},${Math.min(y, heightDots - Math.round(heightDots * 0.14))}^A0N,${Math.round(heightDots * 0.1)},${Math.round(heightDots * 0.1)}^FD${sanitize(footer)}^FS`);
  }
