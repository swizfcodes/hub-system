 ...
 */
export function useStaffByContact(contactId: string | undefined): { staff: StaffProfile | null; isLoading: boolean } {
  const { data, isLoading } = useQuery({
    queryKey: ['staff', 'all-200'],
    queryFn: () => listStaff({ limit: 200 }),
  });
  const staff = contactId ? (data?.data ?? []).find((s) => s.contact_id === contactId) ?? null : null;
