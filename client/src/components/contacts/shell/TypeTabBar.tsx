...
export function TypeTabBar({ active, onChange, counts = {} }: Props) {
  const tabs: { key: TabKey; label: string; icon: React.ComponentType<{ className?: string }>; ring?: string }[] = [
  ...
  return (
    <div role="tablist" className="flex overflow-x-auto hide-scrollbar -mx-1 px-1">
            ...
            >
              ...
              {typeof c === 'number' && (
                <span className={cn(
                  ...
                  isActive ? 'bg-orika-black/10 text-orika-black' : 'bg-orika-graphite text-orika-cloud',
                )}>{c}</span>
              )}
            </button>
          );
