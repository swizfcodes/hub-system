...
export function ViewSwitcher({ value, onChange }: { value: DirectoryView; onChange: (v: DirectoryView) => void }) {
  return (
    <div className="inline-flex p-0.5 rounded-lg bg-orika-charcoal border border-orika-graphite">
      <button
        ...
        className={cn(
          ...
          value === 'rail' ? 'bg-orika-graphite text-orika-cream' : 'text-orika-smoke hover:text-orika-cream',
        )}
      ...
      <button
        ...
        className={cn(
          'inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[0.65rem] font-semibold uppercase tracking-wide transition-all',
          value === 'cards' ? 'bg-orika-graphite text-orika-cream' : 'text-orika-smoke hover:text-orika-cream',
        )}
