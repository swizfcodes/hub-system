...
export function DirectoryFilters({ value, onChange }: Props) {
  ...
  const hasAdvanced = !!(value.priority || value.source || value.showAllBusinesses);

  const update = (patch: Partial<DirectoryFilterValues>) => onChange({ ...value, ...patch });
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
        <div className="flex-1">
          <Input
            ...
            rightSlot={value.search ? (
              <button onClick={() => update({ search: '' })} className="text-orika-smoke hover:text-orika-cream" aria-label="Clear search">
        ...
        >
          Filters {hasAdvanced && <span className="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-orika-black/20 text-[0.55rem]">●</span>}
        </Button>
      </div>
      ...
      <div className={cn('overflow-hidden transition-all', expanded ? 'max-h-96' : 'max-h-0')}>
        <div className="pt-2 grid gap-3 sm:grid-cols-3 animate-slide-down">
          ...
          <Select
            ...
            label="Source"
            value={value.source || ''}
            onChange={(e) => update({ source: (e.target.value || '') as ContactSource | '' })}
            options={[
