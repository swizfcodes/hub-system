...
export function ContactCard({ contact, onClick, style }: Props) {
  return (
    <article
      onClick={onClick}
      style={style}
      className="group relative rounded-2xl border border-orika-graphite bg-orika-charcoal/70 hover:border-orika-gold/40 hover:shadow-card-lg hover:-translate-y-1 transition-all overflow-hidden cursor-pointer animate-tile-in"
    >
      <div className="p-5">
        ...
        <div className="mt-4 space-y-1.5 text-xs">
          <div className="flex items-center gap-2 text-orika-cloud">
            <Phone className="w-3 h-3 text-orika-smoke shrink-0" />
            <span className="truncate">{contact.primary_phone}</span>
          </div>
          {contact.email && (
            <div className="flex items-center gap-2 text-orika-cloud">
              <Mail className="w-3 h-3 text-orika-smoke shrink-0" />
