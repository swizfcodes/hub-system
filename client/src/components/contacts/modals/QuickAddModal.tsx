...
import { quickAddSchema, type QuickAddValues, CONTACT_TYPES, PRIORITY_LEVELS, CONTACT_SOURCES } from '@lib/schemas/contact';
...
export function QuickAddModal({ open, onClose, defaultType, onCreated }: Props) {
  ...
  const { register, handleSubmit, control, watch, reset, formState: { errors, isSubmitting } } = useForm<QuickAddValues>({
  ...
  const mutation = useMutation({
    mutationFn: (v: QuickAddValues) => createContact({
      ...v,
    ...
    onSuccess: (c) => {
      ...
      showToast.success('Contact added', `${c.display_name} is in the directory`);
      reset();
      onClose();
    ...
    >
      <form onSubmit={handleSubmit((v) => mutation.mutate(v))} noValidate className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {CONTACT_TYPES.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <span className="w-2 h-2 rounded-full" style={{ background: m.ringColor }} />
                      {m.label}
                    </button>
                  );
                })}
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          {selectedTypes.includes('customer') && (
            <Select
              ...
              options={CONTACT_SOURCES.map((s) => ({ value: s, label: s.replace('_', ' ') }))}
            />
          )}
        </div>
        ...
        <div>
          ...
          {(visibleTo || []).length > 1 && (
            <p className="mt-2 text-[0.65rem] text-text-on-light-muted ml-1">This contact will appear under each selected business.</p>
          )}
        </div>
      </form>
    </Modal>
