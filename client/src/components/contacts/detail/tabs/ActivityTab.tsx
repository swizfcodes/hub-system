...
import { MessageSquare, Phone, Calendar, CheckSquare, Receipt, TrendingUp, History, ArrowDownRight, ArrowUpRight } from 'lucide-react';
...
type ActivityKind = 'call' | 'sms' | 'email' | 'meeting' | 'task' | 'deal' | 'invoice' | 'system';
...
const KIND_META: Record<ActivityKind, { icon: React.ComponentType<{ className?: string }>; color: string; label: string }> = {
...
export function ActivityTab({ contactId }: Props) {
  ...
  const { data: timeline, isLoading: tlLoading } = useQuery({ queryKey: ['contacts', contactId, 'timeline'], queryFn: () => getTimeline(contactId) });
  const { data: tasks }    = useQuery({ queryKey: ['contacts', contactId, 'tasks'], queryFn: () => listTasks({ reference_type: 'contact', reference_id: contactId, limit: 100 }) });
  const { data: events }   = useQuery({ queryKey: ['contacts', contactId, 'events'], queryFn: () => listEventsForReference('contact', contactId) });
  const { data: audit }    = useQuery({ queryKey: ['contacts', contactId, 'audit', 'shared.contacts'], queryFn: () => getRecordAudit('shared.contacts', contactId, 50) });

  const merged: TimelineEntry[] = useMemo(() => {
    const out: TimelineEntry[] = [];

    for (const a of (timeline?.activities ?? [])) {
      const kind: ActivityKind = ['call','sms','email','meeting'].includes(a.activity_type) ? (a.activity_type as ActivityKind) : 'system';
      out.push({
        id: `a-${a.activity_id}`,
        kind,
        title: a.summary,
  ...
  return (
    <div className="space-y-5">
      ...
      {tlLoading ? (
        <div className="space-y-3">{[0,1,2,3].map((i) => <Skeleton key={i} className="h-16" />)}</div>
      ) : merged.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-orika-graphite bg-orika-charcoal/40 p-10 text-center">
          ...
          <p className="text-sm text-orika-smoke">No activity to show with this filter.</p>
        </div>
      ) : (
                ...
                <div className="rounded-xl border border-orika-graphite bg-orika-charcoal/50 p-3.5">
                  <div className="flex items-start justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2 min-w-0">
                      {e.direction === 'in'  && <ArrowDownRight className="w-3 h-3 text-living-sage shrink-0" />}
                      {e.direction === 'out' && <ArrowUpRight   className="w-3 h-3 text-orika-gold shrink-0" />}
                      <span className="text-sm text-orika-cream truncate">{e.title}</span>
                    </div>
                    <Badge tone="neutral" size="xs">{meta.label}</Badge>
                  </div>
                  {e.detail && <div className="text-xs text-orika-cloud mt-1">{e.detail}</div>}
                  {e.amount && (e.kind === 'invoice' || e.kind === 'deal') && (
                    <div className="text-xs font-mono text-orika-gold mt-1">{fmtMoney(e.amount, 'NGN')}</div>
                  )}
                  <div className="text-[0.65rem] text-orika-smoke mt-1.5">{fmtDateTime(e.at)} · {fmtRelative(e.at)}</div>
                </div>
              </li>
            );
          })}
