...
export function NotesTab({ contact }: { contact: Contact }) {
  ...
  const mutation = useMutation({
    mutationFn: () => updateContact(contact.contact_id, { notes: value || undefined }),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4 max-w-3xl">
      ...
      <div className="flex justify-end">
        <Button variant="gold" leftIcon={<Save className="w-4 h-4" />} disabled={!dirty} loading={mutation.isPending} onClick={() => mutation.mutate()}>
          Save notes
        </Button>
      </div>
    </div>
