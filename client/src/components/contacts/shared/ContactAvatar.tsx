...
const SIZES: Record<NonNullable<Props['size']>, { box: string; text: string; ring: string }> = {
...
export function ContactAvatar({ contact, size = 'sm', emphasiseType, className }: Props) {
  ...
  return (
    ...
    >
      ...
      {contact.contact_type && contact.contact_type.length > 1 && (
        <span
          className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-orika-charcoal"
          style={{ background: CONTACT_TYPE_META[contact.contact_type[1]].ringColor }}
          title={`Also: ${contact.contact_type.slice(1).map((t) => CONTACT_TYPE_META[t].label).join(', ')}`}
        />
      )}
    </div>
  );
