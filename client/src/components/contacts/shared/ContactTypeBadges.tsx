...
export function ContactTypeBadges({ contact, max = 3 }: { contact: Pick<Contact, 'contact_type' | 'priority_level'>; max?: number }) {
  ...
  return (
    <div className="inline-flex items-center gap-1.5 flex-wrap">
      {types.map((t) => {
        ...
        return (
          <Badge key={t} tone={m.tone} size="xs" dot>{m.label}</Badge>
        );
      })}
      {contact.priority_level && contact.priority_level !== 'regular' && (
        <Badge tone={priority.tone} size="xs">{priority.label}</Badge>
      )}
    </div>
  );
}
