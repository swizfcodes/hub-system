...
interface Props {
  contact: Pick<Contact, 'primary_phone' | 'whatsapp_number' | 'email' | 'addresses'>;
  size?: 'sm' | 'md';
  surface?: 'dark' | 'light';
  className?: string;
}
...
function defaultAddressUrl(addresses?: ContactAddress[] | null): string | null {
  ...
  const q = encodeURIComponent(`${def.line1}, ${def.city}, ${def.state}, ${def.country}`);
  return `https://www.google.com/maps/search/?api=1&query=${q}`;
}

export function QuickActions({ contact, size = 'sm', surface = 'dark', className }: Props) {
  ...
  return (
    <div className={cn('inline-flex items-center gap-1.5', className)} onClick={stop}>
      {phone && (
        <a href={`tel:${phone}`} aria-label="Call" className={baseCls} title={contact.primary_phone || ''}>
          <Phone className={iconSizeMap[size]} />
        </a>
      )}
      {whatsapp && (
        <a href={`https://wa.me/${whatsapp}`} target="_blank" rel="noopener" aria-label="WhatsApp" className={baseCls} title="WhatsApp">
          <MessageCircle className={iconSizeMap[size]} />
        </a>
      )}
      {email && (
        <a href={`mailto:${email}`} aria-label="Email" className={baseCls} title={email}>
          <Mail className={iconSizeMap[size]} />
        </a>
      )}
      {map && (
        <a href={map} target="_blank" rel="noopener" aria-label="Open in Maps" className={baseCls} title="Open in Maps">
