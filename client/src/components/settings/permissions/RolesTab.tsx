    ...
    <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
      ...
      <aside>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-[0.65rem] tracking-[0.18em] uppercase text-orika-gold">
            Roles
          </h3>
          <Button
            variant="ghost"
        ...
        {rolesLoading ? (
          ...
          <EmptyState
            ...
            title="No roles"
            description="The seed migration should have created system roles."
          />
        ) : (
            ...
            {roles.map((r) => (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-medium text-sm text-orika-cream capitalize truncate">
                    {r.role_name}
                  </span>
                  {r.is_system && (
                ...
                <div className="text-[0.65rem] text-orika-smoke mt-0.5 truncate">
                  {r.business ?? "Global"}
                </div>
              </button>
            ))}
            ...
            <header className="mb-5 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
              <div>
                <div className="flex items-center gap-2 flex-wrap">
                  <h2 className="font-display text-3xl text-orika-cream capitalize">
                    {role.role_name}
                  </h2>
                  {role.is_system && (
                ...
                {role.description && (
                  <p className="text-sm text-orika-cloud mt-1.5">
                    {role.description}
                  </p>
                )}
      ...
      <ConfirmationModal
        ...
        onConfirm={() => {
          deleting && del.mutateAsync(deleting.role_id);
        }}
        title={`Delete role “${deleting?.role_name}”?`}
        message={
          <div className="space-y-2">
            <p>
              This will remove the role and every permission attached to it.
            </p>
            <p className="text-text-on-light-muted">
              The backend refuses this if any user is currently assigned to it —
              reassign them first.
            </p>
          </div>
        }
...
function CreateRoleModal({
  ...
  const mutation = useMutation({
    mutationFn: (v: RoleCreateValues) =>
      createRole({
        ...v,
        business: v.business || null,
        clone_from_role_id: v.clone_from_role_id || undefined,
      }),
    onSuccess: (r) => {
      showToast.success(`Role “${r.role_name}” created`);
      reset();
      onCreated(r);
    },
    onError: (e) => showToast.error("Failed", errMsg(e)),
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Select
          {...register("business")}
          label="Scoped to business (optional)"
          placeholder="Global — usable everywhere"
