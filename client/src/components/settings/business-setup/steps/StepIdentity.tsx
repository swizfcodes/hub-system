...
export function StepIdentity({ register, errors }: Props) {
  return (
    <div className="space-y-6">
      <header>
        <h2 className="font-display font-light text-3xl text-orika-black">Identity</h2>
        <p className="text-sm text-text-on-light-muted mt-1.5">Tell us about this business. The <em>key</em> is permanent — pick carefully.</p>
      </header>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Textarea
          ...
          className="sm:col-span-2"
        />
        <Input {...register('tin')}        label="TIN (Tax ID)"   placeholder="123456789-0001" error={errors.tin?.message as string | undefined} />
        <Input {...register('cac_number')} label="CAC number"     placeholder="RC 1234567"     error={errors.cac_number?.message as string | undefined} />
      </div>
    </div>
  );
}
