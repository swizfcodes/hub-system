...
export function StepProvisioning({ control, register, errors, provisionSchema }: Props) {
  return (
    <div className="space-y-6">
      <header>
        <h2 className="font-display font-light text-3xl text-orika-black">Advanced — Database Provisioning</h2>
        <p className="text-sm text-text-on-light-muted mt-1.5">
          Choose how this business's Postgres schema gets created.
          Provisioning runs migrations and seeds default sequences — it can't be partially undone.
        </p>
      </header>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-orika-black/80">
              ...
              <ul className="mt-2 list-disc pl-5 space-y-1 text-text-on-light-muted">
                ...
                <li>Every table in the standard business template (sales, stock, accounting, payroll, logistics, etc.)</li>
                <li>A complete set of document numbering sequences (invoice, PO, quotation, payslip, etc.)</li>
...
function ProvOption({
  selected, onClick, icon, title, tagline, body, accent,
}: {
  selected: boolean;
  onClick: () => void;
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected
          ? (accent ? 'bg-orika-gold/[0.08] border-orika-gold shadow-glow-sm' : 'bg-orika-black/[0.04] border-orika-black')
          : 'bg-white/40 border-orika-cloud/40 hover:border-orika-black/40',
      )}
    >
      <div className="flex items-start gap-4">
        <div className={cn('w-11 h-11 rounded-xl flex items-center justify-center shrink-0', accent ? 'bg-orika-gold/20 text-orika-gold-dim' : 'bg-orika-cream text-orika-black')}>
          {icon}
        </div>
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="font-display text-xl text-orika-black">{title}</span>
            {accent && <span className="text-[0.6rem] uppercase tracking-widest px-2 py-0.5 bg-orika-gold/20 text-orika-gold-dim rounded-full font-bold">Recommended</span>}
          </div>
          <div className="text-xs text-text-on-light-muted mt-0.5 italic">{tagline}</div>
          <p className="text-sm text-orika-black/70 mt-2 leading-relaxed">{body}</p>
        </div>
        <div className={cn(
