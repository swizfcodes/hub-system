...
export function BrandingTab({ business }: { business: Business }) {
  ...
  const { register, control, handleSubmit, formState: { errors, isDirty } } = useForm<BrandingPatchValues>({
  ...
  return (
    <form onSubmit={handleSubmit((v) => mutation.mutate(v))} noValidate className="space-y-8">
      {/* ── Logo & Colours ───────────────────────────────── */}
      <section>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <BrandColorPicker value={field.value || '#C9A86C'} onChange={field.onChange} label="Primary colour" />
              )}
            />
            <Controller
              ...
              control={control}
              render={({ field }) => (
                <BrandColorPicker value={field.value || '#F5F0EB'} onChange={field.onChange} label="Secondary colour" />
              )}
            />
          </div>
        </div>
      ...
      {/* ── Typography ───────────────────────────────────── */}
      <section>
        <SectionHeader title="Typography" hint="Font names used in emails and documents. Use web-safe fonts or Google Fonts names." />
      ...
      {/* ── Social Links ─────────────────────────────────── */}
      <section>
        <SectionHeader title="Social links" hint="Only filled links appear as icons in your transactional emails. Leave blank to hide." />
        <div className="grid gap-4 sm:grid-cols-2">
          {SOCIAL_PLATFORMS.map((p) => (
            <Input
              ...
              error={(errors.social_links as Record<string, { message?: string }> | undefined)?.[p.key]?.message}
            />
          ))}
        </div>
      </section>
      ...
      <div className="flex justify-end">
        <Button type="submit" variant="primary" disabled={!isDirty} loading={mutation.isPending}>Save branding</Button>
      </div>
    </form>
  );
}
