...
import { financialPatchSchema, type FinancialPatchValues } from '@lib/schemas/business';
...
export function FinancialTab({ business }: { business: Business }) {
  ...
  const { register, handleSubmit, formState: { errors, isDirty } } = useForm<FinancialPatchValues>({
  ...
  const mutation = useMutation({
    mutationFn: (values: FinancialPatchValues) => updateBusiness(business.business_key, {
      ...values,
    ...
    <form onSubmit={handleSubmit((v) => mutation.mutate(v))} noValidate className="space-y-6">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Input
          ...
          label="VAT rate" hint="Decimal 0–1 (0.075 = 7.5%)"
          error={errors.vat_rate?.message}
        />
        <Input
          {...register('wht_rate', { valueAsNumber: true })}
          type="number" step="0.001"
          label="WHT rate" hint="Decimal 0–1 (0.05 = 5%)"
          error={errors.wht_rate?.message}
        />
        <Input {...register('vat_number')} label="VAT number" className="sm:col-span-2" />
      </div>
      <div className="flex justify-end">
        <Button type="submit" variant="primary" disabled={!isDirty} loading={mutation.isPending}>Save financial</Button>
      </div>
    </form>
  );
}
