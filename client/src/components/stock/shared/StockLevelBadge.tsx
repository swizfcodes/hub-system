 ...
 */
export function StockLevelBadge({ onHand, reorderLevel = 0, available, compact, className }: Props) {
  const out = onHand <= 0;
  ...
  const label = out ? 'Out of stock' : low ? `Low (${onHand})` : `${onHand} on hand`;
  ...
  return (
    <span className={cn(
      ...
      <span className={cn('w-1.5 h-1.5 rounded-full',
        tone === 'danger'  ? 'bg-state-danger animate-pulse' :
        tone === 'warn'    ? 'bg-state-warn' :
        tone === 'sage'    ? 'bg-living-sage' : 'bg-orika-cloud',
      )} />
      {label}
