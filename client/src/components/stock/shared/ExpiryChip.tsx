...
export function ExpiryChip({ expiry, className }: { expiry?: string | null; className?: string }) {
  if (!expiry) return null;
  const status = batchExpiryStatus(expiry);
  const days = Math.round((new Date(expiry).getTime() - Date.now()) / 86400000);

  const tone = status === 'expired' ? 'danger' : status === 'critical' ? 'warn' : status === 'soon' ? 'info' : 'sage';
  const cls = {
  ...
  return (
    <span className={cn('inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[0.6rem] font-medium', cls, className)}>
      {status === 'expired' || status === 'critical' ? <AlertTriangle className="w-2.5 h-2.5" /> : <Calendar className="w-2.5 h-2.5" />}
      {label}
    </span>
  );
}
