...
export function ByLocationView({ rows, locations, loading }: Props) {
  ...
  if (locations.length === 0) {
    return <EmptyState icon={<MapPin className="w-6 h-6" />} title="No locations defined" description="Add warehouses, showrooms, or POS terminals in Catalogue → Locations first." />;
  }
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="font-medium text-sm text-orika-cream truncate">{loc.name}</span>
              </div>
              <div className="text-[0.6rem] uppercase tracking-widest text-orika-smoke">{loc.location_type.replace('_', ' ')}</div>
              <div className="text-[0.65rem] text-orika-cloud mt-1">{items.length} SKU · {totalUnits} units</div>
            </button>
          );
        })}
      </aside>
      ...
      <section>
        {!activeLocation ? (
          <Card className="p-12 text-center">
            ...
            <p className="text-sm text-orika-smoke">Pick a location to see what's there.</p>
          </Card>
        ) : (
          <div className="space-y-2">
            {(byLocation[activeLocation] ?? []).map(({ row, qty }) => (
              <Card key={row.product_id} className="p-3 flex items-center gap-3">
                <ProductImage product={{ name: row.product_name, primary_image_url: row.primary_image_url }} size="sm" />
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-orika-cream truncate">{row.product_name}</div>
                  <div className="text-[0.6rem] font-mono text-orika-smoke">{row.product_sku}</div>
                </div>
                <StockLevelBadge onHand={qty} reorderLevel={row.reorder_level} compact />
              </Card>
            ))}
            {(byLocation[activeLocation] ?? []).length === 0 && (
              <EmptyState icon={<Package className="w-6 h-6" />} title="No stock at this location" description="When stock arrives or transfers in, products appear here." />
            )}
          </div>
        )}
      </section>
