...
import { AlertTriangle, TrendingUp, Clock, RefreshCcw, Banknote } from 'lucide-react';
...
import {
  EXPENSE_STATUS_META, ADVANCE_STATUS_META,
  CATEGORY_OPTIONS } from '@lib/constants/expensesConstants';
import {
  rejectExpenseSchema, type RejectExpenseValues,
  createAdvanceSchema, type CreateAdvanceValues,
  approveAdvanceSchema, type ApproveAdvanceValues,
} from '@lib/schemas/expenses';
import {
  rejectExpense, getExpenseKpis,
  createAdvance, approveAdvance } from '@services/expenses';
import { fmtMoney } from '@lib/format';
import { showToast } from '@hooks/useToast';
import { errMsg } from '@services/api';
import type { ExpenseStatus, AdvanceStatus, ExpenseKpis } from '@typedefs/expenses';

// ── Badges ────────────────────────────────────────────────────────────────────

export function ExpenseStatusBadge({ status, size = 'sm' }: { status: ExpenseStatus; size?: 'xs' | 'sm' }) {
  const meta = EXPENSE_STATUS_META[status];
  return <Badge tone={meta.tone} size={size} dot={meta.dot}>{meta.label}</Badge>;
}

export function AdvanceStatusBadge({ status, size = 'sm' }: { status: AdvanceStatus; size?: 'xs' | 'sm' }) {
  ...
  const cards = [
    ...
    },
    {
      ...
      value: kpis?.top_category_this_month
        ? CATEGORY_OPTIONS.find((c) => c.value === kpis.top_category_this_month)?.label ?? kpis.top_category_this_month
        : '—',
      icon:  Banknote,
      color: '#9E9891',
    },
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
      {cards.map((card) => {
        ...
        return (
          <div key={card.label} className="rounded-2xl border border-white/5 bg-orika-charcoal px-5 py-4">
            <div className="flex items-center gap-2 mb-1.5">
              <Icon className="h-3.5 w-3.5" style={{ color: card.color }} />
              <p className="text-[0.65rem] uppercase tracking-widest text-orika-smoke">{card.label}</p>
            </div>
            <p className="font-display text-xl font-light tabular-nums" style={{ color: card.color }}>
              {card.value}
            </p>
            {card.sub && <p className="text-xs text-orika-smoke mt-0.5 tabular-nums">{card.sub}</p>}
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-5">
      ...
      <div className="space-y-3">
        {categories.map((cat) => {
          const label = CATEGORY_OPTIONS.find((c) => c.value === cat.category)?.label ?? cat.category;
          const pct   = Math.round((cat.total / max) * 100);
          return (
            <div key={cat.category} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="text-orika-cloud">{label}</span>
                <span className="tabular-nums text-orika-smoke">{fmtMoney(cat.total, currency)}</span>
              </div>
...
export function RejectModal({ open, onClose, expenseId, expenseNum }: RejectModalProps) {
  ...
  const mutation = useMutation({
    mutationFn: (values: RejectExpenseValues) => rejectExpense(expenseId, values),
    onSuccess: () => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Reject Expense" size="sm" surface="light"
      footer={
      ...
      <div className="space-y-3">
        ...
        <Controller name="rejection_reason" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Reason *" placeholder="e.g. Receipt missing, category incorrect, not a business expense"
...
export function AdvanceFormModal({ open, onClose, currency: _currency = 'NGN' }: AdvanceFormModalProps) {
  const qc = useQueryClient();
  ...
  return (
    <Modal open={open} onClose={onClose} title="Request Cash Advance" size="sm" surface="light"
      footer={
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={onClose} disabled={mutation.isPending}>Cancel</Button>
      ...
      <div className="space-y-4">
        <p className="text-xs text-text-on-light-muted">
          Your manager will review and approve the advance amount. Once disbursed,
          submit expense claims against it.
        </p>
        <Controller name="purpose" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Purpose *" placeholder="e.g. Client site visit — Abuja" surface="light"
            error={fieldState.error?.message} />
        )} />
        <Controller name="amount_requested" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Amount Requested (₦) *" type="number" step="500" min={0}
        ...
        <Controller name="reason" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Justification *" placeholder="Why is this advance needed?" surface="light"
            error={fieldState.error?.message} />
        )} />
      </div>
    </Modal>
...
export function ApproveAdvanceModal({
  open, onClose, advanceId, requested, currency = 'NGN',
  ...
  const mutation = useMutation({
    mutationFn: (values: ApproveAdvanceValues) => approveAdvance(advanceId, values),
    onSuccess: () => {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Approve Cash Advance" size="sm" surface="light"
      footer={
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={onClose} disabled={mutation.isPending}>Cancel</Button>
          <Button onClick={form.handleSubmit((v) => mutation.mutate(v))} loading={mutation.isPending}>
      ...
      <div className="space-y-4">
        <div className="rounded-xl bg-orika-cloud/20 px-4 py-3 text-sm flex justify-between">
          ...
          <span className="font-semibold text-orika-black">{fmtMoney(requested, currency)}</span>
        </div>
        <Controller name="amount_approved" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Amount to Approve (₦) *" type="number" step="500" min={0}
            surface="light"
