...
import { createExpenseSchema, type CreateExpenseValues } from '@lib/schemas/expenses';
import {
  CATEGORY_OPTIONS, EXPENSE_TYPE_OPTIONS,
...
export function ExpenseFormModal({
  open, onClose, onCreated, linkedAdvanceId: _linkedAdvanceId,
  ...
  const isAutoApprove  = watchedAmount > 0 && watchedAmount <= AUTO_APPROVE_THRESHOLD_NGN;
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          {watchedCat && watchedType && watchedAmount > 0 && (
            <p className="text-xs text-text-on-light-muted font-mono">
              DR {CATEGORY_COA[watchedCat]} / CR {EXPENSE_TYPE_CR_ACCOUNT[watchedType]}
            </p>
          )}
    ...
    >
      <div className="space-y-5">
        ...
        {isAutoApprove && (
          <div className="flex items-start gap-2 rounded-xl border border-green-500/30 bg-green-900/10 px-4 py-3">
            ...
            <p className="text-xs text-green-400">
              Amounts up to {fmtMoney(AUTO_APPROVE_THRESHOLD_NGN, currency)} are auto-approved.
              This claim will be approved immediately on submission.
            </p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <Controller name="expense_type" control={form.control} render={({ field }) => (
            <Select label="Payment Method *" options={EXPENSE_TYPE_OPTIONS}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Controller name="amount" control={form.control} render={({ field, fieldState }) => (
            <Input {...field} label="Amount (₦) *" type="number" step="0.01" min={0}
              surface="light"
              onChange={(e) => field.onChange(parseFloat(e.target.value) || 0)}
              error={fieldState.error?.message} />
          )} />

          <Controller name="expense_date" control={form.control} render={({ field, fieldState }) => (
        ...
        <Controller name="description" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Description *" placeholder="What was this expense for?"
            surface="light" error={fieldState.error?.message} />
        )} />
        ...
        <div className="space-y-3">
          ...
          />
          <Controller name="vendor_name" control={form.control} render={({ field }) => (
            <Input {...field} label="Or free-text vendor name"
        ...
        <div className={cn(
          ...
          {watchedAmount > 10_000 ? (
            <span className="font-medium">Receipt required for amounts above {fmtMoney(10_000, currency)}.</span>
          ) : (
