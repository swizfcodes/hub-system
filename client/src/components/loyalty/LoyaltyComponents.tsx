...
import { createTier, updateTier, redeemPoints, manualAward } from '@services/loyalty';
import {
  TRANSACTION_TYPE_META, DEFAULT_TIER_COLOURS,
  createTierSchema, awardPointsSchema, redeemPointsSchema,
  type CreateTierValues, type AwardPointsValues, type RedeemPointsValues,
...
export function TierBadge({
  tier, size = 'sm',
}: { tier: LoyaltyTier | null; size?: 'xs' | 'sm' | 'md' }) {
  if (!tier) {
    return (
      <span className={cn(
        'inline-flex items-center gap-1 rounded-full border border-white/10 font-medium text-orika-smoke',
        size === 'xs' ? 'px-1.5 py-0.5 text-[0.55rem]' : size === 'md' ? 'px-3 py-1 text-xs' : 'px-2 py-0.5 text-[0.65rem]',
      )}>
        No Tier
      </span>
    );
  ...
  return (
    <span
      className={cn(
        ...
        size === 'xs' ? 'px-2 py-0.5 text-[0.55rem]' : size === 'md' ? 'px-3 py-1 text-sm' : 'px-2.5 py-0.5 text-xs',
      )}
      style={{
        color:           tier.colour,
...
export function PointsCard({
  balance, tier, onRedeem, onAward, canApprove = false,
}: {
  balance: number; tier: LoyaltyTier | null;
  onRedeem: () => void; onAward: () => void; canApprove?: boolean;
}) {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          <p className="text-[0.65rem] uppercase tracking-widest text-orika-smoke mb-1">Points Balance</p>
          <p className="font-display text-4xl font-light tabular-nums" style={{ color: accentColor }}>
            {balance.toLocaleString()}
          </p>
          <p className="text-[0.65rem] text-orika-smoke mt-0.5">loyalty points</p>
        </div>
      ...
      {tier && (
        <div className="space-y-1">
          <div className="flex items-center justify-between text-xs text-orika-smoke">
            ...
            <span>{tier.max_points ? `${tier.max_points.toLocaleString()} pts` : 'No limit'}</span>
          </div>
...
export function TransactionList({ transactions }: { transactions: LoyaltyTransaction[] }) {
          ...
          <div key={tx.transaction_id}
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs text-orika-cream truncate">
                {tx.notes ?? (tx.reference_type ? `Ref: ${tx.reference_type}` : '—')}
              </p>
              <p className="text-[10px] text-orika-smoke/60">{fmtDate(tx.created_at)}</p>
            </div>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <Controller name="tier_name" control={form.control} render={({ field, fieldState }) => (
          <Input {...field} label="Tier Name *" placeholder="e.g. Silver, Gold, Platinum"
            surface="light" error={fieldState.error?.message} />
        )} />

        <div className="grid grid-cols-2 gap-3">
          <Controller name="min_points" control={form.control} render={({ field, fieldState }) => (
            <Input {...field}
              label="Min Points *"
              type="number" min={0} surface="light"
              value={field.value}
              onChange={(e) => field.onChange(parseInt(e.target.value) || 0)}
              error={fieldState.error?.message} />
          )} />
          <Controller name="max_points" control={form.control} render={({ field, fieldState }) => (
            <Input
              label="Max Points"
              type="number" min={0} surface="light"
              value={field.value ?? ''}
        ...
        <div className="space-y-2">
          <p className="text-[0.7rem] font-medium uppercase tracking-widest text-text-on-light-muted">Tier Colour</p>
            ...
            {DEFAULT_TIER_COLOURS.map((c) => (
              <button key={c} type="button"
                ...
                className={cn(
                  ...
                  watchedColour === c ? 'border-gray-800 scale-110' : 'border-transparent',
                )}
                style={{ backgroundColor: c }}
              />
            ))}
            <input type="color" {...form.register('colour')}
              className="h-7 w-7 rounded-full border-2 border-gray-200 cursor-pointer overflow-hidden"
              title="Custom colour" />
          </div>
          {form.formState.errors.colour && (
            <p className="text-xs text-state-danger">{form.formState.errors.colour.message}</p>
          )}
        </div>

        <Controller name="display_order" control={form.control} render={({ field }) => (
          <Input label="Display Order" type="number" min={0} surface="light"
...
export function RedeemModal({ open, onClose, contactId, balance }: RedeemModalProps) {
  const qc   = useQueryClient();
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      showToast.success(`Redeemed! New balance: ${result.balance_after.toLocaleString()} pts`);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Redeem Points" size="sm" surface="light"
      footer={
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={onClose} disabled={mutation.isPending}>Cancel</Button>
          <Button variant="gold" onClick={form.handleSubmit((v) => mutation.mutate(v))}
    ...
    >
      <div className="space-y-4">
        <div className="rounded-xl border border-orika-gold/20 bg-orika-gold/5 px-4 py-3 text-sm">
          ...
          <span className="font-semibold text-orika-gold">{balance.toLocaleString()} pts</span>
        </div>
        <Controller name="points" control={form.control} render={({ field, fieldState }) => (
          <Input label="Points to Redeem *" type="number" min={1} max={balance}
            surface="light"
            value={field.value}
            onChange={(e) => field.onChange(parseInt(e.target.value) || 0)}
            error={fieldState.error?.message}
...
export function AwardModal({ open, onClose, contactId }: AwardModalProps) {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (result) => {
      showToast.success(`Done. New balance: ${result.balance_after.toLocaleString()} pts`);
      qc.invalidateQueries({ queryKey: ['contact-loyalty', contactId] });
      form.reset();
      onClose();
    },
  ...
  return (
    <Modal open={open} onClose={onClose} title="Award / Adjust Points" size="sm" surface="light"
      footer={
        <div className="flex justify-end gap-3">
          <Button variant="ghost" onClick={onClose} disabled={mutation.isPending}>Cancel</Button>
          <Button onClick={form.handleSubmit((v) => mutation.mutate(v))} loading={mutation.isPending}>
    ...
    >
      <div className="space-y-4">
        <Controller name="transaction_type" control={form.control} render={({ field }) => (
          <Select label="Type" surface="light"
            value={field.value}
            onChange={(e) => field.onChange(e.target.value)}
            options={[
              { value: 'bonus',      label: 'Bonus — add points (promotion, reward)' },
              { value: 'adjustment', label: 'Adjustment — correct an error (can be negative)' },
            ]} />
        )} />
        <Controller name="points" control={form.control} render={({ field, fieldState }) => (
          <Input label="Points *" type="number" surface="light"
        ...
        <Controller name="notes" control={form.control} render={({ field }) => (
          <Input {...field} label="Notes" placeholder="Reason for this award or adjustment"
            surface="light" />
        )} />
      </div>
    </Modal>
