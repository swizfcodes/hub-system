...
import { useForm as useFormMF, Controller as ControllerMF } from 'react-hook-form';
import { zodResolver as zodMF } from '@hookform/resolvers/zod';
import { useMutation as useMutMF, useQueryClient as useQCMF } from '@tanstack/react-query';
...
import { markFailedSchema, markReturnedSchema, type MarkFailedValues, type MarkReturnedValues } from '@lib/schemas/logistics';
...
export function MarkFailedModal({ open, onClose, deliveryId, mode }: MarkFailedModalProps) {
  const qc = useQCMF();
      ...
      <ModalMF
        ...
        footer={
          <div className="flex justify-end gap-3">
            <ButtonMF variant="ghost" onClick={onClose} disabled={returnMutation.isPending}>Cancel</ButtonMF>
            <ButtonMF
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-text-on-light-muted">
            Marking as returned will automatically restock all items in this delivery.
          </p>
          <ControllerMF
            name="notes"
  ...
  return (
    <ModalMF
      ...
      footer={
        <div className="flex justify-end gap-3">
          <ButtonMF variant="ghost" onClick={onClose} disabled={failMutation.isPending}>Cancel</ButtonMF>
          <ButtonMF
            variant="danger"
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-text-on-light-muted">
          The logistics manager will be notified. When the package is returned to the store,
          use "Mark Returned" to restock automatically.
        </p>
        <ControllerMF
          name="failure_reason"
