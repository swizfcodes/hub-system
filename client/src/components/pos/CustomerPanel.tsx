...
export function CustomerPanel({ currency = 'NGN' }: CustomerPanelProps) {
  const { customer, loyaltyInfo, setCustomer, setLoyaltyInfo } = usePOSStore((s) => ({
    customer:       s.customer,
  ...
  useEffect(() => {
    if (!customer) { setLoyaltyInfo(null); return; }
    ...
    <div className="flex flex-col gap-3">
      ...
      {customer && loyaltyInfo && (
        <div
          ...
          style={{
            borderColor: loyaltyInfo.tier ? `${loyaltyInfo.tier.colour}40` : 'rgba(255,255,255,0.05)',
            backgroundColor: loyaltyInfo.tier ? `${loyaltyInfo.tier.colour}08` : 'transparent',
          }}
        >
          <Star
          ...
          <div className="min-w-0">
            <p className="text-xs font-medium" style={{ color: loyaltyInfo.tier?.colour ?? '#9E9891' }}>
