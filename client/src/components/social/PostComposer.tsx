...
import {
  Image as Video, Hash, BookOpen,
  Plus, X, Clock, Send, FileText } from 'lucide-react';
import { Modal }    from '@components/ui/Modal';
import { Button }   from '@components/ui/Button';
import { Input }    from '@components/ui/Input';
import { ChannelSelector } from '@components/social/SocialComponents';
import { createPost, updatePost, publishNow, listTemplates, listHashtagSets } from '@services/social';
import { createPostSchema, type CreatePostValues, CHANNEL_META } from '@lib/constants/socialConstants';
...
export function PostComposer({ open, onClose, existing, defaultDate }: PostComposerProps) {
  ...
  const needsImage  = channels.some((c) => c === 'instagram' || c === 'facebook');
  ...
  const { data: templates   = [] } = useQuery({ queryKey: ['social-templates'],   queryFn: listTemplates   });
  const { data: hashtagSets = [] } = useQuery({ queryKey: ['social-hashtag-sets'], queryFn: listHashtagSets });

  const mutation = useMutation({
    mutationFn: async (values: CreatePostValues) => {
      const payload = {
        ...values,
        status:       (mode === 'draft' ? 'draft' : 'scheduled') as 'draft' | 'scheduled',
        scheduled_at: mode === 'draft' ? undefined
          : mode === 'now' ? new Date().toISOString()
  ...
  function removeMedia(idx: number) {
    const current = form.getValues('media_paths') ?? [];
    form.setValue('media_paths', current.filter((_, i) => i !== idx));
  }
  ...
  return (
    ...
    >
      <div className="space-y-5">
        {/* Channel selector */}
        <Controller name="channels" control={form.control} render={({ field, fieldState }) => (
          <div>
            <ChannelSelector
            ...
            />
            {fieldState.error && (
              <p className="mt-1 text-xs text-state-danger">{fieldState.error.message}</p>
            )}
          </div>
        )} />
        ...
        {mode === 'schedule' && (
          <Controller name="scheduled_at" control={form.control} render={({ field, fieldState }) => (
            <Input
              {...field}
        ...
        {needsImage && (
          <div className="space-y-2">
            <p className="text-[0.7rem] font-medium uppercase tracking-widest text-text-on-light-muted">
              Images {channels.includes('instagram') && <span className="normal-case font-normal">(up to 10 for carousel)</span>}
            </p>
            {mediaPaths.length > 0 && (
            ...
            <div className="flex gap-2">
              <input type="url" value={mediaUrlInput}
              ...
              />
              <Button size="sm" type="button" variant="secondary" onClick={addMediaUrl}
        ...
        {/* Video path — TikTok */}
        {needsVideo && (
          <Controller name="video_path" control={form.control} render={({ field }) => (
            <Input {...field} label="Video Path (TikTok)" surface="light"
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className={cn('text-[10px] tabular-nums', caption.length > charLimit * 0.9 ? 'text-red-500' : 'text-gray-400')}>
            ...
            {templates.length > 0 && (
              <div className="relative group">
                ...
                <div className="absolute left-0 top-full mt-1 z-20 hidden group-hover:block w-56 rounded-xl border border-gray-200 bg-white shadow-lg">
                  {templates.map((t) => (
                    <button key={t.template_id} type="button"
            ...
            {hashtagSets.length > 0 && (
              <div className="relative group">
                ...
                <div className="absolute left-0 top-full mt-1 z-20 hidden group-hover:block w-64 rounded-xl border border-gray-200 bg-white shadow-lg">
                  {hashtagSets.map((s) => (
                    <button key={s.set_id} type="button"
                      ...
                      <span className="text-[10px] text-gray-400 truncate">
                        ...
                        {s.hashtags.length > 4 && ` +${s.hashtags.length - 4} more`}
                      </span>
                    </button>
                  ))}
                </div>
          ...
          <Controller name="caption" control={form.control} render={({ field }) => (
            <textarea
              {...field}
        ...
        {needsVideo && channels.includes('tiktok') && (
          <div className="space-y-3 rounded-xl border border-[#69C9D0]/20 bg-[#69C9D0]/5 p-4">
            <p className="text-xs font-semibold text-[#69C9D0]">🎵 TikTok video details</p>
            <Controller name="title" control={form.control} render={({ field }) => (
              <Input {...field} label="Video Title (max 150 chars)" surface="light"
                placeholder="Bejewelled Gold Collection 2025" />
            )} />
            <Controller name="description" control={form.control} render={({ field }) => (
              <Input {...field} label="Description" surface="light"
