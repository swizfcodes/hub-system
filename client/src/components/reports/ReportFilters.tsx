...
import {
  DATE_PRESETS, GROUP_BY_OPTIONS, COMPARE_OPTIONS, REPORT_FAMILIES,
  getPresetRange, getCompareRange,
} from '@lib/constants/reportsConstants';
import { cn } from '@lib/cn';
import type { ReportFilters as Filters, CompareMode, GroupBy } from '@typedefs/reports';
...
export function ReportFilters({
  family, reportType, filters, onChange, onRun, isLoading,
}: ReportFiltersProps) {
  ...
  const needsDates     = params.includes('start_date') || params.includes('end_date');
  ...
  function handleCompareMode(mode: string) {
    ...
    if (cm === 'none') {
      onChange({ ...filters, compareMode: 'none', compareStart: undefined, compareEnd: undefined });
      setShowCompare(false);
    } else if (cm !== 'custom') {
      const primary = { startDate: filters.startDate, endDate: filters.endDate };
      const cmp     = getCompareRange(primary, cm);
      onChange({ ...filters, compareMode: cm, compareStart: cmp.startDate, compareEnd: cmp.endDate });
      setShowCompare(true);
    } else {
  ...
  return (
    <div className="space-y-4 rounded-2xl border border-white/5 bg-orika-charcoal p-4">
      <div className="flex flex-wrap items-start gap-4">
        ...
        {needsDates && (
          <div className="flex flex-wrap items-end gap-3">
            ...
            <div className="space-y-1">
              <p className="text-[0.65rem] uppercase tracking-widest text-orika-smoke">Quick range</p>
            ...
            <Input label="From" type="date" surface="dark"
              value={filters.startDate}
              onChange={(e) => onChange({ ...filters, startDate: e.target.value })} />
            <Input label="To" type="date" surface="dark"
        ...
        {/* As-of date */}
        {needsAsOfDate && (
          <Input label="As of Date" type="date" surface="dark"
            value={filters.endDate}
            onChange={(e) => onChange({ ...filters, endDate: e.target.value })} />
        )}
        ...
        {/* Group by */}
        {needsGroupBy && (
          <Select label="Group By" options={GROUP_BY_OPTIONS} surface="dark"
        ...
        {needsDates && (
          <div className="space-y-1">
            <p className="text-[0.65rem] uppercase tracking-widest text-orika-smoke">Compare to</p>
            <Select options={COMPARE_OPTIONS} surface="dark"
      ...
      {showCompare && filters.compareMode === 'custom' && (
        <div className="flex gap-3 border-t border-white/5 pt-3">
          ...
          <Input label="From" type="date" surface="dark"
            ...
            onChange={(e) => onChange({ ...filters, compareStart: e.target.value })} />
          <Input label="To" type="date" surface="dark"
            value={filters.compareEnd ?? ''}
            onChange={(e) => onChange({ ...filters, compareEnd: e.target.value })} />
        </div>
      )}

      {/* Comparison range display */}
      {showCompare && filters.compareMode !== 'custom' && filters.compareStart && (
