...
export function ReportTable({ report, compareReport, maxRows = 500 }: ReportTableProps) {
    ...
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto rounded-2xl border border-white/5">
        <table className="w-full text-sm">
              ...
              {columns.map((col) => (
                <th key={col.key}
                  ...
                  className={cn(
                    ...
                    NUMERIC_TYPES.includes(col.type) ? 'text-right' : 'text-left',
                  )}
                >
                  {col.label}
                  {sortKey === col.key && (
          ...
          <tbody className="divide-y divide-white/5">
            {visible.length === 0 && (
              <tr>
                <td colSpan={columns.length} className="px-4 py-12 text-center text-sm text-orika-smoke">
            ...
            {visible.map((row, i) => {
              ...
              const numericCol = columns.find((c) => NUMERIC_TYPES.includes(c.type));
              let delta: number | null = null;
              if (compareRow && numericCol) {
                const cur  = parseFloat(String(row[numericCol.key]  || 0));
                const prev = parseFloat(String(compareRow[numericCol.key] || 0));
                delta = prev !== 0 ? ((cur - prev) / prev) * 100 : null;
              }

              return (
                <tr key={i} className="bg-orika-charcoal hover:bg-orika-graphite/20 transition-colors">
                  {columns.map((col) => (
                    <td key={col.key}
                      className={cn(
                        'px-4 py-3 tabular-nums',
                        NUMERIC_TYPES.includes(col.type) ? 'text-right font-medium text-orika-cream' : 'text-orika-cloud',
                      )}>
                  ...
                  {compareReport && (
                    <td className="px-4 py-3 text-right">
                      {delta !== null ? <DeltaBadge delta={delta} /> : <span className="text-orika-smoke/30">—</span>}
                    </td>
                  )}
                </tr>
              );
                  ...
                  return (
                    <td key={col.key}
                      className={cn(
                        ...
                        NUMERIC_TYPES.includes(col.type) ? 'text-right text-orika-cream' : 'text-orika-smoke',
                      )}>
                      {i === 0 ? 'Totals' : total != null ? formatCellValue(total, col.type) : ''}
                    </td>
                  );
                })}
                {compareReport && <td />}
      ...
      {truncated && (
        <p className="text-xs text-center text-orika-smoke">
          Showing {maxRows.toLocaleString()} of {rows.length.toLocaleString()} rows — export to see all.
        </p>
      )}
    </div>
  );
...
function SummaryCards({ meta, columns }: { meta: ReportData['meta']; columns: ReportColumn[] }) {
  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      {entries.slice(0, 6).map(([key, value]) => {
        ...
        return (
          <div key={key} className="rounded-2xl border border-white/5 bg-orika-charcoal px-4 py-3">
            <p className="text-[0.65rem] uppercase tracking-widest text-orika-smoke mb-1">{label}</p>
            <p className="font-display text-xl font-light tabular-nums text-orika-cream">{formatted}</p>
          </div>
        );
      })}
    </div>
...
function DeltaBadge({ delta }: { delta: number }) {
  ...
  if (isZero) return <Minus className="mx-auto h-3.5 w-3.5 text-orika-smoke/40" />;
