...
import { REPORT_CHART_CONFIG, CHART_COLORS, formatCellValue } from '@lib/constants/reportsConstants';
...
export function ReportChart({ report, familyKey, reportType }: ReportChartProps) {
  ...
  const yKeys     = config?.yKeys ?? columns
    .filter((c) => ['int','decimal','currency','percent'].includes(c.type) && c.key !== xKey)
    .map((c) => c.key)
    .slice(0, 3);

  // Find column metadata for labels
  const getLabel = (key: string) => columns.find((c) => c.key === key)?.label ?? key;
  const getType  = (key: string) => columns.find((c) => c.key === key)?.type ?? 'int';

  const tooltipFormatter = (value: unknown, name: string) =>
    [formatCellValue(value, getType(name)), getLabel(name)];
        ...
        <ResponsiveContainer width="100%" height={320}>
          <PieChart>
            ...
            <Tooltip
              ...
              contentStyle={{ background: '#1C1C1C', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 12, color: '#F5F0E8' }}
            />
  ...
  const SeriesComponent = (chartType === "area" ? Area : Bar) as unknown as ElementType;
    ...
    <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-4">
      <ResponsiveContainer width="100%" height={320}>
        <ChartComponent {...sharedProps}>
          ...
          />
          <YAxis tickFormatter={yAxisFormatter} tick={{ fontSize: 11, fill: '#9E9891' }} />
          <Tooltip
