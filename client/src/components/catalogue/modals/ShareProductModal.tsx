...
import { Copy, Check, Mail, MessageCircle, ExternalLink, AlertTriangle, Link2 } from 'lucide-react';
...
export function ShareProductModal({ open, onClose, productId }: Props) {
  ...
  async function copy(text: string, which: 'link' | 'msg') {
    try {
      ...
      showToast.error('Could not copy', 'Your browser blocked clipboard access.');
    }
  }
  ...
  return (
    <Modal open={open} onClose={onClose} surface="light" size="md" title="Share product"
      description="Copy the link or send it via WhatsApp or email with a ready-made message.">
      {isLoading ? (
        <div className="space-y-3"><Skeleton className="h-20" /><Skeleton className="h-28" /></div>
      ) : isError || !data ? (
        <p className="text-sm text-red-600">Could not load the share details. Please try again.</p>
      ) : (
        <div className="space-y-4">
          {!data.published && (
            <div className="flex items-start gap-2 rounded-xl border border-amber-300 bg-amber-50 p-3 text-xs text-amber-800">
              ...
              <span>This product isn’t published to the website yet, so the link may not open a live page. Publish it from <strong>Edit → Storefront listing</strong> first.</span>
            </div>
          )}

          <div className="flex gap-3 items-center">
            {data.image_url ? (
              <img src={data.image_url} alt={data.name} className="w-16 h-16 rounded-xl object-cover bg-orika-cloud/30 shrink-0" />
            ) : (
              <div className="w-16 h-16 rounded-xl bg-orika-cloud/30 shrink-0" />
            )}
            <div className="min-w-0">
              <div className="text-sm font-medium text-orika-charcoal truncate">{data.name}</div>
              {data.price != null && data.price > 0 && (
                <div className="text-xs text-orika-smoke">{data.currency} {Number(data.price).toLocaleString()}</div>
              )}
            </div>
          </div>
          ...
          {/* Link row */}
          <div>
            <label className="mb-1.5 block text-xs font-medium text-orika-smoke">Link</label>
            <div className="flex gap-2">
              ...
              <Button variant="secondary" onClick={() => copy(url, 'link')} leftIcon={copied === 'link' ? <Check className="w-3.5 h-3.5 text-emerald-500" /> : <Copy className="w-3.5 h-3.5" />}>
          ...
          {/* Editable message */}
          <div>
            <label className="mb-1.5 block text-xs font-medium text-orika-smoke">Message</label>
            <textarea
              value={message}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <button onClick={() => copy(shareText, 'msg')}
              ...
              {copied === 'msg' ? <Check className="w-4 h-4 text-emerald-500" /> : <Copy className="w-4 h-4" />} {copied === 'msg' ? 'Copied' : 'Message'}
            </button>
