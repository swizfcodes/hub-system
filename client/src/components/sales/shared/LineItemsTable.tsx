    ...
    <div className={cn('w-full', className)}>
        ...
        <table className="w-full min-w-[520px] text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {lines.map((line, i) => (
              ...
              >
                <td className={cn('px-4 text-orika-cream', compact ? 'py-2.5' : 'py-3')}>
                  {line.description}
                </td>
                <td className={cn('px-4 text-right tabular-nums text-orika-cloud', compact ? 'py-2.5' : 'py-3')}>
                  {line.quantity}
                </td>
                <td className={cn('px-4 text-right tabular-nums text-orika-cloud', compact ? 'py-2.5' : 'py-3')}>
                ...
                {showDiscount && (
                  <td className={cn('px-4 text-right tabular-nums text-orika-smoke', compact ? 'py-2.5' : 'py-3')}>
                    {getDiscount(line) > 0
                      ? `−${fmtMoney(getDiscount(line), currency)}`
                      : '—'}
                  </td>
                )}
                <td className={cn('px-4 text-right tabular-nums font-medium text-orika-cream', compact ? 'py-2.5' : 'py-3')}>
      ...
      <div className="mt-3 flex justify-end">
        <div className="w-full max-w-xs space-y-2 rounded-lg border border-white/5 bg-orika-graphite/30 px-4 py-3">
          <TotalsRow label="Subtotal"  value={fmtMoney(totals.subtotal, currency)} currency={currency} />
          ...
          {totals.vat_amount > 0 && (
            <TotalsRow label="VAT (7.5%)" value={fmtMoney(totals.vat_amount, currency)} currency={currency} muted />
          )}
...
function TotalsRow({
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      <span className={cn('text-xs', muted ? 'text-orika-smoke' : 'text-orika-cloud')}>
        {label}
      </span>
      <span
        className={cn(
