...
export function KpiCard({
  label, value, type = 'number', currency = 'NGN',
  sub, delta, alertColor, restricted = false, onClick, size = 'md',
}: KpiCardProps) {
  // Format the value
  ...
  return (
    <div
      ...
      className={cn(
        ...
        onClick && 'cursor-pointer hover:border-white/15 hover:bg-orika-graphite/20',
      )}
...
export function StatRow({ label, value, type = 'number', currency = 'NGN', accent }: StatRowProps) {
  function fmt(v: string | number): string {
    const n = typeof v === 'string' ? parseFloat(v) : v;
    if (type === 'currency') return isNaN(n) ? String(v) : fmtMoney(n, currency);
    if (type === 'percent')  return isNaN(n) ? String(v) : `${n.toFixed(1)}%`;
    if (type === 'number')   return isNaN(n) ? String(v) : n.toLocaleString('en-NG');
    return String(v);
  }
  return (
    <div className="flex items-center justify-between border-b border-white/5 py-2 last:border-0">
      ...
      <span className="text-xs font-semibold tabular-nums text-orika-cream" style={accent ? { color: accent } : undefined}>
        {fmt(value)}
      </span>
    </div>
  );
