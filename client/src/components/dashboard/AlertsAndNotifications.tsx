...
import {
  X, Bell, CheckCheck, ExternalLink, Settings } from 'lucide-react';
        ...
        return (
          <div key={alert.id}
            className={cn(
              ...
              isError ? 'border-red-500/30 bg-red-900/10' : 'border-amber-500/30 bg-amber-900/10',
            )}>
            <span className="text-base shrink-0">{alert.icon}</span>
            <p className={cn('flex-1 text-sm font-medium', isError ? 'text-red-300' : 'text-amber-300')}>
              {alert.label}
            ...
            <button
              ...
              className={cn('shrink-0', isError ? 'text-red-400/50 hover:text-red-400' : 'text-amber-400/50 hover:text-amber-400')}
            >
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          <h2 className="text-base font-semibold text-orika-cream">Notifications</h2>
          {unreadCount > 0 && (
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn(
              ...
              unreadOnly ? 'bg-orika-gold text-orika-black' : 'bg-orika-graphite/30 text-orika-smoke hover:text-orika-cream',
            )}
          >
            Unread only
          </button>
...
function NotificationRow({
  notification: n, onRead, onNavigate,
}: {
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        <p className={cn('text-sm font-medium', n.is_read ? 'text-orika-smoke' : 'text-orika-cream')}>
          {n.title}
        </p>
        {n.body && (
          <p className="text-xs text-orika-smoke/70 mt-0.5 line-clamp-2">{n.body}</p>
        )}
        <p className="text-[10px] text-orika-smoke/50 mt-1">{fmtDate(n.created_at)}</p>
      </div>
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        {!n.is_read && (
          <div className="h-2 w-2 rounded-full bg-orika-gold" />
        )}
      </div>
    </div>
  );
...
function groupByDate(notifications: AppNotification[]) {
  ...
  for (const n of notifications) {
    ...
    else if (isSameDay(d, yesterday)) { buckets['Yesterday'].push(n); }
