...
import { Plus, X, Sparkles, Phone, StickyNote, CalendarPlus } from 'lucide-react';
...
export function CrmQuickActionsFab() {
  ...
  const [lastDealId] = useState<string | null>(() => localStorage.getItem('orika_last_deal_id'));
  ...
  const actions = [
    { key: 'deal',     label: 'New deal',     icon: Sparkles,     onClick: () => { setNewDeal(true); close(); } },
    { key: 'call',     label: 'Log activity', icon: Phone,        onClick: () => { if (lastDealId) { setLogActivity(true); close(); } else { navigate('/crm'); showToast(); } } },
    { key: 'note',     label: 'Add note',     icon: StickyNote,   onClick: () => { if (lastDealId) navigate(`/crm/${lastDealId}?tab=notes`); else navigate('/crm'); close(); } },
    { key: 'schedule', label: 'Schedule',     icon: CalendarPlus, onClick: () => { navigate('/calendar?new=1'); close(); } },
  ];
  ...
  return (
    <>
      {/* Backdrop on mobile */}
      {!isDesktop && open && (
        <div className="lg:hidden fixed inset-0 z-40 bg-orika-black/70 backdrop-blur-sm animate-fade-in" onClick={close} />
      )}
      ...
      {!isDesktop && open && (
        <div className="lg:hidden fixed inset-x-0 bottom-0 z-50 bg-orika-charcoal border-t border-orika-graphite rounded-t-3xl p-5 pb-safe animate-slide-up shadow-modal">
          ...
          <h3 className="text-[0.7rem] tracking-widest uppercase text-orika-gold mb-3 text-center">Quick actions</h3>
          <div className="space-y-2">
            {actions.map((a) => {
              ...
              return (
                ...
                >
                  <span className="w-10 h-10 rounded-full bg-orika-gold/15 text-orika-gold flex items-center justify-center"><Icon className="w-5 h-5" /></span>
      ...
      {isDesktop && (
        ...
        >
          <div className={cn('flex flex-col-reverse items-end gap-2 transition-all', open ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-2 pointer-events-none')}>
            {actions.map((a) => {
              const Icon = a.icon;
              return (
                <button
                ...
                >
                  <span className="w-7 h-7 rounded-full bg-orika-gold/15 text-orika-gold flex items-center justify-center"><Icon className="w-3.5 h-3.5" /></span>
                  <span className="text-xs font-semibold text-orika-cream whitespace-nowrap">{a.label}</span>
                </button>
              );
            })}
          </div>
      ...
      <NewDealModal open={newDeal} onClose={() => setNewDeal(false)} onCreated={(id) => navigate(`/crm/${id}`)} />
      {lastDealId && <LogActivityModal open={logActivity} onClose={() => setLogActivity(false)} dealId={lastDealId} />}
    </>
  );
}
...
function showToast() {
  ...
  import('@hooks/useToast').then(({ showToast }) => showToast.info('Pick a deal first', 'Open any deal to log a call against it.'));
}
