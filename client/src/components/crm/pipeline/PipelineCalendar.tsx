...
import { addMonths, startOfMonth, endOfMonth, eachDayOfInterval, isSameMonth, isSameDay, format } from 'date-fns';
...
export function PipelineCalendar({ pipeline, loading }: Props) {
  ...
  const dealsByDate = useMemo(() => {
    const map: Record<string, Array<PipelineStageWithDeals['deals'][number] & { stage_colour: string }>> = {};
  ...
  const days = useMemo(() => {
    ...
    for (let i = padBefore - 1; i >= 0; i--) {
      const d = new Date(start); d.setDate(start.getDate() - i - 1); pad.push(d);
    }
    return [...pad, ...days];
  }, [cursor]);
  ...
  if (!hasAnyDates) {
    return <EmptyState icon={<CalendarRange className="w-6 h-6" />} title="No expected close dates" description="Add expected close dates to your deals to see them in this calendar view." />;
  }
  ...
  return (
    <div className="rounded-2xl border border-orika-graphite bg-orika-charcoal/60 overflow-hidden">
      <div className="px-4 py-3 flex items-center justify-between border-b border-orika-graphite">
        <h3 className="font-display text-xl text-orika-cream">{format(cursor, 'MMMM yyyy')}</h3>
        <div className="inline-flex items-center gap-1">
          <Button variant="ghost" size="sm" onClick={() => setCursor(addMonths(cursor, -1))}><ChevronLeft className="w-4 h-4" /></Button>
          <Button variant="ghost" size="sm" onClick={() => setCursor(new Date())}>Today</Button>
          <Button variant="ghost" size="sm" onClick={() => setCursor(addMonths(cursor, 1))}><ChevronRight className="w-4 h-4" /></Button>
        </div>
      </div>

      <div className="grid grid-cols-7 border-b border-orika-graphite">
        {['Mon','Tue','Wed','Thu','Fri','Sat','Sun'].map((d) => (
          <div key={d} className="px-2 py-2 text-[0.6rem] uppercase tracking-widest text-orika-smoke font-semibold text-center">{d}</div>
        ))}
      </div>
          ...
          return (
            <div key={i} className={cn(
              ...
              <div className={cn(
                ...
                isToday ? 'bg-orika-gold text-orika-black font-bold' : 'text-orika-smoke',
              )}>{format(day, 'd')}</div>
              <div className="mt-1 space-y-1">
                {list.slice(0, 3).map((d) => (
                  <button
                    ...
                    style={{ borderLeft: `3px solid ${d.stage_colour}`, color: '#F0EAE0' }}
                ...
                {list.length > 3 && (
                  <div className="text-[0.55rem] text-orika-smoke px-1.5">+{list.length - 3} more</div>
                )}
              </div>
            </div>
          );
