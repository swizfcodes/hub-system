...
export function DealActivityFeed({ activities }: Props) {
  ...
  return (
    <ol className="relative pl-7 border-l border-orika-graphite space-y-4">
      {sorted.map((a) => {
        const meta = CRM_ACTIVITY_TYPES[a.activity_type] ?? CRM_ACTIVITY_TYPES.note;
        return (
          <li key={a.activity_id} className="relative">
            <span className="absolute -left-[34px] top-2">
              <ActivityIcon type={a.activity_type} direction={a.direction} size="sm" />
            </span>
            <div className="rounded-xl border border-orika-graphite bg-orika-charcoal/50 p-3.5">
              <div className="flex items-start justify-between gap-2 flex-wrap">
                ...
                <span className={`text-[0.6rem] uppercase tracking-widest font-semibold ${meta.textClass}`}>
                  {meta.label}{a.is_auto && ' · auto'}
                </span>
              </div>
              <div className="text-[0.65rem] text-orika-smoke mt-1.5">{fmtDateTime(a.performed_at)} · {fmtRelative(a.performed_at)}</div>
            </div>
          </li>
        );
      })}
