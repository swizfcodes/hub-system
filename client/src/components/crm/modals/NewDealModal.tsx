...
export function NewDealModal({ open, onClose, defaultStage, defaultContactId, onCreated }: Props) {
  ...
  const { register, control, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<DealCreateValues>({
    ...
    defaultValues: {
      ...
      title: '', stage: defaultStage ?? '',
      expected_value: undefined, probability: 50,
  ...
  const mutation = useMutation({
    mutationFn: (v: DealCreateValues) => createDeal({
      ...v,
  ...
  return (
    <Modal open={open} onClose={() => { reset(); onClose(); }} surface="light" size="md" title="New deal"
      footer={<>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                {...field}
                label="Stage"
                options={(pipeline?.pipeline ?? []).map((s) => ({ value: s.stage_key, label: s.stage_label }))}
                error={errors.stage?.message}
              />
            )}
          />
          <Input {...register('expected_value', { valueAsNumber: true })} type="number" step="0.01" label="Expected value (NGN)" placeholder="0" />
          <Input {...register('probability', { valueAsNumber: true })} type="number" min={0} max={100} label="Probability (%)" />
          <Input {...register('expected_close_date')} type="date" label="Expected close" />
        </div>
        <Select {...register('source')} label="Source (optional)"
          options={[
            { value: '', label: '—' }, { value: 'walk_in', label: 'Walk in' }, { value: 'referral', label: 'Referral' },
            { value: 'social_media', label: 'Social media' }, { value: 'repeat', label: 'Repeat client' },
            { value: 'campaign', label: 'Campaign' }, { value: 'website', label: 'Website' }, { value: 'event', label: 'Event' },
          ]} />
      </form>
    </Modal>
  );
