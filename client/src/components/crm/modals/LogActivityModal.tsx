...
import { CRM_ACTIVITY_TYPES, QUICK_TEMPLATE_ORDER, DEFAULT_SUMMARIES } from '@lib/constants/crmActivityTypes';
...
export function LogActivityModal({ open, onClose, dealId, defaultType }: Props) {
  const qc = useQueryClient();
  const [type, setType] = useState<ActivityType>(defaultType ?? 'call');

  const { register, handleSubmit, reset, setValue, watch, formState: { errors, isSubmitting } } = useForm<LogActivityValues>({
  ...
  return (
    <Modal open={open} onClose={() => { reset(); onClose(); }} surface="light" size="md" title="Log activity"
      description="A few seconds now saves you hours later."
      footer={<>
        <Button variant="outline-light" onClick={() => { reset(); onClose(); }}>Cancel</Button>
        <Button variant="primary" loading={isSubmitting || mutation.isPending} onClick={handleSubmit((v) => mutation.mutate(v))}>Log activity</Button>
      </>}>
      <div className="space-y-5">
        {/* Type chips */}
        <div>
          <div className="text-[0.7rem] tracking-widest uppercase font-medium text-text-on-light-muted mb-2 ml-1">What happened?</div>
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active ? 'border-orika-black bg-orika-black text-orika-cream shadow-card' : 'bg-white border-orika-cloud/40 text-orika-black/70 hover:border-orika-black/40',
                  )}
                >
                  <Icon className="w-5 h-5" style={{ color: active ? m.color : undefined }} />
                  <span className="text-[0.65rem] font-semibold">{m.label}</span>
                  {m.shortcut && <kbd className="text-[0.55rem] px-1 py-0.5 rounded bg-orika-cloud/40 text-orika-black/60">{m.shortcut}</kbd>}
                </button>
              );
            })}
          </div>
        ...
        {CRM_ACTIVITY_TYPES[type].defaultDirection && (
          <div>
            <div className="text-[0.7rem] tracking-widest uppercase font-medium text-text-on-light-muted mb-2 ml-1">Direction</div>
              ...
              {(['outbound', 'inbound'] as const).map((d) => (
                <button
                  ...
                  className={cn(
                    ...
                    direction === d ? 'bg-white shadow-sm text-orika-black' : 'text-text-on-light-muted hover:text-orika-black',
                  )}
                >
                  {d === 'inbound' ? <ArrowDownRight className="w-3 h-3" /> : <ArrowUpRight className="w-3 h-3" />}
