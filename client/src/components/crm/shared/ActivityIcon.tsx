...
export function ActivityIcon({ type, direction, size = 'sm', className }: Props) {
  ...
  return (
    <div
      className={cn('relative rounded-full flex items-center justify-center shrink-0', dims.box, className)}
      ...
      {direction && (
        <span
          className={cn(
            ...
            direction === 'inbound'  ? 'bg-living-sage text-orika-black border-orika-charcoal' : 'bg-orika-gold text-orika-black border-orika-charcoal',
          )}
        >
          {direction === 'inbound' ? '↓' : '↑'}
        </span>
