  ...
  useEffect(() => {
    function onKeydown(e: KeyboardEvent) {
      if ((e.metaKey || e.ctrlKey) && e.key === 'k') {
        e.preventDefault();
        if (open) onClose(); else {/* parent opens it */}
      }
    }
    ...
    >
      ...
      <div className="w-full max-w-xl rounded-2xl border border-white/10 bg-orika-black shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/5">
          ...
          />
          {query && (
            <button onClick={() => { setQuery(''); setResults([]); }} className="text-orika-smoke hover:text-orika-cream transition-colors">
              <X className="h-4 w-4" />
            </button>
          )}
        ...
        <div className="max-h-[55vh] overflow-y-auto">
          {query.trim().length === 0 ? (
            ...
            <div className="px-4 py-8 text-center space-y-4">
              ...
              <div className="flex items-center justify-center gap-4 text-xs text-orika-smoke/30">
                <span className="flex items-center gap-1">
                  <kbd className="px-1.5 py-0.5 bg-orika-graphite rounded border border-white/10 font-mono">↑↓</kbd>
                  navigate
                </span>
                <span className="flex items-center gap-1">
                  <kbd className="px-1.5 py-0.5 bg-orika-graphite rounded border border-white/10 font-mono">↵</kbd>
                ...
                <span className="flex items-center gap-1">
                  <kbd className="px-1.5 py-0.5 bg-orika-graphite rounded border border-white/10 font-mono">Esc</kbd>
                  close
                </span>
              </div>
            </div>
            ...
            <div className="px-4 py-6 text-center">
              <p className="text-sm text-orika-smoke/40">Keep typing to search…</p>
            </div>
          ) : results.length === 0 && !loading ? (
            <div className="px-4 py-8 text-center">
              <p className="text-sm text-orika-smoke/50">No results for <span className="text-orika-cream">"{query}"</span></p>
            </div>
          ) : (
            /* Grouped results */
            Object.entries(grouped).map(([category, items]) => {
              const { label, Icon } = CATEGORY_META[category as keyof typeof CATEGORY_META];
              return (
                <div key={category}>
                  {/* Group header */}
                  <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 bg-orika-charcoal/40">
                    ...
                    <span className="ml-auto text-[10px] text-orika-smoke/30">{items.length}</span>
                  </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className={cn(
                            'text-sm truncate',
                            isActive ? 'text-orika-cream font-medium' : 'text-orika-smoke',
                          )}>
                            {result.label}
                          </p>
                          {result.sublabel && (
                            <p className="text-[11px] text-orika-smoke/40 truncate">{result.sublabel}</p>
                          )}
                        </div>
                        {isActive && (
