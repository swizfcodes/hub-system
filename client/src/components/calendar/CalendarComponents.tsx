...
import { createEvent, updateEvent, deleteEvent, listParticipants } from '@services/calendar';
import { createEventSchema, type CreateEventValues } from '@lib/schemas/scheduling';
import {
  EVENT_TYPE_META, EVENT_TYPE_OPTIONS, fmtEventRange, REF_TYPE_LABEL,
...
export function EventTypeChip({
  type, size = 'sm',
}: { type: string; size?: 'xs' | 'sm' }) {
  const meta = EVENT_TYPE_META[type as keyof typeof EVENT_TYPE_META] ?? EVENT_TYPE_META.other;
  return (
    <span
      className={cn(
...
export function EventPill({
  event, onClick,
}: { event: CalendarEvent; onClick: (e: CalendarEvent) => void }) {
...
export function ClashWarningModal({ open, clashes, onForce, onCancel }: ClashWarningModalProps) {
  return (
    <Modal open={open} onClose={onCancel} title="Booking Clash Detected" size="sm" surface="light"
      footer={
      ...
      <div className="space-y-3">
        <p className="text-sm text-text-on-light-muted">
          This time slot conflicts with {clashes.length} existing event{clashes.length > 1 ? 's' : ''} at this location:
        </p>
        {clashes.map((c) => (
          <div key={c.event_id} className="rounded-xl border border-state-danger/20 bg-state-danger/5 px-4 py-3">
            <p className="font-medium text-orika-black text-sm">{c.title}</p>
            <p className="text-xs text-text-on-light-muted">
              {fmtDate(c.start_at)} · {new Date(c.start_at).toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit' })} – {new Date(c.end_at).toLocaleTimeString('en-NG', { hour: '2-digit', minute: '2-digit' })}
            </p>
          </div>
        ))}
      </div>
...
export function EventFormModal({ open, onClose, existing, defaultStart }: EventFormModalProps) {
  ...
  const form = useForm<CreateEventValues>({
    ...
    defaultValues: {
      ...
      event_type:  (existing?.event_type ?? 'meeting') as CreateEventValues['event_type'],
        ...
        footer={
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex gap-3 ml-auto">
              ...
              <Button
                onClick={form.handleSubmit((v) => mutation.mutate(v as CreateEventValues))}
      ...
      >
        <div className="space-y-4">
          <Controller name="title" control={form.control} render={({ field, fieldState }) => (
            <Input {...field} label="Title *" placeholder="e.g. Client Meeting with Adaeze" surface="light"
              error={fieldState.error?.message} />
          )} />

          <div className="grid grid-cols-2 gap-3">
            <Controller name="event_type" control={form.control} render={({ field }) => (
              <Select label="Type *" options={EVENT_TYPE_OPTIONS}
                value={field.value} onChange={(e) => field.onChange(e.target.value)} surface="light" />
            )} />
            <div className="flex items-end">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" {...form.register('all_day')} className="rounded" />
          ...
          <div className="grid grid-cols-2 gap-3">
            <Controller name="start_at" control={form.control} render={({ field, fieldState }) => (
              <Input {...field} label="Start *" type={allDay ? 'date' : 'datetime-local'}
                surface="light" error={fieldState.error?.message} />
            )} />
            <Controller name="end_at" control={form.control} render={({ field, fieldState }) => (
              <Input {...field} label="End *" type={allDay ? 'date' : 'datetime-local'}
                surface="light" error={fieldState.error?.message} />
            )} />
          </div>

          <Controller name="location" control={form.control} render={({ field }) => (
            <Input {...field} label="Location" placeholder="e.g. Lekki Showroom, Zoom" surface="light"
              leftIcon={<MapPin className="h-4 w-4 text-gray-400" />} />
          )} />

          <Controller name="description" control={form.control} render={({ field }) => (
            <Input {...field} label="Description / Agenda" placeholder="Add notes or agenda items"
              surface="light" />
          )} />

          <div className="flex items-center gap-3">
            <label className="flex items-center gap-2 text-sm cursor-pointer">
              <input type="checkbox" {...form.register('is_private')} className="rounded" />
              <Lock className="h-3.5 w-3.5 text-gray-400" />
              <span className="text-text-on-light-muted">Private — show as "Busy" to others</span>
            </label>
          </div>
        </div>
      </Modal>
...
export function EventDetailPanel({ event, onEdit, onClose }: EventDetailPanelProps) {
  const meta = EVENT_TYPE_META[event.event_type] ?? EVENT_TYPE_META.other;
  ...
  return (
    <div className="space-y-4">
      ...
      {event.description && (
        <p className="text-sm text-orika-smoke whitespace-pre-wrap">{event.description}</p>
      )}
      ...
      {participants.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke">Participants</p>
          {participants.map((p) => (
            <div key={p.participant_id} className="flex items-center justify-between text-sm">
              <span className="text-orika-cloud">{p.display_name ?? 'Unknown'}</span>
              <Badge
                tone={p.status === 'accepted' ? 'sage' : p.status === 'declined' ? 'danger' : 'neutral'}
                size="xs"
              >
                {p.status}
              </Badge>
