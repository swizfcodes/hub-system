...
import type { CreateAccountValues, CreateJournalValues } from '@lib/schemas/accounting';
...
export async function listAccounts(
  ...
  try {
    const { data } = await api.get<{ data: Account[] }>('/accounting/accounts', { params });
    return data.data ?? [];
  } catch {
    return [];
  }
...
export async function updateAccount(
  ...
  const { data } = await api.patch<Account>(`/accounting/accounts/${id}`, values);
  return data;
}
...
export async function listJournals(
  params: { reference_type?: string; start_date?: string; end_date?: string } = {},
): Promise<{ data: JournalEntry[] }> {
  try {
    const { data } = await api.get<{ data: JournalEntry[] }>('/accounting/journals', { params });
    return { data: data.data ?? [] };
  } catch {
    return { data: [] };
  }
...
export async function createManualJournal(values: CreateJournalValues): Promise<JournalEntry> {
...
export async function reverseJournal(id: string): Promise<JournalEntry> {
  const { data } = await api.post<JournalEntry>(`/accounting/journals/${id}/reverse`, {});
  return data;
}
...
export async function getProfitAndLoss(
  ...
  const { data } = await api.get<PLReport>('/accounting/reports/profit-and-loss', { params });
  return data;
}
...
export async function getCashFlow(
  ...
  const { data } = await api.get<CashFlowReport>('/accounting/reports/cash-flow', { params });
  return data;
}
...
export async function reconcileItem(
  payload: { statement_id: string; payment_id: string },
): Promise<void> {
...
export async function listFiscalPeriods(): Promise<FiscalPeriod[]> {
  try {
    const { data } = await api.get<{ data: FiscalPeriod[] }>('/accounting/fiscal-periods');
    return data.data ?? [];
  } catch {
    return [];
  }
...
export async function closePeriod(periodId: string): Promise<FiscalPeriod> {
  const { data } = await api.post<FiscalPeriod>(`/accounting/fiscal-periods/${periodId}/close`, {});
  return data;
}

export async function reopenPeriod(periodId: string): Promise<FiscalPeriod> {
  const { data } = await api.post<FiscalPeriod>(`/accounting/fiscal-periods/${periodId}/reopen`, {});
  return data;
}
