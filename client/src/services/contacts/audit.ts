...
export async function getRecordAudit(table: string, recordId: string, limit = 50): Promise<AuditEntry[]> {
  try {
    const { data } = await api.get<{ data: AuditEntry[] }>(`/audit/record/${table}/${recordId}`, { params: { limit } });
    return data.data ?? [];
  } catch {
