...
export async function deleteTask(id: string): Promise<{ task_id: string; is_deleted: boolean }> {
  const { data } = await api.delete<{ task_id: string; is_deleted: boolean }>(`/tasks/${id}`);
  return data;
}

// ── Subtasks ──
export async function listSubtasks(taskId: string): Promise<Subtask[]> {
  const { data } = await api.get<{ data: Subtask[] }>(`/tasks/${taskId}/subtasks`);
  return data.data;
}
export async function addSubtask(taskId: string, payload: { title: string; display_order?: number }): Promise<Subtask> {
  const { data } = await api.post<Subtask>(`/tasks/${taskId}/subtasks`, payload);
  return data;
}
export async function setSubtaskDone(subtaskId: string, is_done: boolean): Promise<Subtask> {
  const { data } = await api.patch<Subtask>(`/tasks/subtasks/${subtaskId}`, { is_done });
  return data;
}
export async function deleteSubtask(subtaskId: string): Promise<{ deleted: boolean }> {
  const { data } = await api.delete<{ deleted: boolean }>(`/tasks/subtasks/${subtaskId}`);
  return data;
}
