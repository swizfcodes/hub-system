...
export async function listBusinesses(includeInactive = false): Promise<Business[]> {
...
export async function updateBusiness(key: string, patch: Partial<Business>): Promise<Business> {
  const { data } = await api.patch<Business>(`/settings/businesses/${key}`, patch);
  return data;
}

export async function deactivateBusiness(key: string): Promise<{ business_key: string; is_active: boolean }> {
