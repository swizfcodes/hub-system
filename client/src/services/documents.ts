...
import type {
  HubDocument, DocumentsResponse, DocumentsFilter,
  VerifyResult, TagStat, DocumentTag,
  UploadDocumentInput,
} from '@typedefs/documents';

export async function listDocuments(filters: DocumentsFilter): Promise<DocumentsResponse> {
...
export async function downloadDocument(
  doc: Pick<HubDocument, 'document_id' | 'document_number' | 'title' | 'mime_type'>
...
export async function addDocumentTag(
  ...
  const { data } = await api.post<DocumentTag>(
    `/documents/${id}/tags`,
    { tag_name: tagName, colour }
  );
  return data;
}
...
export async function listDocumentTags(business: string): Promise<TagStat[]> {
  const { data } = await api.get<{ data: TagStat[] }>(
    '/documents/tags',
    { params: { business } }
  );
  return data.data;
