...
export async function generateReport(params: GenerateParams): Promise<ReportData> {
  const { family, reportType, format = 'json', archive, ...rest } = params;
  const { data } = await api.get<ReportData>(`/reports/${family}/${reportType}`, {
    params: {
...
export async function generateConsolidated(
  family: string, reportType: string, startDate: string, endDate: string,
): Promise<ReportData> {
  const { data } = await api.get<ReportData>(`/reports/consolidated/${family}/${reportType}`, {
...
export async function createSavedReport(values: {
  report_name: string; report_type: string; filters?: Record<string, unknown>;
  is_shared?: boolean; schedule?: Record<string, unknown>;
}): Promise<SavedReport> {
...
export async function updateSavedReport(
  id: string, values: Partial<{ report_name: string; filters: Record<string,unknown>; is_shared: boolean; schedule: Record<string,unknown> }>,
): Promise<SavedReport> {
...
export async function getSalesDashboard(params?: { start_date?: string; end_date?: string }) {
  try {
...
export async function getFinanceDashboard(params?: { start_date?: string; end_date?: string }) {
  try {
...
export async function getCustomerDashboard(params?: { start_date?: string; end_date?: string }) {
  try {
...
export async function getLogisticsDashboard(params?: { start_date?: string; end_date?: string }) {
  try {
...
export async function getOverview(params?: { start_date?: string; end_date?: string }) {
  try {
