...
import type { CreditNote, CreditNoteListResponse, CreditNoteStatus } from '@typedefs/invoicing';
...
export async function listCreditNotes(params?: {
  ...
  try {
    const { data } = await api.get<CreditNoteListResponse>('/invoicing/credit-notes', { params });
    return data;
  } catch {
    return { data: [] };
  }
