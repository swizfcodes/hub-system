...
export async function initiateRun(
...
export async function markRunPaid(id: string): Promise<PayrollRun> {
  const { data } = await api.post<PayrollRun>(`/payroll/runs/${id}/mark-paid`, {});
  return data;
}
...
export async function getPayslips(runId: string): Promise<{ data: Payslip[] }> {
  try {
    const { data } = await api.get<{ data: Payslip[] }>(`/payroll/runs/${runId}/payslips`);
