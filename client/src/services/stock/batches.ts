...
export async function listBatches(params: { product_id?: string; expiring_within_days?: number } = {}): Promise<StockBatch[]> {
  try {
    const { data } = await api.get<{ data: StockBatch[] } | StockBatch[]>('/stock/batches', { params });
...
export async function createBatch(payload: BatchCreateValues): Promise<StockBatch> {
...
export function batchExpiryStatus(expiryISO?: string | null): 'fresh' | 'soon' | 'critical' | 'expired' | null {
  if (!expiryISO) return null;
  const days = Math.round((new Date(expiryISO).getTime() - Date.now()) / 86400000);
