...
import {
  Shield, Users, Key, BookOpen, AlertTriangle, CheckCircle,
  LogIn, Clock, Download, ChevronRight, Search, UserPlus,
  Eye, EyeOff, Trash2, RefreshCw } from 'lucide-react';
...
import { PermissionMatrix, RoleEditor } from '@components/security/PermissionMatrix';
import { InviteModal, SessionsPanel }   from '@components/security/InviteAndSessions';
import {
  getSecurityStats, queryAuditLog, downloadAuditCsv,
  listStaffUsers, deactivateLogin, resetPassword,
    ...
    <div className="px-4 sm:px-8 py-6 max-w-6xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          <button key={item.href} onClick={() => navigate(item.href)}
            ...
            <p className="text-xs font-medium text-orika-cream text-center">{item.label}</p>
          </button>
        ))}
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="space-y-3">
          <p className="text-sm font-semibold text-orika-cream">Security Health</p>
          {isLoading ? (
              ...
              {healthItems.map((item) => (
                <button key={item.id} onClick={() => navigate(item.href)}
                  className={cn(
                    ...
                    item.severity === 'error' ? 'border-red-500/30 bg-red-900/10' :
                    item.severity === 'warn'  ? 'border-amber-500/30 bg-amber-900/10' :
                    'border-emerald-500/30 bg-emerald-900/10',
                  )}>
                  {item.severity === 'ok'
                    ? <CheckCircle className="h-4 w-4 shrink-0 text-emerald-400" />
                    : <AlertTriangle className={cn('h-4 w-4 shrink-0', item.severity === 'error' ? 'text-red-400' : 'text-amber-400')} />}
                  <p className={cn('flex-1 text-sm', item.severity === 'ok' ? 'text-emerald-300' : item.severity === 'error' ? 'text-red-300' : 'text-amber-300')}>
                    {item.label}
                  </p>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-orika-cream">Recent Events</p>
            <button onClick={() => navigate('/security/audit')}
              className="text-xs text-orika-gold hover:underline">View all</button>
          </div>
          {isLoading ? (
                ...
                <div key={e.log_id}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-orika-cream truncate">
                      ...
                      <span className="text-orika-smoke"> · {e.module} / {e.action}</span>
                    </p>
                    <p className="text-[10px] text-orika-smoke/50">{fmtDate(e.occurred_at)}</p>
                  </div>
                </div>
              ))}
              {(stats?.recent_events ?? []).length === 0 && (
                <p className="text-sm text-orika-smoke text-center py-6">No recent events</p>
              )}
            </div>
          )}
        </div>
...
export function UsersPage() {
  ...
  const deactivateMutation = useMutation({
    ...
    onSuccess: () => { showToast.success('Access deactivated'); qc.invalidateQueries({ queryKey: ['staff'] }); },
    onError: (err) => showToast.error(errMsg(err)),
  });

  const resetMutation = useMutation({
    mutationFn: (profileId: string) => resetPassword(profileId),
    onSuccess: (result) => showToast.success(`Temp password: ${result.temp_password} — send securely`),
    onError: (err) => showToast.error(errMsg(err)),
  });
            ...
            <div key={user.profile_id}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <p className="font-medium text-orika-cream">{user.display_name}</p>
                  {user.role_name && (
                    <Badge tone="neutral" size="xs">{user.role_name}</Badge>
                  )}
                  {!user.user_id && (
                    <Badge tone="warn" size="xs">No login</Badge>
                  )}
                  {user.user_id && !user.is_active && (
                    <Badge tone="danger" size="xs">Deactivated</Badge>
                  )}
                </div>
                <p className="text-xs text-orika-smoke mt-0.5">
                  {user.email ?? 'No email'}
                  {user.job_title ? ` · ${user.job_title}` : ''}
                  {user.last_login_at ? ` · Last login ${fmtDate(user.last_login_at)}` : user.user_id ? ' · Never logged in' : ''}
                </p>
              </div>
              ...
              <div className="hidden sm:flex gap-1 shrink-0">
                <Badge tone="info" size="xs">{BUSINESS_LABELS[user.business] ?? user.business}</Badge>
              </div>

              {/* Actions */}
              {user.user_id && (
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => setSessions(user)} title="View sessions"
                  ...
                  {user.is_active && (
                    <button onClick={() => {
                      if (confirm(`Deactivate ${user.display_name}'s access? They will be logged out.`))
...
export function AuditLogPage() {
  const [filters, setFilters] = useState({
    module: '', action: '', start_date: '', end_date: '', page: 1,
  });
  ...
  const actionOptions = [
    ...
    ...['create','edit','delete','login','logout','permission_change','export','approve','invite_sent','account_created'].map((a) => ({ value: a, label: a })),
  ];
...
function AuditRow({ entry, isExpanded, onToggle }: {
  entry: AuditLogEntry; isExpanded: boolean; onToggle: () => void;
}) {
  const isSecurityAction = ['login','logout','permission_change','provision_login'].includes(entry.action);
  return (
    <div className={cn(
      ...
      <button onClick={onToggle}
        ...
        <span className="text-xs text-orika-smoke">{MODULE_LABELS[entry.module] ?? entry.module}</span>
        <span
        ...
        {entry.ip_address && (
          <span className="text-[10px] text-orika-smoke/40 shrink-0">{entry.ip_address}</span>
        )}
      </button>
      {isExpanded && (
        <div className="border-t border-white/5 px-4 py-4 space-y-3">
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div>
              ...
              <p className="text-orika-cream font-mono">{entry.table_name ?? '—'}</p>
            </div>
            <div>
              <p className="text-orika-smoke/60 mb-1">Record ID</p>
              <p className="text-orika-cream font-mono truncate">{entry.record_id ?? '—'}</p>
            </div>
          </div>
          {(entry.before_state || entry.after_state) && (
            <div className="grid gap-3 md:grid-cols-2">
              {entry.before_state && (
                <div>
                  <p className="text-[10px] text-red-400/70 uppercase tracking-widest mb-1">Before</p>
              ...
              {entry.after_state && (
                <div>
                  <p className="text-[10px] text-emerald-400/70 uppercase tracking-widest mb-1">After</p>
...
export function AcceptInvitePage() {
  ...
  const [tokenData, setTokenData] = useState<import('@typedefs/security').InviteToken | null>(null);
  ...
  useEffect(() => {
    if (!token) { setTokenError('Invalid invite link'); setTokenLoading(false); return; }
    verifyInviteToken(token).then((data) => {
      if (!data) {
        setTokenError('This invite link is invalid, has expired, or has already been used.');
      } else {
        setTokenData(data);
  ...
  const mutation = useMutation({
    mutationFn: (values: AcceptValues) => acceptInvite(token, {
  ...
  if (tokenLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-orika-black">
        <div className="animate-pulse text-orika-smoke text-sm">Verifying invite link…</div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-orika-black px-4">
        <div className="max-w-sm w-full rounded-2xl border border-red-500/30 bg-orika-charcoal px-8 py-10 text-center space-y-4">
          ...
          <h1 className="text-lg font-semibold text-orika-cream">Link Unavailable</h1>
          <p className="text-sm text-orika-smoke">{tokenError ?? 'This invite link cannot be used.'}</p>
          <p className="text-xs text-orika-smoke/50">Please ask your admin to send a new invite.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-orika-black px-4">
        <div className="max-w-sm w-full rounded-2xl border border-emerald-500/30 bg-orika-charcoal px-8 py-10 text-center space-y-4">
          ...
          <h1 className="text-lg font-semibold text-orika-cream">Account Created!</h1>
          <p className="text-sm text-orika-smoke">Your Orika Hub account is ready.</p>
    ...
    <div className="min-h-screen flex items-center justify-center bg-orika-black px-4">
      <div className="max-w-sm w-full space-y-6">
        ...
        <div className="text-center">
          ...
          <h1 className="text-2xl font-display font-light text-orika-cream">Welcome to Orika Hub</h1>
          <p className="text-sm text-orika-smoke mt-1">
            You've been invited as <strong className="text-orika-cream">{tokenData.role_name}</strong>
          </p>
          <p className="text-xs text-orika-smoke/50 mt-0.5">
            Access: {tokenData.businesses.map((b) => BUSINESS_LABELS[b] ?? b).join(', ')}
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-orika-charcoal px-6 py-6 space-y-4">
          <p className="text-[0.65rem] text-orika-smoke text-center">
            Complete your account setup. This link expires {fmtDate(tokenData.expires_at)}.
          </p>

          <Controller name="display_name" control={form.control} render={({ field, fieldState }) => (
            <Input {...field} label="Your Full Name *" surface="dark"
              placeholder="Amara Okafor" error={fieldState.error?.message} />
          )} />

          <div className="relative">
            <Controller name="password" control={form.control} render={({ field, fieldState }) => (
              <Input {...field} label="Password *" type={showPassword ? 'text' : 'password'}
                surface="dark" hint="Minimum 12 characters"
                rightSlot={
          ...
          <Controller name="confirm_password" control={form.control} render={({ field, fieldState }) => (
            <Input {...field} label="Confirm Password *" type="password"
              surface="dark" error={fieldState.error?.message} />
          )} />
