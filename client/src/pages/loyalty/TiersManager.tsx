...
export default function TiersManager() {
  ...
  function handleDrop(e: React.DragEvent, targetIndex: number) {
    ...
    const updates = reordered.map((t, i) => ({ tier_id: t.tier_id, display_order: i }));
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal py-16 text-center">
          ...
          <p className="text-xs text-orika-smoke/50 mt-1">
            Create tiers to segment your loyalty members (e.g. Bronze, Silver, Gold)
          </p>
          ...
          {sortedTiers.map((tier, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-orika-cream">{tier.tier_name}</p>
              ...
              {Object.keys(tier.benefits ?? {}).length > 0 && (
                <div className="hidden sm:flex flex-wrap gap-1 max-w-[200px]">
                  {Object.keys(tier.benefits).slice(0, 3).map((key) => (
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button
                  type="button"
                  onClick={() => {
                    if (!confirm(`Delete tier "${tier.tier_name}"? This cannot be undone if members are in this tier.`)) return;
