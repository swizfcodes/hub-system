    ...
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
      ...
      {statsLoading ? (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: 'Points Issued',    value: (stats?.total_issued    ?? 0).toLocaleString(), color: '#C9A86C' },
            { label: 'Points Redeemed',  value: (stats?.total_redeemed  ?? 0).toLocaleString(), color: '#4E9AF1' },
            { label: 'Active Members',   value: (stats?.active_contacts ?? 0).toLocaleString(), color: '#2D9CDB' },
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-orika-cream">Tier Distribution</p>
            <Button size="sm" variant="ghost" onClick={() => navigate('/loyalty/tiers')}>
          ...
          {statsLoading ? (
            ...
            <div className="rounded-2xl border border-white/5 bg-orika-charcoal py-10 text-center">
              ...
              <p className="text-sm text-orika-smoke">No tiers configured yet</p>
            ...
            <div className="space-y-2">
              {stats.tier_distribution.map((td) => {
                const totalMembers = stats.tier_distribution.reduce((s, t) => s + t.member_count, 0);
                const pct = totalMembers > 0 ? (td.member_count / totalMembers) * 100 : 0;

                return (
                  <div key={td.tier_id}
                    className="flex items-center gap-4 rounded-xl border border-white/5 bg-orika-charcoal px-4 py-3">
                    <div className="h-3 w-3 rounded-full shrink-0" style={{ backgroundColor: td.colour }} />
                    <p className="text-sm text-orika-cream flex-1">{td.tier_name}</p>
        ...
        {/* Leaderboard */}
        <div className="space-y-4">
          <p className="text-sm font-semibold text-orika-cream">Top Members by Points</p>

          {leadLoading ? (
            <div className="space-y-2">{[1,2,3,4,5].map((i) => <Skeleton key={i} className="h-12 rounded-xl" />)}</div>
          ) : leaderboard.length === 0 ? (
            <div className="rounded-2xl border border-white/5 bg-orika-charcoal py-10 text-center">
              <p className="text-sm text-orika-smoke">No loyalty members yet</p>
            </div>
          ) : (
            <div className="space-y-1.5">
              {leaderboard.slice(0, 15).map((row, i) => {
                ...
                const tier     = [...tiers]
                  ...
                  .find((t) => t.min_points <= balance && (t.max_points === null || t.max_points >= balance)) ?? null;

                return (
                  <button key={row.contact_id}
                    onClick={() => navigate(`/loyalty/contact/${row.contact_id}`)}
                    ...
                    <span className={cn(
                      ...
                      i === 0 ? 'text-orika-gold' : i === 1 ? 'text-[#A8A9AD]' : i === 2 ? 'text-[#B87333]' : 'text-orika-smoke/50',
                    )}>
                      {i + 1}
                    </span>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-orika-cream truncate">{row.display_name}</p>
                      {row.primary_phone && (
                        <p className="text-[10px] text-orika-smoke">{row.primary_phone}</p>
                      )}
                    </div>
