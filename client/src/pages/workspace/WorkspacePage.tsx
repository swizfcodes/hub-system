...
import { Plus, ChevronRight, AlertCircle, CheckCircle2, Clock, Users, Calendar } from 'lucide-react';
import { PageHeader } from '@components/ui/PageHeader';
import { Button } from '@components/ui/Button';
import { Skeleton } from '@components/ui/Skeleton';
import { Badge } from '@components/ui/Badge';
import { EventFormModal, EventDetailPanel, EventTypeChip } from '@components/calendar/CalendarComponents';
...
export default function WorkspacePage() {
  ...
  const endStr   = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 23, 59, 59).toISOString();
  ...
  const byAssignee = allActiveTasks.reduce((acc, task) => {
    const name = task.assigned_to_name ?? 'Unassigned';
    if (!acc[name]) acc[name] = { overdue: 0, urgent: 0, total: 0 };
    acc[name].total++;
    if (task.due_at && new Date(task.due_at) < today && !task.completed_at) acc[name].overdue++;
    ...
    <>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-orika-cream">Today's Schedule</p>
          ...
          {eventsLoading ? (
            ...
            <div className="rounded-2xl border border-white/5 bg-orika-charcoal py-10 text-center">
              ...
              <p className="text-sm text-orika-smoke">Nothing scheduled for today</p>
              <Button variant="ghost" size="sm" className="mt-3" onClick={() => setShowCreateEvent(true)}>
                Add event
              </Button>
            </div>
          ) : (
                ...
                .map((event) => (
                  ...
                  >
                    <div className="text-center w-12 shrink-0">
                      <p className="text-sm font-semibold text-orika-cream">{fmtTime(event.start_at)}</p>
                      <p className="text-[10px] text-orika-smoke">{fmtTime(event.end_at)}</p>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-orika-cream truncate">{event.title}</p>
                      {event.location && (
                        <p className="text-xs text-orika-smoke truncate">{event.location}</p>
                      )}
                    </div>
          ...
          {/* Upcoming this week */}
          {weekEvents.filter((e) => !isToday(new Date(e.start_at))).length > 0 && (
            <>
                  ...
                  .map((e) => (
                    <div key={e.event_id}
                      ...
                      <span className="text-xs text-orika-smoke w-16 shrink-0">
                        {new Date(e.start_at).toLocaleDateString('en-NG', { weekday: 'short', day: 'numeric' })}
                      </span>
                      <span className="text-orika-cloud truncate">{e.title}</span>
                    </div>
                  ))}
              </div>
            </>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-orika-cream">Team Task Health</p>
          ...
          {boardLoading ? (
            ...
            <div className="rounded-2xl border border-white/5 bg-orika-charcoal py-10 text-center">
              ...
              <Button variant="ghost" size="sm" className="mt-3" onClick={() => setShowCreateTask(true)}>
                Create first task
              </Button>
            </div>
          ) : (
            <div className="space-y-2">
              {teamMembers.map(([name, stats]) => (
                <div key={name}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-orika-cream">{name}</p>
                    <p className="text-xs text-orika-smoke">{stats.total} task{stats.total !== 1 ? 's' : ''}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    {stats.overdue > 0 && (
                      <Badge tone="danger" size="xs">{stats.overdue} overdue</Badge>
                    )}
                    {stats.urgent > 0 && (
                      <Badge tone="warn" size="xs">{stats.urgent} urgent</Badge>
                    )}
                    {stats.overdue === 0 && stats.urgent === 0 && (
                      <Badge tone="sage" size="xs">On track</Badge>
                    )}
                  </div>
                </div>
              ))}
              ...
              <div className="space-y-1.5">
                {overdueTasks.slice(0, 5).map((task) => (
                  <div key={task.task_id}
                    ...
                    <span className="flex-1 text-orika-cream truncate">{task.title}</span>
                    <span className="text-xs text-red-400 shrink-0">{fmtDate(task.due_at!)}</span>
                  </div>
                ))}
      ...
      {detailEvent && (
        <div className="fixed inset-0 z-20 flex items-center justify-center bg-black/40 p-4"
          ...
          <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-orika-charcoal p-5"
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        <p className="text-[0.65rem] uppercase tracking-widest text-orika-smoke">{label}</p>
      </div>
      <p className="font-display text-2xl font-light tabular-nums" style={{ color }}>
        {value}
      </p>
    </div>
  );
