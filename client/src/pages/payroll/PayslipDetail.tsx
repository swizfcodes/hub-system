...
import { EarningsRow, DeductionRow } from '@components/payroll/PayrollComponents';
...
export default function PayslipDetail() {
  ...
  const period = formatPeriod(payslip.period_month ?? 0, payslip.period_year ?? 0);
    ...
    <div className="px-4 sm:px-8 py-6 max-w-3xl mx-auto space-y-6">
      <PageHeader
        ...
        crumbs={[
          ...
          { label: payslip.run_number ?? 'Run', to: `/payroll/runs/${payslip.run_id}` },
          { label: 'Payslip' },
        ]}
        actions={
          <div className="flex gap-2">
            <a href={payslipPdfUrl(id!)} target="_blank" rel="noopener noreferrer">
      ...
      <div className="rounded-2xl border border-white/5 bg-orika-charcoal overflow-hidden">
        ...
        <div className="bg-orika-graphite/40 px-6 py-4 border-b border-white/5">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-display text-xl font-light text-orika-cream">{payslip.display_name}</p>
              <p className="text-sm text-orika-smoke">{payslip.job_title}</p>
            </div>
            <div className="text-right">
              <p className="text-xs uppercase tracking-widest text-orika-smoke">Period</p>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <div className="space-y-2">
              <EarningsRow label="Basic Salary"         value={payslip.basic_salary}        currency={currency} />
              {isFull && <>
                <EarningsRow label="Housing Allowance"   value={payslip.housing_allowance}   currency={currency} muted />
                <EarningsRow label="Transport Allowance" value={payslip.transport_allowance}  currency={currency} muted />
              </>}
              {payslip.commission_amount > 0 && (
                <EarningsRow label="Commission"          value={payslip.commission_amount}   currency={currency} />
              )}
              <div className="border-t border-white/10 pt-2">
                <EarningsRow label="Gross Salary"        value={payslip.gross_salary}        currency={currency} bold />
              </div>
            </div>
          </div>
          ...
          {isFull && (
            <div>
              ...
              <div className="space-y-2">
                <DeductionRow label="PAYE Tax"             value={payslip.paye_deduction}    currency={currency} highlight />
                <DeductionRow label="Pension (Employee 8%)" value={payslip.pension_employee} currency={currency} />
                <DeductionRow label="NHF (2.5% of Basic)"  value={payslip.nhf_deduction}    currency={currency} />
                {payslip.advance_recovery > 0 && (
                  <DeductionRow label="Advance Recovery"   value={payslip.advance_recovery}  currency={currency} />
                )}
                {payslip.other_deductions > 0 && (
                  <DeductionRow label={`Absence (${payslip.days_absent}d)`}
                    value={payslip.other_deductions} currency={currency} />
                )}
                <div className="border-t border-white/10 pt-2">
                  <DeductionRow label="Total Deductions"   value={payslip.total_deductions} currency={currency} highlight />
                </div>
              </div>
            </div>
          )}
          ...
          {isFull && payslip.pension_employer > 0 && (
            <div className="rounded-xl border border-white/5 bg-orika-graphite/20 p-4">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="tabular-nums text-orika-cloud">{fmtMoney(payslip.pension_employer, currency)}</span>
              </div>
            </div>
          )}
          ...
          <div className="rounded-2xl border border-green-500/30 bg-green-900/10 px-6 py-5 text-center">
            <p className="text-xs uppercase tracking-widest text-green-400 mb-1">Net Pay</p>
          ...
          {(payslip.bank_name || payslip.bank_account_number) && (
            <div className="text-sm space-y-1 border-t border-white/10 pt-4">
              <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke mb-2">Payment To</p>
              {payslip.bank_name && <div className="flex justify-between">
                <span className="text-orika-smoke">Bank</span>
                <span className="text-orika-cloud">{payslip.bank_name}</span>
              </div>}
              {payslip.bank_account_number && <div className="flex justify-between">
                <span className="text-orika-smoke">Account</span>
                <span className="font-mono text-orika-cloud">{payslip.bank_account_number}</span>
              </div>}
            </div>
          )}
        </div>
