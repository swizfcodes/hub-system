...
import { RunStatusBadge, PayrollModePicker, PaymentMethodPicker } from '@components/payroll/PayrollComponents';
...
const YEAR_OPTIONS: SelectOption[] = [currentYear - 1, currentYear, currentYear + 1].map(
      ...
      <div className="space-y-3">
        ...
        {isLoading ? (
          ...
          <div className="py-12 text-center rounded-2xl border border-white/5 bg-orika-charcoal">
            <p className="text-sm text-orika-smoke mb-3">No payroll runs yet.</p>
            ...
            <table className="w-full min-w-[600px] text-sm">
              <thead>
                <tr className="border-b border-white/5 bg-orika-charcoal">
                  {['Run #', 'Period', 'Headcount', 'Total Gross', 'Total Net', 'PAYE', 'Status', ''].map((h) => (
              ...
              <tbody className="divide-y divide-white/5">
                {runs.map((run) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-xs text-orika-gold">{run.run_number}</td>
                    <td className="px-4 py-3 font-medium text-orika-cream">
                      {formatPeriod(run.period_month, run.period_year)}
                    </td>
                    <td className="px-4 py-3 text-orika-smoke">{run.payslip_count ?? '—'}</td>
                    <td className="px-4 py-3 tabular-nums text-orika-cream">{fmtMoney(run.total_gross, currency)}</td>
                    <td className="px-4 py-3 tabular-nums text-green-400">{fmtMoney(run.total_net, currency)}</td>
                    <td className="px-4 py-3 tabular-nums text-orika-smoke">
                      {mode === 'full_paye' ? fmtMoney(run.total_paye, currency) : '—'}
                    </td>
                    <td className="px-4 py-3"><RunStatusBadge status={run.status} size="xs" /></td>
                    <td className="px-4 py-3"><ChevronRight className="h-4 w-4 text-orika-smoke" /></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <Modal
        ...
        footer={
          <div className="flex justify-end gap-3">
            <Button variant="ghost" onClick={() => setShowInitiate(false)} disabled={mutation.isPending}>Cancel</Button>
      ...
      >
        <div className="space-y-5">
          ...
          <div className="rounded-xl border px-4 py-3 text-sm"
            style={{
              borderColor: mode === 'full_paye' ? '#C9A86C40' : 'rgba(255,255,255,0.05)',
              backgroundColor: mode === 'full_paye' ? '#C9A86C08' : 'transparent',
            }}>
            <p className="font-medium" style={{ color: mode === 'full_paye' ? '#C9A86C' : '#9E9891' }}>
              {mode === 'full_paye' ? 'Full PAYE Mode' : 'Simplified Salary Mode'}
            </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Select label="Month *" options={MONTH_OPTIONS}
              value={month} onChange={(e) => setMonth(e.target.value)} surface="light" />
            <Select label="Year *" options={YEAR_OPTIONS}
              value={year} onChange={(e) => setYear(e.target.value)} surface="light" />
          </div>

          <PaymentMethodPicker value={paymentMethod} onChange={setPaymentMethod} />

          <p className="text-xs text-text-on-light-muted">
            The run will calculate payslips for all active staff. You can review
            before approving. Once approved, journals post to accounting automatically.
          </p>
        </div>
      </Modal>
    </div>
