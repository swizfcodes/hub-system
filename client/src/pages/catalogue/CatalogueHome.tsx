...
export default function CatalogueHome() {
  ...
  return (
    <>
      ...
      <CategoryFormModal open={creatingCategory || !!editingCategory} onClose={() => { setCreatingCategory(false); setEditingCategory(null); }} editing={editingCategory} />
    </>
  );
}
...
function ProductsTab({
  ...
  return (
    <>
          ...
          {(['cards', 'table'] as View[]).map((v) => (
            <button key={v} onClick={() => onViewChange(v)}
              className={cn(
                ...
                view === v ? 'bg-orika-graphite text-orika-cream' : 'text-orika-smoke hover:text-orika-cream',
              )}>
              {v === 'cards' ? <LayoutGrid className="w-3.5 h-3.5" /> : <Rows3 className="w-3.5 h-3.5" />}
      ...
      {isLoading ? (
        <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">{[0,1,2,3,4,5,6,7].map((i) => <Skeleton key={i} className="h-72" />)}</div>
      ) : products.length === 0 ? (
        <EmptyState
          icon={<Package className="w-7 h-7" />}
          title={search ? 'No matches' : 'Your catalogue is empty'}
          description={search ? 'Adjust your filters.' : 'Add your first product — it can be a physical jewellery piece, a fragrance, or a service.'}
          action={!search && <Button variant="gold" leftIcon={<Plus className="w-4 h-4" />} onClick={onNewProduct}>New product</Button>}
        />
      ) : view === 'cards' ? (
          ...
          <table className="w-full text-sm">
            <thead className="bg-orika-charcoal border-b border-orika-graphite">
              <tr>
                ...
                <Th className="text-right">Cost</Th><Th className="text-right">Selling</Th>
                <Th className="text-right">Reorder</Th><Th>Updated</Th><Th></Th>
              </tr>
            </thead>
            <tbody>
              {products.map((p) => (
                <tr key={p.product_id} onClick={() => navigate(`/catalogue/${p.product_id}`)}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="text-orika-cream truncate flex items-center gap-1.5">
                          {p.name}
                          {!p.is_active && <Badge tone="warn" size="xs">Inactive</Badge>}
                          {p.is_deleted && <Badge tone="danger" size="xs">Archived</Badge>}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-2.5 font-mono text-orika-smoke text-xs">{p.sku}</td>
                  <td className="px-4 py-2.5 text-orika-cloud text-xs">{p.category_name ?? '—'}</td>
                  <td className="px-4 py-2.5 text-right text-xs"><ProductPrice cost={null} selling={p.cost_price} currency={p.currency} hideCost /></td>
                  <td className="px-4 py-2.5 text-right text-xs"><ProductPrice cost={null} selling={p.selling_price} currency={p.currency} hideCost /></td>
                  <td className="px-4 py-2.5 text-right text-xs text-orika-cloud">{p.reorder_level}</td>
                  <td className="px-4 py-2.5 text-[0.65rem] text-orika-smoke">{fmtRelative(p.updated_at)}</td>
...
function Th({ children, className }: { children?: React.ReactNode; className?: string }) {
  return <th className={cn('px-4 py-2.5 text-left text-[0.6rem] tracking-widest uppercase text-orika-smoke font-semibold', className)}>{children}</th>;
}
...
function CategoriesTab({ onNew, onEdit }: { onNew: () => void; onEdit: (c: ProductCategory) => void }) {
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['catalogue', 'categories'] }); showToast.success('Category deleted'); },
  ...
  if (!cats || cats.length === 0) {
    return <EmptyState icon={<Tag className="w-6 h-6" />} title="No categories" description="Group products under categories like Rings, Necklaces, Reed Diffusers." action={<Button variant="gold" leftIcon={<Plus className="w-4 h-4" />} onClick={onNew}>New category</Button>} />;
  }

  return (
    <div className="space-y-2">
      {cats.map((c) => (
        <div key={c.category_id} className="rounded-xl border border-orika-graphite bg-orika-charcoal/60 p-4 flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg bg-orika-gold/15 text-orika-gold flex items-center justify-center"><Tag className="w-4 h-4" /></div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-sm font-medium text-orika-cream">{c.name}</span>
              {!c.is_active && <Badge tone="warn" size="xs">Inactive</Badge>}
            </div>
            {c.description && <p className="text-xs text-orika-smoke truncate">{c.description}</p>}
          </div>
          <DropdownMenu items={[
            { label: 'Edit',   onClick: () => onEdit(c) },
            { label: 'Delete', destructive: true, onClick: () => remove.mutate(c.category_id) },
          ]} />
        </div>
      ))}
    </div>
