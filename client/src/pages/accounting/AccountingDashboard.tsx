...
import { TrendingUp, TrendingDown, Banknote, AlertCircle, BookOpen, Scale, FileText, CreditCard } from 'lucide-react';
    ...
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal px-5 py-4">
          <p className="text-xs uppercase tracking-widest text-orika-smoke mb-2">Current Fiscal Period</p>
          {dash?.open_period ? (
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium text-orika-cream">{dash.open_period.name}</p>
                <p className="text-xs text-orika-smoke">
                  {fmtDate(dash.open_period.start_date)} — {fmtDate(dash.open_period.end_date)}
                </p>
              </div>
            ...
            <div className="flex items-center gap-2 text-amber-400">
              ...
              <p className="text-sm">No open fiscal period — journals cannot post</p>
            </div>
          )}
        </div>
        ...
        {/* Unreconciled items */}
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal px-5 py-4">
          <p className="text-xs uppercase tracking-widest text-orika-smoke mb-2">Bank Reconciliation</p>
          <div className="flex items-center justify-between">
            <div>
            ...
            {(dash?.unreconciled_count ?? 0) > 0 && (
              <Button variant="secondary" size="sm" onClick={() => navigate('/accounting/reconciliation')}>
                Reconcile
              </Button>
            )}
          </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
          {[
            ...
            { label: 'Journal Entries',   icon: FileText,  to: '/accounting/journals'      },
            { label: 'Financial Reports', icon: Scale,     to: '/accounting/reports'       },
            { label: 'Bank Reconciliation', icon: CreditCard, to: '/accounting/reconciliation' },
            { label: 'Fiscal Periods',    icon: AlertCircle, to: '/accounting/periods'     },
          ].map((item) => {
            ...
            return (
              <button key={item.to} onClick={() => navigate(item.to)}
                ...
                <p className="text-xs text-orika-cloud group-hover:text-orika-cream transition-colors">{item.label}</p>
              </button>
            );
          })}
        </div>
...
function KpiCard({ label, value, sub, icon: Icon, color }: {
  ...
  return (
    <div className="rounded-2xl border border-white/5 bg-orika-charcoal px-5 py-4">
      ...
      <p className="font-display text-xl font-light tabular-nums" style={{ color }}>{value}</p>
