    ...
    <>
      ...
      <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-5xl mx-auto">
        <PageHeader
          ...
          crumbs={[{ label: 'Hub', to: '/' }, { label: 'Procurement', to: '/procurement' }, { label: 'Suppliers' }]}
          actions={<Button variant="gold" leftIcon={<Plus className="w-4 h-4" />} onClick={() => setInviting(true)}>Add supplier</Button>}
        />
        ...
        {isLoading ? (
          ...
          <EmptyState
            ...
            description={search ? 'Adjust your search.' : 'Add your first supplier to start sending RFQs.'}
            action={!search && <Button variant="gold" leftIcon={<Plus className="w-4 h-4" />} onClick={() => setInviting(true)}>Add the first one</Button>}
          />
        ) : (
          <div className="space-y-2">{suppliers.map((s) => <SupplierRow key={s.supplier_id} supplier={s} />)}</div>
        )}
      </div>
