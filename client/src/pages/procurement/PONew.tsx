...
import { SupplierSearchInput, type SupplierOption } from '@components/procurement/suppliers/SupplierSearchInput';
...
export default function PONew() {
  ...
  useEffect(() => {
    ...
    if (selectedSupplier?.preferred_currency) setValue('currency', selectedSupplier.preferred_currency);
  }, [selectedSupplier, setValue]);

  const subtotal = useMemo(
    () => (linesWatch ?? []).reduce((s, l) => s + (l.quantity_ordered ?? 0) * (l.unit_price ?? 0), 0),
    [linesWatch]
  );
        ...
        <form onSubmit={handleSubmit((v) => mutation.mutate(v))} className="space-y-6">
          ...
          <Card className="p-5 sm:p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <Button type="button" size="sm" variant="secondary" leftIcon={<Plus className="w-3.5 h-3.5" />}
                onClick={() => append({ product_id: '', quantity_ordered: 1, unit_price: 0 })}>
                Add line
              </Button>
            </div>
              ...
              {fields.map((f, i) => (
                <div key={f.id} className="rounded-xl border border-orika-graphite bg-orika-black/30 p-3.5">
                  <div className="flex items-start gap-2">
                      ...
                      <div className="grid gap-3 sm:grid-cols-[2fr_auto_auto] items-end">
                        <div>
                          ...
                          <input
                            ...
                            onChange={(e) => setLineDescriptions((d) => ({ ...d, [i]: e.target.value }))}
                    ...
                    {fields.length > 1 && (
                      <button type="button" onClick={() => remove(i)} className="p-2 mt-7 text-orika-smoke hover:text-state-danger" aria-label="Remove">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    )}
                  </div>
                  {(linesWatch?.[i]?.quantity_ordered ?? 0) > 0 && (linesWatch?.[i]?.unit_price ?? 0) > 0 && (
          ...
          <Card className="p-5 sm:p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              <Select {...register('currency')} label="Currency" options={CURRENCIES.map((c) => ({ value: c.code, label: `${c.symbol} ${c.code}` }))} />
