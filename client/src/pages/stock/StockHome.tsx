...
export default function StockHome() {
  ...
  useEffect(() => { localStorage.setItem('orika_stock_view', view); }, [view]);
  ...
  const movementTypeOptions = useMemo(() => [
    ...
    ...Object.values(MOVEMENT_TYPE_META).map((m) => ({ value: m.key, label: `${m.direction === 1 ? '↓' : '↑'} ${m.label}` })),
  ], []);
    ...
    <>
      ...
      <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-7xl mx-auto">
        <PageHeader
          ...
          actions={
            <div className="flex flex-wrap gap-2">
              <Button variant="secondary" size="md" leftIcon={<BellRing className="w-4 h-4" />} onClick={() => navigate('/stock/alerts')}>Alerts</Button>
              <Button variant="secondary" size="md" leftIcon={<Truck className="w-4 h-4" />} onClick={() => navigate('/stock/transfers')}>Transfers</Button>
              <Button variant="secondary" size="md" leftIcon={<Lock className="w-4 h-4" />} onClick={() => navigate('/stock/reservations')}>Reservations</Button>
              <Button variant="gold" size="md" leftIcon={<ClipboardCheck className="w-4 h-4" />} onClick={() => navigate('/stock/count')}>New count</Button>
            </div>
          }
        />
        ...
        <StockKpiStrip
          ...
          pendingTransfers={(transfers?.data ?? []).filter((t) => t.status !== 'received' && t.status !== 'cancelled').length}
          pendingQC={(qcs?.data ?? []).filter((q) => q.result === 'conditional').length}
        />
        ...
        <Tabs
          variant="pill"
          tabs={[
            { key: 'product',   label: 'By Product',    icon: <LayoutGrid className="w-3.5 h-3.5" /> },
            { key: 'location',  label: 'By Location',   icon: <MapPin className="w-3.5 h-3.5" /> },
            { key: 'movements', label: 'Movement Log',  icon: <History className="w-3.5 h-3.5" /> },
          ]}
          active={view}
        ...
        <div className="mb-5 grid gap-3 sm:grid-cols-[1fr_auto_auto_auto] items-end">
          ...
          {view !== 'movements' && (
            <Select
              ...
              options={[{ value: '', label: 'All categories' }, ...categories.map((c) => ({ value: c.category_id, label: c.name }))]}
            />
          )}
          <Select
            surface="dark"
            ...
            options={[{ value: '', label: 'All locations' }, ...locations.map((l) => ({ value: l.location_id, label: l.name }))]}
          />
          {view !== 'movements' ? (
            <Checkbox surface="dark" checked={lowStockOnly} onChange={setLowStockOnly} label="Low stock only" />
          ) : (
            <Select
              surface="dark"
              value={movementType}
        ...
        {view === 'movements' && (
          <div className="mb-5 grid gap-3 sm:grid-cols-2 max-w-md">
            <Input surface="dark" type="date" label="From" value={movFrom} onChange={(e) => setMovFrom(e.target.value)} />
            <Input surface="dark" type="date" label="To"   value={movTo}   onChange={(e) => setMovTo(e.target.value)} />
          </div>
        )}

        {/* Views */}
        {view === 'product'  && <ByProductView rows={rows} loading={isLoading} onAdjust={setAdjustingProductId} />}
        {view === 'location' && <ByLocationView rows={rows} locations={locations} loading={isLoading} />}
        {view === 'movements' && <MovementLogView filters={{
