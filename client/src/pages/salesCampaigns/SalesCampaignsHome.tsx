...
import {
  Plus, ExternalLink, BarChart2, Share2, MoreVertical, Play, XCircle, Copy, Eye } from 'lucide-react';
...
export default function SalesCampaignsHome() {
  ...
  const [confirmModal, setConfirmModal] = useState<{ action: 'publish' | 'expire'; campaign: SalesCampaign } | null>(null);

  const { data, isLoading } = useQuery({
    queryKey: ['sales-campaigns', activeTab],
    queryFn:  () => listCampaigns(activeTab === 'all' ? undefined : { status: activeTab }),
  });
  ...
  const publishMutation = useMutation({
    ...
    onSuccess: (campaign) => {
      showToast.success(`Campaign is now ${campaign.status === 'scheduled' ? 'scheduled' : 'live'}!`);
  ...
  function copyLink(campaign: SalesCampaign) {
    ...
    navigator.clipboard.writeText(url).then(() => showToast.success('Link copied!'));
  }
  ...
  return (
    <>
        ...
        <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {campaigns.map(campaign => (
            <CampaignCard
              ...
              onMenuToggle={() => setMenuOpen(m => m === campaign.campaign_id ? null : campaign.campaign_id)}
              onEdit={() => navigate(`/sales-campaigns/${campaign.campaign_id}`)}
              onPublish={() => setConfirmModal({ action: 'publish', campaign })}
              onExpire={() => setConfirmModal({ action: 'expire', campaign })}
              onCopyLink={() => copyLink(campaign)}
              onViewOrders={() => navigate(`/sales-campaigns/${campaign.campaign_id}?tab=orders`)}
              onAnalytics={() => navigate(`/sales-campaigns/${campaign.campaign_id}?tab=analytics`)}
            />
          ))}
        </div>
      )}
      ...
      <Modal
        ...
        title={confirmModal?.action === 'publish' ? 'Publish Campaign' : 'Expire Campaign'}
        size="sm"
        footer={
          <div className="flex gap-2 justify-end">
            <Button variant="ghost" onClick={() => setConfirmModal(null)}>Cancel</Button>
            <Button
              variant={confirmModal?.action === 'expire' ? 'danger' : 'primary'}
              loading={publishMutation.isPending || expireMutation.isPending}
              onClick={() => {
                if (!confirmModal) return;
                if (confirmModal.action === 'publish') publishMutation.mutate(confirmModal.campaign.campaign_id);
                else expireMutation.mutate(confirmModal.campaign.campaign_id);
              }}
            >
              {confirmModal?.action === 'publish' ? 'Yes, Publish' : 'Yes, Expire'}
            </Button>
          </div>
        }
      >
...
function CampaignCard({ campaign, business, menuOpen, onMenuToggle, onEdit, onPublish, onExpire, onCopyLink, onViewOrders, onAnalytics }: CardProps) {
    ...
    >
      ...
      <div className="relative h-32 bg-orika-charcoal overflow-hidden">
        {campaign.hero_image_url
          ? <img src={campaign.hero_image_url} alt="" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
        ...
        <div className="absolute top-3 left-3">
          <Badge tone={STATUS_BADGE[campaign.status] ?? 'neutral'} size="xs" dot>
      ...
      <div className="p-4 space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="font-semibold text-orika-cream truncate">{campaign.campaign_name}</p>
            <p className="text-xs text-orika-smoke mt-0.5 truncate">/{campaign.slug}</p>
          </div>
          {/* Menu */}
          <div className="relative shrink-0" onClick={e => e.stopPropagation()}>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-8 z-20 min-w-[160px] rounded-xl border border-white/10 bg-orika-charcoal shadow-2xl py-1">
                ...
                <MenuItem icon={<Copy className="h-3.5 w-3.5" />} label="Copy link" onClick={onCopyLink} />
                <MenuItem icon={<Eye className="h-3.5 w-3.5" />} label="View orders" onClick={onViewOrders} />
                <MenuItem icon={<BarChart2 className="h-3.5 w-3.5" />} label="Analytics" onClick={onAnalytics} />
                <a
        ...
        <div className="grid grid-cols-3 gap-2 pt-1 border-t border-white/5">
          ...
          <Stat label="Revenue" value={fmtMoney(campaign.confirmed_revenue ?? 0)} />
        ...
        {campaign.is_evergreen && <p className="text-xs text-orika-gold/60">Evergreen — no expiry</p>}
      </div>
    </div>
  );
}
...
function Stat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="text-center">
      ...
      <p className="text-[10px] text-orika-smoke/60 uppercase tracking-wide">{label}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center">
      ...
      <p className="text-sm text-orika-smoke max-w-sm mb-6">
        Create a shareable campaign page in minutes. Add products, set a deadline, and share the link.
      </p>
