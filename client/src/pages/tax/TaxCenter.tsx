...
import {
  Calculator, Download, CheckCircle2, Send, Banknote, AlertTriangle, FileText,
} from 'lucide-react';
...
import {
  getTaxDashboard, previewTax, saveFilingDraft, confirmFiling, markFiled,
  settleFiling, exportFiling,
  type TaxType, type TaxWorkpaper, type TaxFiling,
...
export default function TaxCenter() {
  ...
  const { data: dash } = useQuery({ queryKey: ['tax-dashboard'], queryFn: getTaxDashboard });
  ...
  function switchTax(t: TaxType) {
    ...
    setPeriod(c === 'year' ? String(new Date().getUTCFullYear()) : priorMonth());
  }

  const verify = useMutation({
    mutationFn: () => previewTax(taxType, period),
    onSuccess: (data) => { setWp(data); setFiling(null); },
    onError: (e: any) => showToast.error(e?.response?.data?.message ?? 'Could not compute'),
  });
  ...
  const act = useMutation({
    mutationFn: async (kind: 'confirm' | 'file' | 'settle') => {
      ...
      if (kind === 'file') {
        const ref = window.prompt('NRS/LIRS filing reference (optional):') ?? undefined;
        return markFiled(filing.filing_id, ref);
      }
      const bank = window.prompt('Bank account code to pay from:', '1210') ?? '1210';
    ...
    onError: (e: any) => showToast.error(e?.response?.data?.message ?? 'Action failed'),
  });
    ...
    <div className="px-4 sm:px-8 py-6 max-w-6xl mx-auto space-y-6">
        ...
        actions={
          profile && (
            <span className={cn('text-xs px-3 py-1 rounded-full',
              profile.is_small_company ? 'bg-green-500/15 text-green-300' : 'bg-white/10 text-orika-cloud')}>
              {profile.is_small_company ? 'Small company — CIT exempt' : 'Standard company'}
            </span>
          )
        }
      />
      ...
      {dash?.deadlines?.length ? (
        <div className="grid sm:grid-cols-3 gap-3">
          {dash.deadlines.map((d) => (
            <div key={d.tax_type} className="rounded-2xl border border-white/8 bg-orika-graphite px-4 py-3">
              <p className="text-xs text-orika-smoke">{d.tax_type} · {d.period_label}</p>
              <p className="text-sm font-semibold text-orika-cream mt-0.5">Due {d.due}</p>
      ...
      <div className="rounded-2xl border border-white/8 bg-orika-graphite p-5 space-y-4">
          ...
          {TAX_TABS.map((t) => (
            <button
              ...
              className={cn('px-3 py-1.5 rounded-xl text-xs font-medium transition-colors',
                taxType === t.type ? 'bg-orika-gold/15 text-orika-gold border border-orika-gold/30'
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <Button variant="primary" loading={verify.isPending}
            leftIcon={<Calculator className="h-4 w-4" />} onClick={() => verify.mutate()}>
            Verify figure
          </Button>
          {wp && !filing && (
            <Button variant="secondary" loading={save.isPending}
              leftIcon={<FileText className="h-4 w-4" />} onClick={() => save.mutate()}>
              Save as draft
            </Button>
          )}
        </div>
      ...
      {wp && (
        <div className="rounded-2xl border border-white/8 bg-orika-graphite p-5 space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <p className="text-xs text-orika-smoke uppercase tracking-wide">{wp.tax_type} · {wp.period_label}</p>
              <p className="text-3xl font-bold text-orika-cream mt-1">{fmtMoney(filing?.final_amount ?? wp.computed_amount)}</p>
              {wp.meta?.deadline ? <p className="text-xs text-orika-smoke mt-1">Due: {String(wp.meta.deadline)}</p> : null}
            </div>
            {filing && (
              <span className={cn('text-xs px-3 py-1 rounded-full capitalize', STATUS_TONE[filing.status] ?? 'bg-white/10')}>
                {filing.status}
              </span>
            )}
          </div>
          ...
          {wp.warnings?.length > 0 && (
            <div className="space-y-2">
              {wp.warnings.map((w, i) => (
                <div key={i} className="flex gap-2 rounded-xl border border-amber-400/30 bg-amber-400/10 px-3 py-2">
          ...
          <div className="grid sm:grid-cols-2 gap-x-8 gap-y-1.5">
            {Object.entries(wp.summary || {}).filter(([, v]) => v !== null && v !== undefined).map(([k, v]) => (
              <div key={k} className="flex justify-between border-b border-white/5 py-1.5">
                <span className="text-xs text-orika-smoke capitalize">{k.replace(/_/g, ' ')}</span>
                <span className="text-sm text-orika-cream text-right">{fmtSummary(v)}</span>
              </div>
            ))}
          </div>
                ...
                <thead className="bg-white/5 text-orika-smoke">
                  <tr>
                    {(wp.tax_type === 'CIT'
                      ...
                      : ['Date', 'Entry', 'Description', 'Account', 'Debit', 'Credit']
                    ).map((h) => <th key={h} className="text-left font-medium px-3 py-2">{h}</th>)}
                  </tr>
                </thead>
                    ...
                    <tr key={i} className="border-t border-white/5">
                      {wp.tax_type === 'CIT' ? (
                        <>
                          ...
                          <td className="px-3 py-1.5 text-right">{fmtMoney(l.amount)}</td>
                        </>
                      ) : (
                        <>
                          ...
                          <td className="px-3 py-1.5 text-right">{l.debit ? fmtMoney(l.debit) : ''}</td>
                          <td className="px-3 py-1.5 text-right">{l.credit ? fmtMoney(l.credit) : ''}</td>
                        </>
                      )}
                    </tr>
                  ))}
          ...
          {filing && (
            <div className="flex flex-wrap gap-2 pt-1">
              <Button size="sm" variant="ghost" leftIcon={<Download className="h-3.5 w-3.5" />}
                onClick={() => exportFiling(filing.filing_id, `${filing.tax_type}-${filing.period_label}.csv`)}>
                Export CSV
              </Button>
              {filing.status === 'draft' && (
                <Button size="sm" variant="secondary" loading={act.isPending}
                  leftIcon={<CheckCircle2 className="h-3.5 w-3.5" />} onClick={() => act.mutate('confirm')}>
                  Confirm reviewed
                </Button>
              )}
              {['draft', 'reviewed', 'exempt'].includes(filing.status) && (
                <Button size="sm" variant="secondary" loading={act.isPending}
                  leftIcon={<Send className="h-3.5 w-3.5" />} onClick={() => act.mutate('file')}>
                  Mark filed
                </Button>
              )}
              {['reviewed', 'filed'].includes(filing.status) && filing.tax_type !== 'CIT' && (
                <Button size="sm" variant="gold" loading={act.isPending}
                  leftIcon={<Banknote className="h-3.5 w-3.5" />} onClick={() => act.mutate('settle')}>
      ...
      {dash?.filings?.length ? (
        <div className="rounded-2xl border border-white/8 bg-orika-graphite p-5">
          <p className="text-sm font-semibold text-orika-cream mb-3">Recent returns</p>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="text-orika-smoke">
                <tr>
                  {['Type', 'Period', 'Amount', 'Status', 'Reference'].map((h) =>
              ...
              <tbody className="text-orika-cloud">
                {dash.filings.map((f) => (
                  <tr key={f.filing_id} className="border-t border-white/5">
                    <td className="px-3 py-2 font-medium text-orika-cream">{f.tax_type}</td>
                    <td className="px-3 py-2">{f.period_label}</td>
                    <td className="px-3 py-2">{fmtMoney(f.final_amount)}</td>
                    <td className="px-3 py-2">
                      <span className={cn('px-2 py-0.5 rounded-full capitalize', STATUS_TONE[f.status] ?? 'bg-white/10')}>{f.status}</span>
                    </td>
                    <td className="px-3 py-2 text-orika-smoke">{f.filing_reference ?? '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
