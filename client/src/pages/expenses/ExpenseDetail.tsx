...
import { CheckCircle, XCircle, DollarSign, FileText, Receipt } from 'lucide-react';
...
import {
  getExpense, approveExpense, markExpensePaid,
} from '@services/expenses';
import {
  CATEGORY_OPTIONS, EXPENSE_TYPE_LABEL,
  EXPENSE_TYPE_CR_ACCOUNT, CATEGORY_COA,
  ...
  if (!expense) {
    return (
      <div className="px-8 py-16 text-center">
        ...
        <Button variant="ghost" className="mt-4" onClick={() => navigate('/expenses')}>Back</Button>
      </div>
    );
  }

  const categoryLabel = CATEGORY_OPTIONS.find((c) => c.value === expense.category)?.label ?? expense.category;
    ...
    <div className="px-4 sm:px-8 py-6 max-w-3xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-6 space-y-3">
          <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke mb-4">Details</p>

          <DetailRow label="Category"   value={categoryLabel} />
          <DetailRow label="Type"       value={EXPENSE_TYPE_LABEL[expense.expense_type]} />
          <DetailRow label="Amount"     value={fmtMoney(expense.amount, currency)} bold />
          ...
          {expense.approved_at && (
            <DetailRow label="Approved" value={fmtDateTime(expense.approved_at)} />
          )}
          {expense.paid_at && (
            <DetailRow label="Paid" value={fmtDateTime(expense.paid_at)} highlight="success" />
          )}
        </div>
        ...
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-6 space-y-4">
          ...
          <div className="space-y-2 font-mono text-xs">
            <div className="flex items-center justify-between rounded-lg bg-orika-graphite/30 px-3 py-2.5">
              ...
              <span className="text-orika-cream">{fmtMoney(expense.amount, currency)}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-orika-graphite/30 px-3 py-2.5">
          ...
          {/* Reimbursement note */}
          {expense.expense_type === 'reimbursement' && expense.status !== 'paid' && (
            <div className="rounded-lg border border-orika-gold/20 bg-orika-gold/5 px-3 py-2 text-xs text-orika-gold/80">
              Reimbursement — staff is owed {fmtMoney(expense.amount, currency)}.
              Mark paid once the transfer is made.
            </div>
          )}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-6">
        <div className="flex items-center justify-between mb-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke">Receipts</p>
          ...
          <div className="space-y-2">
            {expense.receipts.map((r) => (
              <div key={r.receipt_id}
                ...
                <span className="text-sm text-orika-cloud">Receipt attached</span>
                <span className="ml-auto text-xs text-orika-smoke">{fmtDate(r.uploaded_at)}</span>
              </div>
            ))}
          </div>
        ) : (
...
function DetailRow({ label, value, bold = false, highlight }: {
  label: string; value: string; bold?: boolean; highlight?: 'success';
}) {
  return (
