...
export default function CrmHome() {
  ...
  const [view, setView] = useState<View>(
    () =>
      (localStorage.getItem("orika_crm_view") as View) ||
  ...
  useEffect(() => {
    localStorage.setItem("orika_crm_view", view);
  }, [view]);
  useEffect(() => {
    ...
    <>
      ...
      <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-[1500px] mx-auto">
        ...
        <div className="animate-fade-in">
          {view === "table" && (
            <PipelineTable pipeline={data?.pipeline} loading={isLoading} />
          )}
          {view === "board" && (
            <PipelineBoard
          ...
          {view === "calendar" && (
            <PipelineCalendar pipeline={data?.pipeline} loading={isLoading} />
          )}
          {view === "forecast" && (
            <PipelineForecast pipeline={data?.pipeline} loading={isLoading} />
          )}
        </div>
      </div>
