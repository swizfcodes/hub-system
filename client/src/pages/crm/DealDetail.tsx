    ...
    <>
      ...
      <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-7xl mx-auto">
        <div className="mb-5 flex items-center justify-between gap-3 flex-wrap">
          ...
          <Button variant="ghost" size="sm" leftIcon={<ArrowLeft className="w-4 h-4" />} onClick={() => navigate('/crm')}>Back to pipeline</Button>
        </div>
        ...
        {isLoading ? (
          ...
          <EmptyState icon={<TrendingUp className="w-7 h-7" />} title="Deal not found" description="It may have been archived." />
        ) : (
          <>
            <header className="mb-6">
              <h1 className="font-display font-light text-3xl sm:text-4xl text-orika-cream">{deal.title}</h1>
              {deal.contact_name && <p className="text-sm text-orika-smoke mt-1">for {deal.contact_name}</p>}
            </header>
