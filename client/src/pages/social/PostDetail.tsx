...
import { Send, XCircle, Edit2, ExternalLink, BarChart2, MessageCircle } from 'lucide-react';
  ...
  if (!post) {
    return (
      <div className="px-8 py-16 text-center">
        ...
        <Button variant="ghost" className="mt-4" onClick={() => navigate('/social')}>Back</Button>
      </div>
    );
  }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            {post.channels.map((c) => <ChannelChip key={c} channel={c} />)}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            {post.media_paths?.length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke">Media</p>
            ...
            {post.caption && (
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke">Caption</p>
                <p className="text-sm text-orika-cream whitespace-pre-wrap leading-relaxed">{post.caption}</p>
              </div>
            )}
            ...
            {(post.title || post.description) && (
              <div className="rounded-xl border border-[#69C9D0]/20 bg-[#69C9D0]/5 p-4 space-y-2">
                <p className="text-xs font-semibold text-[#69C9D0]">🎵 TikTok</p>
                {post.title && <p className="text-sm font-medium text-orika-cream">{post.title}</p>}
                {post.description && <p className="text-xs text-orika-smoke">{post.description}</p>}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <InfoCard label="Scheduled"  value={post.scheduled_at ? fmtDateTime(post.scheduled_at) : '—'} />
              <InfoCard label="Published"  value={post.published_at ? fmtDateTime(post.published_at) : '—'} />
            </div>

            {/* Per-channel results */}
            {Object.keys(post.external_ids || {}).length > 0 && (
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-widest text-orika-smoke">Channel Results</p>
                  ...
                  return (
                    <div key={channel}
                      ...
                      <div className="flex-1">
                        <p className="text-xs font-medium text-orika-cream">{meta.label}</p>
                        {result.postId && (
                          <p className="text-[10px] text-orika-smoke font-mono">{result.postId}</p>
                        )}
                        {result.error && (
                      ...
                      {result.status === 'published' && result.postId && (
                        <a
                          href={
                            channel === 'instagram' ? `https://www.instagram.com/p/${result.postId}/`
                            : channel === 'facebook' ? `https://www.facebook.com/${result.postId}`
                            : channel === 'tiktok'  ? `https://www.tiktok.com/@yourhandle/video/${result.postId}`
                            : '#'
                          }
                      ...
                      <Badge tone={result.status === 'published' ? 'sage' : 'danger'} size="xs">
                        {result.status}
                      </Badge>
                    </div>
                  );
      ...
      {activeTab === 'metrics' && (
        !isPublished ? (
          <div className="py-12 text-center rounded-2xl border border-white/5 bg-orika-charcoal">
            ...
            <p className="text-sm text-orika-smoke">Analytics appear after the post is published.</p>
          </div>
        ) : (
      ...
      {activeTab === 'comments' && (
        !isPublished ? (
          <div className="py-12 text-center rounded-2xl border border-white/5 bg-orika-charcoal">
            ...
            <p className="text-sm text-orika-smoke">Comments appear after the post is published.</p>
          </div>
        ) : (
