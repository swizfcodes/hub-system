...
import { Plus, Calendar, List, ChevronLeft, ChevronRight, Image as ImageIcon } from 'lucide-react';
import { PageHeader } from '@components/ui/PageHeader';
import { Button }     from '@components/ui/Button';
import { Skeleton }   from '@components/ui/Skeleton';
import { PostStatusBadge, ChannelChip } from '@components/social/SocialComponents';
import { PostComposer }                 from '@components/social/PostComposer';
import { listPosts } from '@services/social';
import {
  CHANNEL_META, POST_STATUS_META, ALL_CHANNELS,
  generateMonthGrid, isSameDay, isToday,
...
export default function SocialHome() {
  ...
  const posts = allPosts.filter((p) =>
    channelFilter === 'all' || p.channels.includes(channelFilter)
  );

  // KPI strip
  const published = allPosts.filter((p) => p.status === 'published' || p.status === 'partial').length;
  ...
  const postsOnDay = (day: Date) => posts.filter((p) => {
    ...
    <>
      ...
      <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
        <PageHeader
          ...
          actions={
            <Button onClick={() => { setComposerDate(undefined); setShowComposer(true); }}>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            { label: 'Failed',     value: failed,    color: failed > 0 ? '#EF4444' : '#9E9891' },
          ].map((kpi) => (
        ...
        <div className="flex flex-wrap items-center gap-3">
            ...
            {STATUS_FILTERS.map((f) => (
              <button key={f.key} onClick={() => setStatusFilter(f.key)}
                className={cn('rounded-lg px-2.5 py-1 text-xs font-medium transition-colors',
                  statusFilter === f.key ? 'bg-orika-gold text-orika-black' : 'text-orika-smoke hover:text-orika-cream')}>
                {f.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-1.5">
            <button onClick={() => setChannelFilter('all')}
              className={cn('rounded-full px-3 py-1 text-xs font-medium border transition-all',
                channelFilter === 'all' ? 'border-white/30 text-orika-cream bg-orika-graphite' : 'border-white/10 text-orika-smoke hover:border-white/20')}>
              All
            </button>
            {ALL_CHANNELS.map((ch) => {
              ...
              return (
                <button key={ch} onClick={() => setChannelFilter(ch)}
                  className={cn('rounded-full px-3 py-1 text-xs font-medium border transition-all',
                    channelFilter === ch ? 'border-2' : 'border border-white/10 text-orika-smoke hover:border-white/20')}
                  style={channelFilter === ch ? { borderColor: meta.color, color: meta.color, backgroundColor: meta.bg } : {}}>
          ...
          <div className="ml-auto flex items-center gap-1 rounded-xl border border-white/5 bg-orika-charcoal p-1">
            {([{ key: 'calendar', icon: Calendar }, { key: 'list', icon: List }] as const).map(({ key, icon: Icon }) => (
              <button key={key} onClick={() => setView(key)}
                className={cn('rounded-lg p-1.5 transition-colors',
                  view === key ? 'bg-orika-gold text-orika-black' : 'text-orika-smoke hover:text-orika-cream')}>
        ...
        {isLoading ? (
          ...
          <div className={cn('flex gap-4', selectedDay ? 'flex-col lg:flex-row' : '')}>
            {/* Calendar grid */}
            <div className="flex-1">
              {/* Month nav */}
              <div className="flex items-center gap-3 mb-3">
                <button onClick={() => navigate2(-1)}
              ...
              <div className="rounded-2xl border border-white/5 bg-orika-charcoal overflow-hidden">
                <div className="grid grid-cols-7 border-b border-white/5">
                  {DAY_NAMES.map((d) => (
                    <div key={d} className="py-2 text-center text-[0.6rem] font-semibold uppercase tracking-widest text-orika-smoke">{d}</div>
                  ))}
                </div>
                <div className="divide-y divide-white/5">
                  {generateMonthGrid(currentDate.getFullYear(), currentDate.getMonth()).map((week, wi) => (
                    <div key={wi} className="grid grid-cols-7 divide-x divide-white/5" style={{ minHeight: 80 }}>
                      {week.map((day, di) => {
                        ...
                        return (
                          <div key={di}
                            ...
                            className={cn(
                              ...
                              inMonth ? 'bg-transparent' : 'bg-orika-graphite/10',
                            ...
                            <div className={cn(
                              ...
                              !inMonth ? 'text-orika-smoke/30' : 'text-orika-cloud',
                            )}>
                              {day.getDate()}
                            </div>
                            <div className="space-y-0.5">
                              {dayPosts.slice(0, 3).map((post) => {
                                const statusColor = POST_STATUS_META[post.status]?.color ?? '#9E9891';
                                return (
                                  <button key={post.post_id}
                                    onClick={(e) => { e.stopPropagation(); navigate(`/social/${post.post_id}`); }}
                                    className="w-full truncate rounded px-1 py-0.5 text-left text-[0.6rem] font-medium leading-tight"
                                    style={{ backgroundColor: `${statusColor}18`, color: statusColor, borderLeft: `2px solid ${statusColor}` }}>
                                    {post.channels.map((c) => CHANNEL_META[c]?.icon).join('')} {post.caption?.slice(0, 20) ?? 'Post'}
                                  </button>
                                );
                              })}
                              {dayPosts.length > 3 && (
                                <p className="text-[9px] text-orika-smoke pl-1">+{dayPosts.length - 3} more</p>
                              )}
                            </div>
                          </div>
                        );
                ...
                <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-4 sticky top-6 space-y-3">
                  ...
                  {selectedDayPosts.length === 0 ? (
                    ...
                    selectedDayPosts.map((p) => (
                      <button key={p.post_id} onClick={() => navigate(`/social/${p.post_id}`)}
                        ...
                        <div className="flex gap-1 mb-1">
                          {p.channels.map((c) => <ChannelChip key={c} channel={c} size="xs" />)}
                        </div>
                        <p className="text-xs text-orika-cream truncate">{p.caption?.slice(0,40) ?? '—'}</p>
            ...
            <table className="w-full min-w-[640px] text-sm">
              <thead>
                <tr className="border-b border-white/5 bg-orika-charcoal">
                  {['Preview','Channels','Caption','Status','Date',''].map((h) => (
              ...
              <tbody className="divide-y divide-white/5">
                {posts.length === 0 && (
                  <tr><td colSpan={6} className="px-4 py-12 text-center text-sm text-orika-smoke">
                    No posts match your filters.
                  </td></tr>
                )}
                {posts.map((post) => (
                  <tr key={post.post_id} className="bg-orika-charcoal hover:bg-orika-graphite/20 transition-colors">
                    <td className="px-4 py-3">
                      {post.media_paths?.[0] ? (
                        <img src={post.media_paths[0]} alt=""
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {post.channels.map((c) => <ChannelChip key={c} channel={c} size="xs" />)}
                      </div>
                    </td>
                    ...
                    <td className="px-4 py-3"><PostStatusBadge status={post.status} size="xs" /></td>
                    <td className="px-4 py-3 text-orika-smoke text-xs">
                      {fmtDate(post.published_at ?? post.scheduled_at ?? post.created_at)}
                    </td>
                    <td className="px-4 py-3">
                      <button onClick={() => navigate(`/social/${post.post_id}`)}
        ...
        <PostComposer
          open={showComposer}
          onClose={() => { setShowComposer(false); setComposerDate(undefined); }}
