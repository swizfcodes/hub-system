...
export default function POSTerminals() {
  ...
  const { data: locationsData } = useQuery({
    ...
    queryFn:  () => api.get<{ data: Array<{ location_id: string; name: string }> }>('/catalogue/locations').then(r => r.data),
    staleTime: 5 * 60_000,
  });
  ...
  const createMutation = useMutation({
    mutationFn: () => createTerminal({ name: newName.trim(), location_id: locationId }),
    onSuccess: (terminal) => {
      qc.invalidateQueries({ queryKey: ['pos-terminals'] });
      showToast.success(`Terminal "${terminal.name}" created`);
      setShowCreate(false); setNewName(''); setLocationId('');
    },
    onError: (err) => showToast.error(errMsg(err)),
  });
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 bg-orika-charcoal/40 py-20 text-center">
          ...
          <p className="mt-1 text-sm text-orika-smoke">Create your first terminal to start taking sales.</p>
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <p className="flex items-center gap-1 text-xs text-orika-smoke"><MapPin className="h-3 w-3" />{terminal.location_name}</p>
                </div>
      ...
      <Modal
        open={showCreate}
        onClose={() => { setShowCreate(false); setNewName(''); setLocationId(''); }}
        title="New Terminal"
        size="sm"
        surface="light"
        footer={
          ...
          <div>
            ...
            {locations.length === 0 ? (
              <p className="rounded-lg border border-amber-500/30 bg-amber-900/10 px-3 py-2 text-xs text-amber-300">
                No stock locations found. Please create a location in Stock settings first.
              </p>
            ) : (
              <select
                value={locationId}
