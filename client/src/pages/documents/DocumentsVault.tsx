...
import {
  Upload, Download, ShieldCheck, ShieldAlert, Trash2,
  FileText, File, Image, X, Plus, ExternalLink,
  ChevronLeft, ChevronRight, Search, Copy, Check,
} from 'lucide-react';
...
import {
  listDocuments, verifyDocument, downloadDocument,
...
const DOC_TYPE_BADGE_TONE: Record<string, 'gold' | 'sage' | 'info' | 'plum' | 'neutral' | 'warn'> = {
  invoice:                 'gold',
  credit_note:             'warn',
  quotation:               'gold',
  receipt:                 'sage',
...
export default function DocumentsVault() {
  ...
  useEffect(() => { setPage(1); }, [activeTab, debSearch]);
                  ...
                  {documents.map(doc => (
                    <DocumentRow
                      ...
                      onClick={() => setSelectedId(
                        selectedId === doc.document_id ? null : doc.document_id
                      )}
                      onDownload={async () => {
                        try {
                          const { verified } = await downloadDocument(doc);
                          if (!verified) showToast.error('Warning: File integrity check failed');
                        } catch {
...
function DocumentRow({
  doc, isSelected, onClick, onDownload,
}: {
  doc:        HubDocument;
  isSelected: boolean;
  onClick:    () => void;
...
function DocumentDrawer({
  doc, onClose, onDeleted, onTagsChanged,
}: {
  ...
  useEffect(() => { setVerifyResult(null); }, [docId]);
  ...
  async function handleDownload() {
    ...
    try {
      ...
      if (!verified) {
        showToast.error('Download complete — but integrity check failed. The file may have been altered.');
      }
    } catch {
      showToast.error('Download failed');
    } finally {
  ...
  const referenceUrl = d.reference_type && d.reference_id
    ...
    <DrawerShell onClose={onClose}>
      ...
      <div className="p-5 border-b border-white/8">
        <div className="flex items-start gap-3">
          <MimeIcon mime={d.mime_type} className="h-5 w-5 text-orika-gold mt-0.5 shrink-0" />
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        ...
        <section className="space-y-2">
          ...
          <MetaRow label="Business">
            <span className="text-sm text-orika-cream capitalize">{d.business}</span>
          </MetaRow>
          <MetaRow label="Size">
            <span className="text-sm text-orika-cream">{fmtFileSize(d.file_size_bytes)}</span>
          </MetaRow>
          <MetaRow label="Format">
            <span className="text-sm text-orika-cream">{d.mime_type}</span>
          </MetaRow>
          <MetaRow label="Uploaded by">
            <span className="text-sm text-orika-cream">{d.uploaded_by_name ?? '—'}</span>
          </MetaRow>
          <MetaRow label="Date">
            <span className="text-sm text-orika-cream">{fmtDate(d.created_at)}</span>
          </MetaRow>
        </section>
          ...
          <div className="flex flex-wrap gap-1.5 mb-2">
            {d.tags.map(tag => (
              <span
                ...
                style={{ backgroundColor: tag.colour + '30', border: `1px solid ${tag.colour}50` }}
              >
          ...
          <section>
            ...
            >
              <span className="capitalize">
                {REFERENCE_TYPE_OPTIONS.find(r => r.value === d.reference_type)?.label ?? d.reference_type}
              </span>
        ...
        {!confirmDelete ? (
          ...
          <div className="rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 space-y-2">
            <p className="text-xs text-red-300">
              This permanently removes the document from the vault. The audit log is preserved. Are you sure?
            </p>
            <div className="flex gap-2">
              <Button
                variant="ghost" size="sm"
...
function UploadModal({
  open, business, onClose, onUploaded,
}: {
  open:       boolean;
  business:   string;
  onClose:    () => void;
  ...
  useEffect(() => {
    if (open) {
      setFile(null); setDocType(''); setTitle('');
      setRefType(''); setRefId(''); setTagInput('');
      setError(null);
    }
  }, [open]);
      ...
      <div className="space-y-4">
        ...
        >
          {file ? (
            <div className="flex items-center justify-center gap-3">
              <MimeIcon mime={file.type || 'application/pdf'} className="h-6 w-6 text-orika-gold" />
              <div className="text-left">
                <p className="text-sm font-medium text-orika-cream">{file.name}</p>
                <p className="text-xs text-orika-smoke">{fmtFileSize(file.size)}</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <p className="text-sm text-orika-smoke">
                Drag & drop a file here, or <span className="text-orika-gold">click to browse</span>
              </p>
              <p className="text-xs text-orika-smoke/50 mt-1">Max 25 MB · PDF, images, or any file type</p>
            </>
          )}
          <input
            ref={fileRef}
        ...
        <div>
          <p className="text-xs font-medium text-orika-smoke mb-2">
            Link to record <span className="text-orika-smoke/50">(optional)</span>
          </p>
          <div className="grid sm:grid-cols-2 gap-3">
            <Select
              surface="dark"
...
function Th({ children, className }: { children?: React.ReactNode; className?: string }) {
  return (
    <th className={cn('pb-3 text-xs font-medium text-orika-smoke/60 uppercase tracking-wide', className)}>
      {children}
    </th>
  );
}
