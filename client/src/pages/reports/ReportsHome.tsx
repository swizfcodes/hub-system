    ...
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-8">
            ...
            {pinned.map((r) => {
              ...
              return (
                <button key={r.report_id}
                  onClick={() => navigate(`/reports/${family}/${type}?saved=${r.report_id}`)}
                  ...
                  <p className="text-sm font-semibold text-orika-cream">{r.report_name}</p>
                  {r.last_run_at && (
                    <p className="text-[10px] text-orika-smoke">Last run {fmtDate(r.last_run_at)}</p>
                  )}
                </button>
              );
            })}
          ...
          {Object.entries(REPORT_FAMILIES).map(([key, family]) => (
            <div key={key}
              ...
              >
                ...
                <div>
                  <p className="font-semibold text-orika-cream">{family.label}</p>
                  <p className="text-[10px] text-orika-smoke uppercase tracking-widest">
                    {family.types.length} report{(family.types.length as number) !== 1 ? 's' : ''}
                  </p>
                </div>
              </div>
      ...
      <div className="flex items-center justify-between rounded-2xl border border-white/5 bg-orika-charcoal px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-orika-cream">Saved Reports</p>
            <p className="text-xs text-orika-smoke">{savedList.length} saved · includes scheduled deliveries</p>
          </div>
        </div>
