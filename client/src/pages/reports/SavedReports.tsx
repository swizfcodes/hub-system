...
import {
  Play, Trash2, Edit2, Clock, Share2, Lock, Calendar,
  Mail, MessageSquare, Plus, BarChart2,
} from 'lucide-react';
...
import {
  listSavedReports, deleteSavedReport, updateSavedReport,
...
export default function SavedReports() {
  ...
  const [schedule,      setSchedule]      = useState<Partial<ScheduleConfig>>({
    frequency: 'weekly', day_of_week: 1, channels: ['email'], recipients: [],
  });
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, values }: { id: string; values: Parameters<typeof updateSavedReport>[1] }) =>
  ...
  function openSchedule(r: SavedReport) {
    setSchedule(r.schedule ?? { frequency: 'weekly', day_of_week: 1, channels: ['email'], recipients: [] });
    setScheduleFor(r);
  }
  ...
  function removeRecipient(i: number) {
    setSchedule((s) => ({ ...s, recipients: s.recipients?.filter((_, idx) => idx !== i) }));
  }

  function toggleChannel(ch: 'email' | 'whatsapp') {
    setSchedule((s) => {
      const cur = s.channels ?? [];
      return { ...s, channels: cur.includes(ch) ? cur.filter((c) => c !== ch) : [...cur, ch] };
    });
  }
    ...
    <div className="px-4 sm:px-8 py-6 max-w-4xl mx-auto space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal py-16 text-center">
          ...
          <p className="text-xs text-orika-smoke/50 mt-1">Run a report and click "Save" to save it here.</p>
          ...
          {reports.map((r) => {
            ...
            return (
              <div key={r.report_id}
                ...
                {/* Family icon */}
                <span className="text-2xl shrink-0">{familyDef?.icon ?? '📊'}</span>

                {/* Info */}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-medium text-orika-cream">{r.report_name}</p>
                    {r.is_shared ? (
                      <Badge tone="gold" size="xs"><Share2 className="h-2.5 w-2.5 inline mr-0.5" />Pinned</Badge>
                    ) : (
                      <Badge tone="neutral" size="xs"><Lock className="h-2.5 w-2.5 inline mr-0.5" />Private</Badge>
                    )}
                    {hasSchedule && (
                      <Badge tone="info" size="xs">
                        <Clock className="h-2.5 w-2.5 inline mr-0.5" />
                        {r.schedule!.frequency === 'weekly' ? 'Weekly' : 'Monthly'}
                      </Badge>
                    )}
                  </div>
                  <p className="text-xs text-orika-smoke mt-0.5">
                    {familyDef?.label} · {familyDef?.types.find((t) => t.key === type)?.label ?? type}
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button onClick={() => openSchedule(r)} title="Schedule delivery"
                    className="text-orika-smoke hover:text-orika-gold transition-colors">
                  ...
                  <button onClick={() => {
                    if (confirm(`Delete "${r.report_name}"?`)) deleteMutation.mutate(r.report_id);
                  }} title="Delete"
      ...
      <Modal open={!!editReport} onClose={() => setEditReport(null)}
        title="Edit Saved Report" size="sm" surface="light"
        footer={
        ...
        <div className="space-y-4">
          <Input label="Report Name *" value={editName}
            onChange={(e) => setEditName(e.target.value)} surface="light" />
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" checked={editShared}
              onChange={(e) => setEditShared(e.target.checked)} className="rounded" />
            <span className="text-text-on-light-muted">Pin to home dashboard (shared with all staff)</span>
          </label>
        </div>
      </Modal>

      {/* Schedule modal */}
      <Modal open={!!scheduleFor} onClose={() => setScheduleFor(null)}
        title="Schedule Delivery" size="md" surface="light"
        footer={
          <div className="flex justify-end gap-3">
            <Button variant="ghost" onClick={() => setScheduleFor(null)}>Cancel</Button>
            {scheduleFor?.schedule && (
              <Button variant="danger" size="sm"
                onClick={() => updateMutation.mutate({ id: scheduleFor.report_id, values: { schedule: null as any } })}>
                Remove schedule
              </Button>
            )}
            <Button onClick={() => updateMutation.mutate({
        ...
        <div className="space-y-5">
          ...
          {/* Frequency */}
          <Select label="Frequency" surface="light"
            value={schedule.frequency ?? 'weekly'}
            onChange={(e) => setSchedule((s) => ({ ...s, frequency: e.target.value as 'weekly' | 'monthly' }))}
            options={[
          ...
          {schedule.frequency === 'weekly' && (
            <Select label="Day of Week" surface="light"
              value={String(schedule.day_of_week ?? 1)}
              onChange={(e) => setSchedule((s) => ({ ...s, day_of_week: parseInt(e.target.value) }))}
              options={DAY_OPTIONS} />
          )}

          {/* Channels */}
          <div className="space-y-2">
            <p className="text-[0.7rem] font-medium uppercase tracking-widest text-text-on-light-muted">Deliver via</p>
          ...
          {schedule.channels?.includes('email') && (
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button size="sm" type="button" onClick={addRecipient} disabled={!recipientInput.trim()}>
                  Add
                </Button>
              </div>
              <div className="flex flex-wrap gap-1.5">
                {(schedule.recipients ?? []).map((r, i) => (
                  <span key={i} className="flex items-center gap-1 rounded-full bg-gray-100 px-2.5 py-1 text-xs text-gray-700">
                    {r}
                    <button onClick={() => removeRecipient(i)} className="text-gray-400 hover:text-red-500">×</button>
                  </span>
                ))}
              </div>
            </div>
          ...
          {schedule.channels?.includes('whatsapp') && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-gray-400">
                Summary message sent to these numbers. Include country code e.g. +2348012345678
              </p>
              <div className="flex gap-2">
                <input type="tel" value={recipientInput}
                  onChange={(e) => setRecipientInput(e.target.value)}
                  onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addRecipient(); }}}
                  placeholder="+2348012345678"
                  className="flex-1 rounded-xl border border-gray-200 px-3 py-2 text-sm focus:border-orika-gold/40 focus:outline-none"
                />
                <Button size="sm" type="button" onClick={addRecipient}>Add</Button>
              </div>
            </div>
          )}
        </div>
