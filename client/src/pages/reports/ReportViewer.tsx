...
import {
  Table2, BarChart2, Download, Bookmark, Building2, AlertTriangle,
} from 'lucide-react';
...
import {
  generateReport, generateConsolidated,
  downloadReport, createSavedReport,
} from '@services/reports';
import {
  REPORT_FAMILIES, getPresetRange } from '@lib/constants/reportsConstants';
...
export default function ReportViewer() {
  const { family = '', reportType = '' } = useParams<{ family: string; reportType: string }>();
  ...
  const [saveName,      setSaveName]      = useState(`${familyDef?.label} — ${reportDef?.label ?? ''}`);
  ...
  async function runReport() {
    ...
    try {
      if (isConsolidated) {
        const data = await generateConsolidated(family, reportType, filters.startDate, filters.endDate);
        setReport(data);
      } else {
        ...
        if (filters.compareMode !== 'none' && filters.compareStart && filters.compareEnd) {
  ...
  const saveMutation = useMutation({
    mutationFn: () => createSavedReport({
      ...
      filters:     { startDate: filters.startDate, endDate: filters.endDate, groupBy: filters.groupBy },
  ...
  if (!familyDef || !reportDef) {
    return (
      <div className="px-8 py-16 text-center">
        ...
        <Button variant="ghost" className="mt-4" onClick={() => navigate('/reports')}>Back</Button>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
            ...
            {report && (
              <>
                  ...
                  {(['table', 'chart'] as const).map((m) => (
                    <button key={m} onClick={() => setViewMode(m)}
                      className={cn(
                        ...
                        viewMode === m ? 'bg-orika-gold text-orika-black' : 'text-orika-smoke hover:text-orika-cream',
                      )}>
                      {m === 'table' ? <Table2 className="h-3.5 w-3.5" /> : <BarChart2 className="h-3.5 w-3.5" />}
                      {m === 'table' ? 'Table' : 'Chart'}
                    </button>
                  ))}
                </div>
                ...
                {(['pdf','excel','csv'] as const).map((fmt) => (
                  <Button key={fmt} size="sm" variant="secondary"
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <p className="text-xs text-orika-smoke">
            Generated {new Date(report.meta.generatedAt).toLocaleString('en-NG')}
            {isConsolidated && ' · Consolidated view (both brands)'}
            {compareReport && ` · Comparing ${filters.compareStart} → ${filters.compareEnd}`}
          </p>
        ...
        <div className="flex h-48 items-center justify-center rounded-2xl border border-white/5 bg-orika-charcoal">
          <div className="text-center">
            ...
            <p className="text-sm text-orika-smoke">Set your filters and click Run Report</p>
          </div>
        </div>
      )}
      ...
      <Modal open={showSaveModal} onClose={() => setShowSaveModal(false)}
        title="Save Report" size="sm" surface="light"
        footer={
        ...
        <div className="space-y-4">
          <Input label="Report Name *" value={saveName}
            onChange={(e) => setSaveName(e.target.value)} surface="light" />
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" checked={saveShared}
              onChange={(e) => setSaveShared(e.target.checked)} className="rounded" />
