...
import { CheckCircle, AlertCircle, ArrowRight, RotateCcw, Package } from 'lucide-react';
...
type Step = 'loading' | 'error' | 'customer' | 'driver' | 'submitting' | 'done' | 'expired';
...
export default function DeliverySignPage() {
  ...
  useEffect(() => {
    ...
    getSigningInfo(token)
      .then((data) => {
        if (data.already_signed) { setStep('done'); return; }
        setInfo(data);
        setStep('customer');
      })
      .catch((err) => {
        const msg = err?.response?.data?.message ?? err?.message ?? 'Invalid or expired link';
        if (msg.toLowerCase().includes('expire') || msg.toLowerCase().includes('already')) {
          setStep('expired');
        } else {
          setStep('error');
          setErrorMsg(msg);
  ...
  useEffect(() => {
    ...
    if (step === 'driver') {
      setTimeout(() => {
        cleanup = initPad(driverCanvasRef, driverPadRef, () => setDriverEmpty(false));
      }, 50);
    }
  ...
  if (step === 'loading') {
    return <FullPageCenter><Spinner /></FullPageCenter>;
  }
  ...
  if (step === 'done') {
    return (
      <FullPageCenter>
        ...
        <h1 className="text-2xl font-semibold text-gray-800">Delivery Confirmed</h1>
        <p className="mt-2 text-gray-500 text-center max-w-xs">
          Thank you for signing. Your delivery has been confirmed.
          Enjoy your purchase!
        </p>
      </FullPageCenter>
    );
      ...
      <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-4">
          <div className="max-w-lg mx-auto flex items-center gap-3">
            ...
            <div>
              <p className="font-semibold text-gray-800">{info?.delivery_number}</p>
        ...
        <div className="flex-1 px-4 py-6 max-w-lg mx-auto w-full space-y-5">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-2">
            ...
            {info?.delivery_address && (
              <p className="text-sm text-gray-600">
                ...
                {info.delivery_address.area ? `, ${info.delivery_address.area}` : ''}
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
            ...
            <p className="text-xs text-gray-500">
              By signing below, you confirm receipt of the above items in good condition.
            </p>
            <div
            ...
            <div className="flex items-center gap-2">
              {!custEmpty && (
                <button
                  type="button"
                  onClick={() => { custPadRef.current?.clear(); setCustEmpty(true); }}
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 space-y-3">
            ...
            <div className="flex items-center gap-2">
              {!driverEmpty && (
                <button
                  type="button"
                  onClick={() => { driverPadRef.current?.clear(); setDriverEmpty(true); }}
                  className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-700"
                >
                  <RotateCcw className="h-3 w-3" />
                  Clear
...
function StepPill({ current, total, className }: {
