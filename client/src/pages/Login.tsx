...
import {
  Mail, Lock, Eye, EyeOff, AlertCircle, Check, X, ChevronRight,
...
const QUOTES = [
  ...
  { text: 'True luxury is being able to own your own time.',                         author: 'Robert Polet' },
  { text: 'Style is a way to say who you are without having to speak.',              author: 'Rachel Zoe' },
  { text: 'Perfume is the most intense form of memory.',                             author: 'Jean Paul Guerlain' },
  ...
  { text: 'A room should be comfortable, yet elegant enough for a party.',           author: 'Billy Baldwin' },
  { text: 'The soul of luxury is craftsmanship.',                                    author: 'Frédéric Fekkai' },
];

// ── Brand pillars ─────────────────────────────────────────────────────────────
const PILLARS = [
  { icon: Sparkles,    label: 'Craftsmanship',    desc: 'Every detail considered, every finish intentional.' },
  { icon: Users,       label: 'Relationships',    desc: 'Built on trust, sustained by excellence.' },
  { icon: Package,     label: 'Provenance',       desc: 'Curated materials, traceable origins.' },
  { icon: TrendingUp,  label: 'Momentum',         desc: 'Two brands, one vision — always forward.' },
];
...
export default function Login() {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      setSplashProgress((p) => {
        ...
        if (next >= 100) { clearInterval(id); setTimeout(() => setSplashVisible(false), 700); return 100; }
        return next;
      });
    }, 300);
  ...
  if (h >= 5  && h < 12) { greeting = 'Good morning';   subGreeting = 'A fresh start — ready to build something beautiful'; }
  if (h >= 12 && h < 17) { greeting = 'Good afternoon'; subGreeting = 'The day is in full swing — momentum is everything'; }

  // ── Quote rotator
  const [currentQuote, setCurrentQuote] = useState(0);
  useEffect(() => {
    const id = setInterval(() => setCurrentQuote((q) => (q + 1) % QUOTES.length), 7000);
    return () => clearInterval(id);
  }, []);
  ...
  useEffect(() => {
    ...
    let frame: number;
    const resize = () => { w = canvas.width = window.innerWidth; ch = canvas.height = window.innerHeight; };
  ...
  function openLogin() { setError(null); setLoginModalOpen(true); }
  function closeLogin() {
    setLoginModalOpen(false);
    // BUG FIX: clear credentials + error so they don't persist across sessions
    setEmail(''); setPassword(''); setError(false as unknown as null); setShowPassword(false);
  }
  function openForgot() {
    setLoginModalOpen(false);
  ...
  function closeForgot() { setForgotModalOpen(false); setForgotSuccess(false); }
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    setError(null);
    if (!email || !password) { setError('Please enter both email and password.'); triggerShake(); return; }
    setIsLoading(true);
    try {
  ...
  return (
    <div className="min-h-screen relative animate-app-in bg-orika-black font-body text-orika-cream overflow-x-hidden">
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-6 lg:px-12 pt-10 pb-36 min-h-screen flex flex-col transition-all duration-700 ${loginModalOpen ? 'blur-md scale-95 opacity-40 pointer-events-none' : ''}`}>
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center mb-20">
          ...
          <div className="flex-1 text-center lg:text-left">
            ...
            <p className="font-display italic font-light text-xl text-orika-cloud mb-6">Where luxury meets intelligence</p>
            <p className="font-light text-sm md:text-base text-orika-cloud leading-relaxed max-w-2xl mx-auto lg:mx-0">
              The central command for two distinct luxury brands — managing customer relationships,
              inventory, retail partners, and operations with precision.
            </p>
            <div className="flex flex-wrap justify-center lg:justify-start gap-4 mt-8">
              <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-medium tracking-wide uppercase bg-living-sage/10 text-living-sage border border-living-sage/25">
                <span className="w-1.5 h-1.5 rounded-full bg-living-sage animate-pulse" /> Orika Living
              </span>
              <span className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-medium tracking-wide uppercase bg-bejewelled-rose/10 text-bejewelled-rose border border-bejewelled-rose/25">
                <span className="w-1.5 h-1.5 rounded-full bg-bejewelled-rose animate-pulse" /> Bejewelled
              </span>
            </div>
          </div>
        ...
        <div className="mt-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {PILLARS.map((p) => {
              const Icon = p.icon;
              return (
                <div key={p.label} className="rounded-2xl border border-orika-graphite bg-orika-charcoal/60 backdrop-blur-sm p-5 hover:border-orika-gold/30 hover:-translate-y-0.5 transition-all group">
                  ...
                  <p className="font-semibold text-sm text-orika-cream mb-1">{p.label}</p>
                  <p className="font-light text-xs text-orika-smoke leading-relaxed">{p.desc}</p>
                </div>
              );
            })}
          </div>
      ...
      <div className={`fixed bottom-8 lg:bottom-12 left-1/2 -translate-x-1/2 z-40 transition-all duration-500 ${loginModalOpen ? 'translate-y-32 opacity-0' : ''}`}>
        <button
          onClick={openLogin}
      ...
      {loginModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-orika-black/60 backdrop-blur-xl" onClick={closeLogin} />
          <div className="relative w-full max-w-[420px] bg-orika-cream rounded-3xl p-8 lg:p-10 shadow-[0_40px_100px_rgba(0,0,0,0.8)] animate-app-in border border-white/20">
            <button onClick={closeLogin} className="absolute top-6 right-6 text-orika-smoke hover:text-orika-black transition-colors p-2 bg-white/50 rounded-full hover:bg-white" aria-label="Close">
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="w-[80px] h-[80px] mx-auto rounded-full bg-white border border-orika-cloud/50 flex items-center justify-center mb-6 shadow-sm p-2 overflow-hidden">
              <img src="/assets/images/logos/orika-logo-black.png" alt="Orika" className="w-full h-full object-contain" />
            </div>
            ...
            <form onSubmit={handleLogin} noValidate>
              ...
              <div className="mb-6">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••"
                  />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-orika-smoke/70 hover:text-orika-black transition-colors" aria-label="Toggle password visibility">
              ...
              <div className="flex items-center justify-between mb-8 px-1">
                ...
                <button type="button" onClick={openForgot} className="text-xs font-medium text-orika-black hover:text-orika-gold transition-colors">
                  Forgot password?
                </button>
              </div>
      ...
      {forgotModalOpen && (
        <div className="fixed inset-0 z-[8000] bg-orika-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-app-in" onClick={closeForgot}>
          <div className="relative w-full max-w-[420px] bg-orika-cream border border-white/20 rounded-3xl p-8 lg:p-10 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={closeForgot} className="absolute top-6 right-6 text-orika-smoke hover:text-orika-black transition-colors p-2 bg-white/50 rounded-full hover:bg-white" aria-label="Close">
              <X className="w-5 h-5" />
            </button>
            {!forgotSuccess ? (
              <>
