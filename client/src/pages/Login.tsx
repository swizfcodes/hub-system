...
import {
  Mail, Lock, Eye, EyeOff, AlertCircle, Check, X, Play, ChevronRight,
} from 'lucide-react';
...
export default function Login() {
  ...
  else if (h >= 12 && h < 17) { greeting = 'Good afternoon'; subGreeting = 'The day is in full swing — momentum is everything'; }

  // ── Quote rotator
  useEffect(() => {
    const id = setInterval(() => setCurrentQuote((q) => (q + 1) % QUOTES.length), 8000);
    return () => clearInterval(id);
  }, []);
  ...
  useEffect(() => {
    ...
    let frame: number;
    const resize = () => { w = canvas.width = window.innerWidth; h2 = canvas.height = window.innerHeight; };
  ...
  return (
    <div className="min-h-screen relative animate-app-in bg-orika-black font-body text-orika-cream overflow-x-hidden">
      ...
      <div className={`relative z-10 max-w-7xl mx-auto px-6 lg:px-12 pt-10 pb-32 min-h-screen flex flex-col transition-all duration-700 ${loginModalOpen ? 'blur-md scale-95 opacity-40 pointer-events-none' : ''}`}>
        ...
        <div className="flex flex-col lg:flex-row gap-12 items-center mb-20">
          <div className="flex-1 text-center lg:text-left">
            ...
            <p className="font-light text-sm md:text-base text-orika-cloud leading-relaxed max-w-2xl mx-auto lg:mx-0">
              The central command for two distinct luxury brands — managing customer relationships, inventory, retail partners, and operations.
            </p>
          ...
          <div className="flex-1 w-full max-w-lg">
            <div className="p-8 border-l-2 border-orika-gold bg-gradient-to-br from-orika-gold/5 to-transparent rounded-r-2xl backdrop-blur-sm">
              ...
              <div className="flex items-center justify-between">
                <p className="font-body font-medium text-[0.7rem] text-orika-gold tracking-wider uppercase">— {QUOTES[currentQuote].author}</p>
      ...
      {videoModalOpen && (
        <div className="fixed inset-0 z-[8000] bg-orika-black/95 backdrop-blur-md flex items-center justify-center p-4 lg:p-8" onClick={() => setVideoModalOpen(null)}>
          <div className="relative w-full max-w-[840px] aspect-video bg-orika-black border border-orika-gold/15 rounded-2xl overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.6)] animate-scale-in">
            <button onClick={() => setVideoModalOpen(null)} className="absolute -top-12 right-0 text-orika-cream/70 hover:text-orika-cream p-2 transition-colors" aria-label="Close">
              <X className="w-6 h-6" />
            </button>
            <iframe src={`https://www.youtube.com/embed/${videoModalOpen}?autoplay=1&rel=0&modestbranding=1`} className="w-full h-full border-none" allow="autoplay; encrypted-media" allowFullScreen title="Orika video" />
          </div>
        </div>
      )}
      ...
      {forgotModalOpen && (
        <div className="fixed inset-0 z-[8000] bg-orika-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-app-in" onClick={() => { setForgotModalOpen(false); setForgotSuccess(false); }}>
          <div className="relative w-full max-w-[420px] bg-orika-cream border border-white/20 rounded-3xl p-8 lg:p-10 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => { setForgotModalOpen(false); setForgotSuccess(false); }} className="absolute top-6 right-6 text-orika-smoke hover:text-orika-black transition-colors p-2 bg-white/50 rounded-full hover:bg-white" aria-label="Close">
              <X className="w-5 h-5" />
            </button>
            {!forgotSuccess ? (
              <>
                ...
                <form onSubmit={(e) => { e.preventDefault(); setIsLoading(true); setTimeout(() => { setIsLoading(false); setForgotSuccess(true); }, 1200); }}>
