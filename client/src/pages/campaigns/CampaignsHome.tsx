...
import { CampaignStatusBadge, CampaignTypePill } from '@components/campaigns/CampaignComponents';
...
export default function CampaignsHome() {
  ...
  const avgOpen  = (() => {
    const sentCamps = campaigns.filter((c) => c.status === 'sent' && c.delivered_count > 0);
    if (!sentCamps.length) return 0;
    const total = sentCamps.reduce((s, c) => s + (c.opened_count / c.delivered_count) * 100, 0);
  ...
  const sendMutation = useMutation({
    ...
    onError: (err) => { showToast.error(errMsg(err)); setSendingId(null); },
  });
  ...
  return (
    ...
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
        {[
          ...
          <div key={kpi.label} className="rounded-2xl border border-white/5 bg-orika-charcoal px-4 py-3">
            ...
            <p className="font-display text-2xl font-light tabular-nums" style={{ color: kpi.color }}>{kpi.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-center">
        <Tabs tabs={STATUS_TABS} active={statusFilter} onChange={setStatusFilter} surface="dark" variant="underline" />
        <div className="flex gap-2 ml-auto">
          {['all', 'email', 'whatsapp'].map((t) => (
            <button key={t} onClick={() => setTypeFilter(t)}
              ...
              {t === 'all' ? 'All types' : t === 'email' ? '📧 Email' : '💬 WhatsApp'}
            </button>
          ))}
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center rounded-2xl border border-white/5 bg-orika-charcoal">
          ...
          <Button variant="ghost" className="mt-4" onClick={() => navigate('/campaigns/new')}>
          ...
          <table className="w-full min-w-[720px] text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-orika-charcoal">
                {['Name', 'Type', 'Recipients', 'Delivered', 'Open Rate', 'Scheduled / Sent', 'Status', ''].map((h) => (
            ...
            <tbody className="divide-y divide-white/5">
              {campaigns.map((campaign) => {
                ...
                return (
                  <tr key={campaign.campaign_id} className="bg-orika-charcoal hover:bg-orika-graphite/20 transition-colors">
                    <td className="px-4 py-3">
                      ...
                      {campaign.subject_line && (
                        <p className="text-xs text-orika-smoke truncate max-w-[200px]">{campaign.subject_line}</p>
                      )}
                    </td>
                    <td className="px-4 py-3"><CampaignTypePill type={campaign.campaign_type} /></td>
                    <td className="px-4 py-3 tabular-nums text-orika-smoke">{campaign.recipient_count.toLocaleString()}</td>
                    <td className="px-4 py-3 tabular-nums text-orika-smoke">{campaign.delivered_count.toLocaleString()}</td>
                    <td className="px-4 py-3 tabular-nums text-orika-cream">{campaign.campaign_type === 'email' ? `${openRate}%` : '—'}</td>
                    <td className="px-4 py-3 text-orika-smoke">
                      {campaign.sent_at ? fmtDate(campaign.sent_at)
                        : campaign.scheduled_at ? fmtDate(campaign.scheduled_at)
                        : '—'}
                    </td>
                    <td className="px-4 py-3"><CampaignStatusBadge status={campaign.status} size="xs" /></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        {campaign.status === 'draft' && campaign.recipient_count > 0 && (
                          <button
                            onClick={() => { setSendingId(campaign.campaign_id); sendMutation.mutate(campaign.campaign_id); }}
                        ...
                        {['draft', 'queued'].includes(campaign.status) && (
                          <button onClick={() => cancelMutation.mutate(campaign.campaign_id)}
                            title="Cancel" className="text-orika-smoke hover:text-state-danger transition-colors">
