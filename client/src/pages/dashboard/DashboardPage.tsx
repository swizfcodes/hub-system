...
export default function DashboardPage() {
  const { businesses: brandedBusinesses } = useBranding();
  const BRAND_OPTIONS = [
    { value: "combined", label: "Combined", icon: "🔗" },
    ...brandedBusinesses.map((b) => ({
  ...
  // Dashboard controls
  const [brand, setBrand] = useState<string>("combined");
      ...
      <div className="flex flex-col gap-2 border-b border-white/5 px-4 sm:px-8 py-3 flex-shrink-0 sm:flex-row sm:items-center sm:justify-between">
        ...
        {activeTab === "dashboard" && (
          <div className="flex items-center gap-2 overflow-x-auto hide-scrollbar -mx-4 px-4 sm:mx-0 sm:px-0 sm:flex-wrap sm:justify-end">
            {/* Brand toggle */}
            <div className="flex rounded-lg border border-white/10 bg-brand-charcoal overflow-hidden">
              {BRAND_OPTIONS.map((b) => (
                <button
                  key={b.value}
