...
import { ArrowLeft, Upload, Check, Copy, MessageCircle, AlertCircle } from 'lucide-react';
import { placeOrder, submitProofOfPayment } from '@services/salesCampaign';
import { checkoutSchema, type CheckoutFormValues, NIGERIAN_STATES } from '@lib/constants/salesCampaignConstants';
...
export default function Checkout() {
  ...
  const { state }            = useLocation() as { state?: { cart: CartItem[]; campaign: SalesCampaign } };
  const navigate             = useNavigate();
  const source               = new URLSearchParams(window.location.search).get('ref');

  // Restore cart from session if navigating directly
  const cart: CartItem[]     = state?.cart ?? JSON.parse(sessionStorage.getItem('sf_cart') ?? '[]');
  ...
  async function onSubmitDetails(values: CheckoutFormValues) {
    ...
    try {
      const result = await placeOrder(business!, slug!, {
        ...
        delivery_address: values.fulfilment_type === 'delivery' ? values.delivery_address : undefined,
    ...
    try {
      ...
      if (proofUrl) {
        try {
          ...
          setApiError(e2?.response?.data?.message ?? 'Proof submission failed. Please try again.');
        }
      } else {
        setApiError('Upload failed. Please enter the proof URL manually below.');
      }
    } finally {
  ...
  function openWhatsApp() {
    ...
    const msg   = encodeURIComponent(`Hi! I've transferred payment for order ${order?.order_number}. Please confirm. Tracking: ${window.location.origin}/orders/${business}/${order?.tracking_token}`);
      ...
      <div className="border-b border-white/8 px-4 py-4 flex items-center gap-4 max-w-2xl mx-auto">
        ...
        <div>
          <p className="font-semibold text-white">
            {step === 'details' ? 'Your Details' : step === 'payment' ? 'Complete Payment' : step === 'done' ? 'Order Confirmed' : 'Checkout'}
          </p>
          {campaign && <p className="text-xs text-gray-500">{campaign.campaign_name}</p>}
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
        ...
        {step === 'details' && (
          <form onSubmit={handleSubmit(onSubmitDetails)} className="space-y-6">
            ...
            <div className="rounded-2xl border border-white/8 bg-white/5 p-5 space-y-3">
              <p className="text-sm font-semibold text-gray-300 mb-3">Order Summary</p>
              {cart.map(item => (
                <div key={item.campaign_product_id} className="flex items-center gap-3">
                  {item.image_url && <img src={item.image_url} alt="" className="h-10 w-10 rounded-lg object-cover shrink-0" />}
            ...
            <div className="space-y-4">
              <p className="text-sm font-semibold text-gray-300">Your Information</p>
              <Field label="Full name" error={errors.customer_name?.message}>
                <input {...register('customer_name')} placeholder="Ada Okonkwo"
                  className={INPUT_CLASS} />
              </Field>
              <Field label="Phone number" error={errors.customer_phone?.message}>
                <input {...register('customer_phone')} type="tel" placeholder="+234 801 234 5678"
                  className={INPUT_CLASS} />
              </Field>
            ...
            <div className="space-y-3">
              <p className="text-sm font-semibold text-gray-300">Delivery or Pickup</p>
                  ...
                  <Controller key={ft} name="fulfilment_type" control={control} render={({ field }) => (
                    <button type="button" onClick={() => field.onChange(ft)}
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        {ft === 'delivery' ? 'We deliver to you' : `Collect from ${campaign?.store_location ?? 'our store'}`}
                      </p>
                    </button>
                  )} />
                ))}
              ...
              {fulfilmentType === 'delivery' && (
                <div className="space-y-3 pt-2">
                  <Field label="Delivery address" error={errors.delivery_address?.line1?.message}>
                    <input {...register('delivery_address.line1')} placeholder="Street address / house number"
                      className={INPUT_CLASS} />
                  </Field>
                  <div className="grid grid-cols-2 gap-3">
                    <Field label="City" error={errors.delivery_address?.city?.message}>
                      <input {...register('delivery_address.city')} placeholder="Lagos"
                        className={INPUT_CLASS} />
                    </Field>
                    <Field label="State" error={errors.delivery_address?.state?.message}>
                      <Controller name="delivery_address.state" control={control} render={({ field }) => (
                        <select {...field} className={cn(INPUT_CLASS, 'appearance-none')}>
                  ...
                  <Field label="Landmark (optional)">
                    <input {...register('delivery_address.landmark')} placeholder="Near Shoprite, next to..."
                      className={INPUT_CLASS} />
                  </Field>
                </div>
              )}
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                <Controller name="payment_method" control={control} render={({ field }) => (
                  <>
                    <PaymentOption
                      value="paystack" current={field.value} onChange={field.onChange}
                    ...
                    {bankAccounts.length > 0 && (
                      <PaymentOption
                        value="bank_transfer" current={field.value} onChange={field.onChange}
                ...
                {paymentMethod === 'bank_transfer' && bankAccounts.length > 0 && (
                  <div className="rounded-xl border border-white/8 bg-white/5 p-4 space-y-3 mt-2">
                    <p className="text-xs text-gray-400 font-medium">Select account to transfer to:</p>
            ...
            <p className="text-center text-xs text-gray-600">
              By placing an order you agree to our terms. Your contact details will be used to process your order.
            </p>
          </form>
        )}
        ...
        {step === 'payment' && order && (
          <div className="space-y-6">
            ...
            <div className="rounded-xl border border-white/8 bg-white/5 p-4 text-sm text-gray-400 space-y-1.5">
              <p>1. Transfer <strong className="text-white">{fmtMoney(order.total_amount)}</strong> to the account above</p>
              <p>2. Save your bank receipt / screenshot</p>
              <p>3. Upload it below — we'll reserve your items immediately</p>
              <p>4. We verify and confirm your order (usually within the hour)</p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <p className="text-center text-xs text-gray-600">— or enter the receipt/transaction URL —</p>
        ...
        {step === 'done' && order && (
          <div className="space-y-6 text-center py-8">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white mb-2">Receipt Submitted!</h2>
            ...
            <p className="text-xs text-gray-500">
              Track your order anytime at the link below. We'll also send updates to your WhatsApp.
            </p>
            ...
            <a href={`/c/${business}/${slug}`} className="block text-xs text-gray-600 hover:text-gray-400 transition-colors">
              ← Back to campaign
            </a>
          </div>
        )}
...
function PaymentOption({ value, current, onChange, label, desc, icon }: {
  value: string; current: string; onChange: (v: string) => void; label: string; desc: string; icon: string;
}) {
  return (
