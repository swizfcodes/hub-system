...
import { Check, Clock, Package, Truck, Store, X, Upload, RefreshCw } from 'lucide-react';
import { getOrderTracking, submitProofOfPayment } from '@services/salesCampaign';
...
const PICKUP_STEPS = ['pending','proof_submitted','confirmed','ready_for_pickup','completed'];
...
export default function OrderTrackingPage() {
  ...
  useEffect(() => { fetchOrder(); }, [business, token]);
  ...
  async function submitProof() {
    ...
    try {
      ...
      if (!url) { setProofError('Please upload a file or enter a URL.'); return; }

      // We need the order_id — but tracking only returns safe fields.
      // We'll look it up via a dedicated endpoint.
      const lookupRes = await fetch(`/api/c/track/${business}/${token}/order-id`);
      ...
      setProofError(e?.response?.data?.message ?? 'Submission failed. Please try again.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  if (error || !order) return (
    <div className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-4 text-center">
      ...
      <p className="text-gray-400 text-sm">{error ?? 'Invalid tracking link.'}</p>
    </div>
  );
  ...
  const statusMeta = ORDER_STATUS_META[order.status] ?? { label: order.status, color: '#888' };
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 space-y-8">
        ...
        <div className="text-center">
          ...
          <p className="text-gray-400 text-sm mt-3 max-w-sm mx-auto">{order.status_message}</p>
        </div>
        ...
        <div className="rounded-2xl border border-white/8 bg-white/5 overflow-hidden">
          ...
          <div className="px-5 py-4 border-t border-white/8 flex justify-between items-center">
            ...
            <span className="text-lg font-bold text-white">{fmtMoney(order.total_amount)}</span>
          </div>
        </div>
        ...
        {proofDone && (
          <div className="rounded-2xl border border-green-500/30 bg-green-500/10 px-5 py-4 text-center">
            ...
            <p className="text-sm text-gray-400 mt-1">We're verifying your payment. This page will update automatically.</p>
          </div>
        )}
