...
import {
  Rocket, CheckCircle, Package, /*ExternalLink*/
  MapPin, Phone, Clock, PenLine, AlertTriangle,
} from 'lucide-react';
...
import {
  getDelivery, dispatchDelivery, markDelivered, getTracking,
  packingSlipUrl,
...
export default function DeliveryDetail() {
  ...
  if (!delivery) {
    return (
      <div className="px-8 py-16 text-center">
        ...
        <Button variant="ghost" className="mt-4" onClick={() => navigate('/logistics')}>
          Back to Logistics
        </Button>
      </div>
    );
  ...
  const hasBothSigs = !!delivery.customer_signature && !!delivery.driver_signature;
    ...
    <div className="px-4 sm:px-8 py-6 max-w-5xl mx-auto space-y-6">
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            ...
            {['dispatched', 'picked_up', 'in_transit'].includes(delivery.status) && (
              <Button
                variant="secondary"
                size="sm"
      ...
      {delivery.status === 'dispatched' && !isSigned && (
        <div className="flex items-start gap-3 rounded-2xl border border-orika-gold/30 bg-orika-gold/5 px-5 py-4">
          ...
          <div>
            <p className="text-sm font-medium text-orika-gold">Awaiting customer signature</p>
            <p className="mt-0.5 text-xs text-orika-gold/70">
              WhatsApp signing link was sent to {delivery.whatsapp_number ?? 'customer'}.
              Customer must sign and pass phone to driver.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-white/5 bg-orika-charcoal p-6 space-y-4">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-sm font-medium text-orika-cream">{delivery.contact_name}</p>
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-orika-cloud">
              ...
              {addr.landmark && <p className="text-xs text-orika-smoke">Near {addr.landmark}</p>}
            </div>
          </div>
      ...
      {isSigned && hasBothSigs && (
        <div className="rounded-2xl border border-green-500/20 bg-orika-charcoal p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-green-400">Proof of Delivery — Signed</p>
            {delivery.signed_at && (
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="mb-2 text-xs text-orika-smoke">Customer Signature</p>
              <img
          ...
          <ol className="relative border-l border-white/10 space-y-4 pl-5">
            {trackingData.map((entry: any) => (
              <li key={entry.track_id} className="relative">
                ...
                <p className="text-sm font-medium text-orika-cream">{entry.message}</p>
