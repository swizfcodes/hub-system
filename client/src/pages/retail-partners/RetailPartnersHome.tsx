...
export default function RetailPartnersHome() {
  ...
  // Overview KPIs
  const totalOutstanding = partners.reduce((s, p) => s + (p.outstanding_balance ?? 0), 0);
    ...
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <KpiCard icon={Users} label="Active Partners" value={String(activeCount)} />
          <KpiCard icon={Package} label="Units on Consignment" value={totalUnitsHeld.toLocaleString()} />
          <KpiCard icon={Banknote} label="Outstanding Balance" value={fmtMoney(totalOutstanding, currency)} highlight />
        </div>
      )}
          ...
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-orika-charcoal">
                {['Code', 'Partner', 'Type', 'Units Held', 'Outstanding', 'Cycle', ''].map((h) => (
            ...
            <tbody className="divide-y divide-white/5">
              {filtered.map((partner) => (
                <tr key={partner.partner_id}
                  ...
                  onClick={() => navigate(`/retail-partners/${partner.partner_id}`)}
                >
                  <td className="px-4 py-3">
                    <span className="font-mono text-xs text-orika-gold">{partner.partner_code}</span>
                  </td>
                  <td className="px-4 py-3">
                    <p className="font-medium text-orika-cream">{partner.display_name}</p>
                    {partner.company_name && (
                      <p className="text-xs text-orika-smoke">{partner.company_name}</p>
                    )}
                  </td>
                  <td className="px-4 py-3">
                  ...
                  <td className="px-4 py-3 tabular-nums">
                    <span className={(partner.outstanding_balance ?? 0) > 0 ? 'text-amber-400' : 'text-orika-smoke'}>
...
function KpiCard({ icon: Icon, label, value, highlight = false }: {
  icon: typeof Users; label: string; value: string; highlight?: boolean;
}) {
  return (
    <div className="rounded-2xl border border-white/5 bg-orika-charcoal px-5 py-4">
      <div className="flex items-center gap-2 mb-1.5">
        ...
        <p className="text-xs uppercase tracking-widest text-orika-smoke">{label}</p>
      </div>
      <p className={`font-display text-2xl font-light tabular-nums ${highlight ? 'text-amber-400' : 'text-orika-cream'}`}>
        {value}
      </p>
    </div>
  );
