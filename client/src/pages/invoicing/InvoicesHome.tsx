...
import { InvoiceStatusBadge, InvoiceKpiStrip, InvoiceAgingBuckets } from '@components/invoicing/InvoiceDisplay';
import { InvoiceFormModal } from '@components/invoicing/InvoiceFormModal';
import { RecordPaymentModal, SendInvoiceModal } from '@components/invoicing/InvoiceModals';
    ...
    <div className="px-4 sm:px-8 py-6 max-w-7xl mx-auto space-y-6">
      ...
      {listLoading ? (
        ...
        <div className="py-16 text-center">
          <p className="text-sm text-orika-smoke">No invoices in this filter.</p>
          ...
          <table className="w-full min-w-[700px] text-sm">
            <thead>
              <tr className="border-b border-white/5 bg-orika-charcoal">
                {['Invoice', 'Customer', 'Issued', 'Due', 'Amount', 'Outstanding', 'Status', ''].map((h) => (
                  <th
                    key={h}
            ...
            <tbody className="divide-y divide-white/5">
              {invoices.map((inv) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <p className="font-medium text-orika-cream">{inv.contact_name}</p>
                  </td>
                  <td className="px-4 py-3 text-orika-smoke">{fmtDate(inv.issue_date)}</td>
                  <td className={cn(
                    'px-4 py-3',
                    inv.status === 'overdue' ? 'font-medium text-red-400' : 'text-orika-smoke',
                  )}>
                  ...
                  <td className="px-4 py-3 tabular-nums">
                    <span className={cn(
                      inv.amount_outstanding > 0 ? 'text-amber-400' : 'text-orika-smoke',
                    )}>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      {inv.amount_outstanding > 0 && inv.status !== 'voided' && (
                        <button
