...
export interface PendingTransaction {
  ...
  conflict_type?:    'out_of_stock' | 'session_closed' | 'duplicate' | 'validation';
...
export interface XReport {
  ...
  revenue: { cash_total: number; transfer_total: number; card_total: number; total_revenue: number };
  cash_drawer: { opening_float: number; cash_sales: number; expected_cash_on_hand: number };
}

export interface ZReport extends Omit<XReport, 'report_type' | 'snapshot_time' | 'cash_drawer'> {
  report_type:  'Z';
  closed_at:    string;
  cash_drawer: {
    opening_float:  number;
