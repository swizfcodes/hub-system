...
export interface SecurityStats {
  ...
  recent_events:      { log_id: string; occurred_at: string; user_name: string; module: string; action: string; ip_address: string | null }[];
...
export const ACTION_META: Record<ActionKey, { label: string; color: string; description: string }> = {
  view:    { label: 'View',    color: '#4E9AF1', description: 'Read records'              },
  create:  { label: 'Create', color: '#2D9CDB', description: 'Add new records'            },
  edit:    { label: 'Edit',   color: '#C9A86C', description: 'Modify existing records'    },
  delete:  { label: 'Delete', color: '#EF4444', description: 'Remove records'             },
  approve: { label: 'Approve',color: '#7B68EE', description: 'Approve/reject workflows'  },
  export:  { label: 'Export', color: '#9E9891', description: 'Download/export data'       },
};

export const RECORD_SCOPE_META = {
  all:  { label: 'All records',   description: 'Can access every record in this module'           },
  own:  { label: 'Own records',   description: 'Can only access records they created'              },
  team: { label: 'Team records',  description: 'Can access records from their direct reports'      },
};
...
export const HEALTH_CHECKS = [
  { id: 'inactive_accounts',  label: 'Inactive accounts (90+ days)', severity: 'warn',  href: '/security/users' },
  { id: 'no_2fa',             label: 'Users without 2FA enabled',    severity: 'info',  href: '/security/users' },
  { id: 'failed_logins',      label: 'Failed login attempts today',  severity: 'error', href: '/security/audit' },
  { id: 'pending_invites',    label: 'Pending unaccepted invites',   severity: 'info',  href: '/security/users' },
] as const;
